...
class CurveDEXRouter {
  ...
  async _annotateTxCosts(quotes, toToken) {
    ...
    for (const q of quotes) {
      ...
      // Ранжируем по ГАРАНТИРОВАННОМУ выходу (_execOutput), а не по оптимистичной
      // цифре: маршрут, который не может исполнить свою котировку, лучшей цены и
      // не предлагает (Ник, 10.08). Фолбэк на сырой выход — для вызовов без аннотации.
      const out = parseFloat(q._execOutput != null ? q._execOutput : (q.outputAmount || '0'));
  ...
  /**
   * Требуемый запас (%) для конкретной пары и суммы — для поверхностей, которые
   * считают min_dy САМИ, мимо getQuote (страница пула: там маршрут выбирает
   * человек, но премия испаряется точно так же).
   *
   * Опора — самый глубокий пул этой пары. Премия котировки над ним и есть та
   * часть, которая исчезает от одной чужой сделки; min_dy, посчитанный от неё,
   * недостижим (реверт 09.08 «Exchange resulted in fewer coins than expected»).
   * Ошибка/нет опоры → возвращаем слипадж пользователя без изменений.
   *
   * @param {string} fromToken
   * @param {string} toToken
   * @param {bigint} amountWei вход
   * @param {bigint} outputWei выход, от которого поверхность считает минимум
   * @param {number} userSlippage % с кнопок
   * @returns {Promise<number>} % запаса (>= userSlippage)
   */
  async requiredHeadroomPct(fromToken, toToken, amountWei, outputWei, userSlippage = 0.5) {
    try {
      const pools = this._findAllPools(fromToken, toToken);
      if (!pools || !pools.length) return userSlippage;
      const deepest = pools[0]; // _findAllPools отдаёт по убыванию TVL
      const { iFrom, iTo, isUnderlying } = this._getPoolIndices(deepest, fromToken, toToken);
      if (iFrom === -1 || iTo === -1) return userSlippage;
      // Один ретрай: публичные RPC отдают 429 пачками, а молчаливый провал здесь
      // схлопывает запас до слипаджа пользователя — то есть ровно к реверту.
      let refWei = null;
      for (let attempt = 0; attempt < 2 && !refWei; attempt++) {
        if (attempt) await new Promise(r => setTimeout(r, 400));
        try { refWei = await this._getDy(deepest, iFrom, iTo, BigInt(amountWei), !!isUnderlying); }
        catch (e) { if (attempt) throw e; }
      }
      if (!refWei || BigInt(refWei) <= 0n) return userSlippage;
      const premiumPct = Number(BigInt(outputWei) * 1000000n / BigInt(refWei)) / 10000 - 100;
      if (!isFinite(premiumPct) || premiumPct <= 0) return userSlippage;
      // Премия выше потолка запаса означает не «сверхпремиальный маршрут», а то,
      // что ОПОРА не держит эту сумму: прямой пул trUSD/USDC на $4k отдаёт за
      // 50 600 trUSD всего 1 679 USDC, и честный маршрут выглядит рядом с ним
      // премией в 2910% (замер 18.09). Считать по такой опоре нельзя — запас
      // упирался в потолок и min_dy уезжал на −5% при выбранных 0.05%.
      if (premiumPct > 5) return userSlippage;
      return Math.max(userSlippage, Math.min(premiumPct * 1.2, 5));
    } catch {
      return userSlippage;
    }
  }
  ...
  async getQuote(fromToken, toToken, amount, fromDecimals, toDecimals, slippage = 0.5, userAddress = null) {
    ...
    // Требуемый запас и ГАРАНТИРОВАННЫЙ выход — ДО сортировки: сравнивать маршруты
    // надо по тому, что они реально доставят.
    //
    // Премия над самым глубоким пулом пары — ровно та часть котировки, которая
    // исчезает от одной чужой сделки. Замер 10.08: пул $47k дал +0.49%, за ночь
    // уехал до −0.47%, симулятор Rabby на ТЕХ ЖЕ байтах перевернулся с success на
    // revert «Exchange resulted in fewer coins than expected». Роутер, сортируя по
    // максимальному выходу, систематически выбирал именно такой пул — то есть самую
    // нестабильную котировку — и вооружал её слипаджем 0.02% (дефолт stable-бакета).
    // Поэтому запас под премией не может быть меньше самой премии, а маршрут со
    // «свободными» +0.5%, которые он не удержит, проигрывает честному глубокому пулу.
    const PREMIUM_HEADROOM_MULT = 1.2;   // премия успевает подрасти между котировкой и подписью
    const PREMIUM_HEADROOM_CAP = 5;      // потолок, чтобы битый пул не открыл дыру на весь размер
    // Опорный пул годится, только если сделка в него ПОМЕЩАЕТСЯ. Потерял на
    // собственном импакте больше, чем весь наш потолок запаса, — он уже не
    // «честный курс», а мелкий пул, и премия над ним мнимая (замер 18.09:
    // trUSD→USDC 50 600, baseline −96.7%, «премия» 2910%, запас 5% вместо 0.05%).
    const _depthQ = quotes.find(q => q._depthBaselineWei);
    const _depthImpact = _depthQ && _depthQ._depthBaselineImpact != null
      ? Number(_depthQ._depthBaselineImpact) : null;
    const _depthUsable = !(_depthImpact != null && isFinite(_depthImpact)
      && Math.abs(_depthImpact) > PREMIUM_HEADROOM_CAP);
    const depthRefWei = _depthUsable ? _depthQ?._depthBaselineWei : null;
    for (const q of quotes) {
      let headroom = slippage;
      if (q.outputAmountWei && depthRefWei && BigInt(depthRefWei) > 0n) {
        // Скейл 1e6 ⇒ шаг 0.0001%. На 1e4 премия 0.4758% усекалась до 0.47% —
        // запас выходил меньше реальной премии, то есть ровно в опасную сторону.
        const premiumPct = Number(BigInt(q.outputAmountWei) * 1000000n / BigInt(depthRefWei)) / 10000 - 100;
        if (premiumPct > 0) {
          const needed = Math.min(premiumPct * PREMIUM_HEADROOM_MULT, PREMIUM_HEADROOM_CAP);
          if (needed > headroom) { headroom = needed; q._premiumPct = premiumPct; }
        }
      }
      q._headroomPct = headroom;
      if (!q.outputAmountWei) continue;
      // floor, а не round: множитель округляем ВНИЗ, чтобы минимум не оказался
      // на волос выше расчётного.
      const factor = BigInt(Math.floor((1 - headroom / 100) * 1e6));
      q._execOutputWei = (BigInt(q.outputAmountWei) * factor / 1000000n).toString();
      q._execOutput = ethers.formatUnits(BigInt(q._execOutputWei), toDecimals);
    }
    ...
    // Минимум = тот самый гарантированный выход, по которому маршрут и выиграл.
    best.minOutputWei = best._execOutputWei || best.outputAmountWei;
    best.minOutput = ethers.formatUnits(BigInt(best.minOutputWei), toDecimals);
    best.slippage = best._headroomPct != null ? best._headroomPct : slippage;
  ...
  async buildSwapTx(quote, userAddress) {
    ...
    //     block a swap.
    if (this._usesMainnetCurveJs(quote)) {
      try {
  ...
  async ensureApproval(quote, userAddress, signer) {
    ...
    return this._approveIfShort(fromToken, spender, BigInt(quote.inputAmountWei), userAddress, signer);
  }

  /** Raise the allowance of `fromToken` for `spender` when it is below `needed`. */
  async _approveIfShort(fromToken, spender, needed, userAddress, signer) {
    ...
    // Разрешение ровно на сумму операции, не на весь баланс. Apply Michwill
    // EIP-1559 gas strategy (tip=5% of base, maxFee=2.05×base) — same as swap tx.
    ...
    const amounts = (typeof window !== 'undefined' && window.approveAmounts)
      ? window.approveAmounts(allowance, needed)
      : [BigInt(needed)];
    let last = null;
    for (const amt of amounts) {
      const approveData = token.interface.encodeFunctionData('approve', [spender, amt]);
  ...
  async _getCurveDirectQuote(fromToken, toToken, amountWei, fromDecimals, toDecimals) {
    ...
    // Импакт САМОГО baseline на этой сумме. Опорный пул считается «честным
    // курсом» только пока сделка в него помещается: пул trUSD/USDC на $4k на
    // 50 600 trUSD отдаёт 1 679 USDC (−96.7%), и любой нормальный маршрут
    // выглядит рядом с ним премией в 2910% — запас упирался в потолок и min_dy
    // уезжал на −5% при выбранных 0.05%. Потребитель — гард в getBestQuote.
    let _depthBaselineImpact = null;
    if (baseline.pool.address === pool.address) {
      _depthBaselineImpact = priceImpact;
    } else {
      try {
        const baseRate = parseFloat(ethers.formatUnits(baseline.outputWei, toDecimals))
          / parseFloat(inputFormatted);
        _depthBaselineImpact = await this._computeBaselinePriceImpact(
          baseRate, [{ pool: baseline.pool, iFrom: baseline.iFrom, iTo: baseline.iTo }],
          () => this._simulateSpotThroughHops(
            [{ pool: baseline.pool, iFrom: baseline.iFrom, iTo: baseline.iTo, isUnderlying: baseline.isUnderlying }],
            fromDecimals, toDecimals),
        );
      } catch (e) { /* спот по опорному пулу не снялся — гард отключится сам */ }
    }
    ...
    return {
      ...
      _pool: pool,
      // Выход САМОГО ГЛУБОКОГО пула этой пары. Опорная точка «честного» курса:
      // всё, что выше него, — премия перекошенного мелкого пула, а не альфа.
      // Потребитель — пол слипаджа в getBestQuote.
      _depthBaselineWei: baseline.outputWei.toString(),
      _depthBaselinePool: baseline.pool.address,
      _depthBaselineImpact,
    };
  }
   ...
   */
  /**
   * Is this quote dispatched through curve-js on mainnet? Then the token is
   * pulled by Router NG, not by the pool — whatever the source says.
   *
   * This used to be inlined in buildSwapTx while _getSpender listed the
   * sources by hand, and the two lists drifted apart: curve-direct and the
   * split/multi-path sources still approved the POOL while the tx went to the
   * Router, so the swap reverted with "ERC20: transfer amount exceeds
   * allowance" right after a successful approve (Alexandr crvecodev/1902).
   * Two sources had already been patched one at a time; one predicate ends
   * the class instead of the next case.
   */
  _usesMainnetCurveJs(quote) {
    return (
      this._chainId === 1 &&
      typeof window !== 'undefined' &&
      typeof window.curveJsReadyForChain === 'function' &&
  ...
  /**
   * Approve against the spender named by the BUILT transaction.
   *
   * ensureApproval has to name the spender before the tx exists, so it can
   * only infer it. The builders know it for a fact and report it as _spender,
   * so this runs after the build and tops up whatever the inference missed —
   * including the legacy fallback path buildSwapTx takes when curve-js throws.
   * Costs one allowance read and sends nothing when the inference was right.
   */
  async ensureApprovalForTx(txParams, quote, userAddress, signer) {
    if (!txParams || !quote) return null;
    const fromToken = quote.fromToken || '';
    if (fromToken.toLowerCase() === ETH_ADDRESS.toLowerCase()) return null;
    // Composite envelopes carry their own legs and approve them themselves.
    const spender = txParams._spender || txParams.to;
    if (!spender || typeof spender !== 'string') return null;
    const inferred = this._getSpender(quote);
    if (inferred && inferred.toLowerCase() === spender.toLowerCase()) return null;
    return this._approveIfShort(fromToken, spender, BigInt(quote.inputAmountWei), userAddress, signer);
  }

  _getSpender(quote) {
    // The builder overrides the source-by-source table below on mainnet.
    if (this._usesMainnetCurveJs(quote)) return CURVE_ROUTER_NG;
    switch (quote.source) {
      case 'weth-wrap':
      ...
      case 'curve-js-baseline':
        ...
        return CURVE_ROUTER_NG;
      case 'curve-zap-metapool':
        // Mainnet is already handled by the _usesMainnetCurveJs check above.
        // On Gnosis this goes through _buildGnosisRouterEureTx → Router NG on
        // chain 100; the legacy path (other non-mainnet) builds
        // zap.exchange_underlying directly.
