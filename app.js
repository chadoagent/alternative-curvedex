...
async function setActiveChain(key) {
  ...
  try {
    if (/^#\/(pool|yield|trade)\//.test(location.hash)) history.replaceState(null, '', location.pathname + location.search);
  } catch (e) {}
...
function handleRoute() {
  ...
  if (parts.length === 0) {
    ...
    if (parts[0] === 'trade') {
      ...
      view = 'trade';
    } else if (parts[0] === 'pool' && parts.length > 2 && !parts[1].startsWith('0x')
               && parts[2] && parts[2].startsWith('0x') && parts[2].length >= 40) {
      // #/pool/<chain>/<address> — the alert bot links here. Without the switch
      // an Arbitrum pool opens on Ethereum, finds nothing and falls back to the
      // top pool, which reads as "your link is broken".
      poolAddr = parts[2];
      view = 'pools';
      const _ch = parts[1];
      let _chTries = 0;
      const _applyChain = () => {
        if (typeof activeChainKey !== 'undefined' && _ch === activeChainKey) return;
        if (CHAINS_CONFIG && CHAINS_CONFIG.chains && CHAINS_CONFIG.chains[_ch]) {
          setActiveChain(_ch);
        } else if (_chTries++ < 30) {
          setTimeout(_applyChain, 200);   // config may still be loading
        }
      };
      _applyChain();
