...
// POOL TYPE LABEL
// ============================================================
// We used to print the raw registry id trimmed to six characters, so a
// twocrypto pool read "f-twoc" (Alexandr crvecodev/1734). Curve's own UI names
// the AMM instead, and we mirror its wording exactly so the two sites agree:
//   FXSwap / Llamma / Stableswap / Stableswap-NG / 2-coin Cryptoswap /
//   Tricrypto / 2-coin Cryptoswap-NG / 3-coin Cryptoswap-NG
// The sync part covers every pool from data we already hold; FXSwap is the one
// flavour the registry cannot tell apart (it is a twocrypto-NG pool that takes
// donations), so it is refined asynchronously from the pool metadata endpoint.
const _CRYPTO_REGISTRIES = new Set(['crypto', 'factory-crypto', 'factory-twocrypto', 'factory-tricrypto']);
const _NG_REGISTRIES = new Set(['factory-stable-ng', 'factory-twocrypto', 'factory-tricrypto']);

function poolTypeLabel(pool) {
  if (!pool) return '--';
  const reg = String(pool.registryId || '').toLowerCase();
  if (!reg) return pool.type && pool.type !== 'unknown' ? pool.type : '--';
  const isCrypto = _CRYPTO_REGISTRIES.has(reg);
  const isNg = _NG_REGISTRIES.has(reg);
  const n = Array.isArray(pool.coins) ? pool.coins.length : 2;
  if (!isCrypto) return isNg ? 'Stableswap-NG' : 'Stableswap';
  if (!isNg) return n === 3 ? 'Tricrypto' : '2-coin Cryptoswap';
  return n === 3 ? '3-coin Cryptoswap-NG' : '2-coin Cryptoswap-NG';
}

// address -> true/false once the metadata answered; null while in flight.
const _fxSwapCache = new Map();

// Curve marks a pool FXSwap when its metadata reports donations. One request
// per pool, only when its page is opened, cached for the session.
async function isFxSwapPool(pool) {
  if (!pool || !pool.address) return false;
  const key = String(pool.address).toLowerCase();
  if (_fxSwapCache.has(key)) return _fxSwapCache.get(key);
  try {
    const chain = typeof getChainKey === 'function' ? getChainKey() : 'ethereum';
    const r = await fetch(`${PRICES_BASE}/pools/${chain}/${key}/metadata`);
    if (!r.ok) { _fxSwapCache.set(key, false); return false; }
    const j = await r.json();
    const val = !!j.has_donations;
    _fxSwapCache.set(key, val);
    return val;
  } catch (e) {
    _fxSwapCache.set(key, false);
    return false;
  }
}

// Paint the label now, upgrade to FXSwap when the metadata says so.
function renderPoolTypeBadge(elId, pool, asBadge) {
  const el = document.getElementById(elId);
  if (!el) return;
  const paint = (text) => {
    const title = pool && (pool.registryId || pool.type) ? ` title="${pool.registryId || pool.type}"` : '';
    el.innerHTML = asBadge ? `<span class="pool-type-badge"${title}>${text}</span>` : text;
    if (!asBadge && title) el.title = pool.registryId || pool.type;
  };
  paint(poolTypeLabel(pool));
  if (!pool || !pool.address) return;
  const addr = String(pool.address).toLowerCase();
  isFxSwapPool(pool).then(isFx => {
    if (!isFx) return;
    const current = window.selectedPool || pool;
    if (String(current.address || '').toLowerCase() !== addr) return;  // user moved on
    paint('FXSwap');
  }).catch(() => { /* keep the sync label */ });
}
...
async function loadChainsConfig() {
  try {
    ...
    // Only follow the wallet when the site is actually CONNECTED to it. An
    // installed-but-unconnected extension still answers eth_chainId, so without
    // this guard the wallet's chain overrode every choice the user made in the
    // picker — pick any network, boot drags you back. Measured 07.09 (Alexandr):
    // his locked wallet sat on Robinhood, so the site returned to Robinhood on
    // every load and after every switch; connecting the wallet (which was on
    // Ethereum) is what appeared to "fix" it. The wallet is the source of truth
    // for the SIGNING chain, and there is no signing chain without a connection.
    let _walletLinked = false;
    try { _walletLinked = !!walletAddress || localStorage.getItem('curvedex_wallet_connected') === '1'; } catch (e) {}
    if (typeof window.ethereum !== 'undefined' && _walletLinked) {
      try {
        ...
        if (walletId && walletId !== activeChainId) {
          ...
          if (match) {
            ...
            // but ONCE per tab. Measured 07.09: when the sync does not settle
            // (extension present but not authorised), this reload re-armed
            // itself on every load and gave 70 navigations in 25 s. The chain
            // picker was unusable: every user click was undone by the next
            // reload, which reads from the outside as "the site is stuck on
            // Robinhood". A boot-time convenience may not loop.
            let _synced = false;
            try { _synced = sessionStorage.getItem('curvedex.chainSyncReloaded') === '1'; } catch (e) {}
            if (!_synced) {
              try { sessionStorage.setItem('curvedex.chainSyncReloaded', '1'); } catch (e) {}
              setTimeout(() => window.location.reload(), 60);
              return;
            }
            // Already reloaded once and still out of sync: apply in place and
            // let the user switch by hand instead of reloading forever.
            activeChainKey = walletKey;
            activeChainId = walletId;
            populateChainSelector();
          }
        }
      } catch (_) {
...
async function setActiveChain(key, opts) {
  ...
  if (typeof window.ethereum !== 'undefined' && walletAddress) {
    ...
    // The wallet request MUST be bounded: a locked or busy extension queues the
    // call until the user unlocks it, and an unbounded await freezes the chain
    // picker with no error at all (Alexandr, 07.09 — page stuck on Robinhood,
    // unfroze only when he connected the wallet). Switching the network in the
    // wallet is a courtesy; it may not hold the UI hostage.
    const walletReq = (req) => Promise.race([
      window.ethereum.request(req),
      new Promise((_, reject) => setTimeout(() => reject({ code: 'TIMEOUT' }), 3500)),
    ]);
    try {
      await walletReq({
      ...
      if (err && err.code === 4902) {
        try {
          ...
          // A hang is not a refusal: keep switching locally, the user can add
          // the network in the wallet by hand. A real rejection still aborts.
          if (!addErr || addErr.code !== 'TIMEOUT') return;
        }
  ...
  // A pool deep-link belongs to the PREVIOUS chain: the same address does not
  // exist on the new one, so after the reload the card renders empty
  // ('Select a pool') and the switch reads as broken. Drop the pool from the
  // URL before reloading.
  try {
    if (!(opts && opts.keepRoute)) {
      // Keep the user on the SAME view. Dropping the whole hash sent them back
      // to Home on every switch (Alexandr, crvecodev/2156); the pool ADDRESS
      // does belong to the previous chain, so only the address goes.
      const seg = stripChainFromHash(location.hash).replace(/^#\/?/, '').split('/').filter(Boolean);
      let next = '#/' + key;
      if (seg.length) {
        const v = (seg[0] === 'pool') ? 'pools' : seg[0];
        next += '/' + (ROUTE_VIEWS.indexOf(v) !== -1 && v !== 'pool' ? v : 'pools');
      }
...
// ---- ROUTE / CHAIN SLUG -------------------------------------------------
// The chain slug is the FIRST segment of every deep route:
//   #/ethereum/pool/0x…   #/arbitrum/trade/CRV%2FUSDC   #/base/yield/0x…
// A route without it opens on whatever chain the tab happens to be on, which
// is why an Arbitrum alert link used to land on Ethereum and a chain switch
// threw the user back to Home (Alexandr, crvecodev/2156).
// Legacy forms (#/pool/0x…, #/pool/<chain>/0x…) are still parsed: links
// already sent out in alerts must keep working.
const ROUTE_VIEWS = ['home', 'trade', 'swap', 'pools', 'yield', 'portfolio', 'pool'];
function isChainSlug(seg) {
  if (!seg || seg.startsWith('0x')) return false;
  return ROUTE_VIEWS.indexOf(seg) === -1;
}
function stripChainFromHash(h) {
  const m = (h || '').match(/^#\/([^/]+)(\/.*)?$/);
  if (m && isChainSlug(m[1])) return '#' + (m[2] || '/');
  return h || '#/';
}
function withChain(h) {
  const rest = stripChainFromHash(h).replace(/^#\/?/, '');
  return '#/' + getChainKey() + (rest ? '/' + rest : '');
}
// One canonical shape for every route we accept, so a legacy alert link is
// upgraded ONCE (in place) instead of being half-parsed everywhere.
//   #/pool/<chain>/0x…  (alert bot, pre-2026-09-07)  ->  #/<chain>/pool/0x…
//   #/yield/0x… (in-page href, old bookmark)         ->  #/<active>/yield/0x…
function canonicalHash(raw) {
  const bare = stripChainFromHash(raw);
  const legacy = bare.match(/^#\/pool\/([^/]+)\/(0x[0-9a-fA-F]{40})$/);
  if (legacy && isChainSlug(legacy[1])) return '#/' + legacy[1] + '/pool/' + legacy[2];
  const m = (raw || '').match(/^#\/([^/]+)(\/.*)?$/);
  if (m && isChainSlug(m[1])) return raw;
  return withChain(bare);
}
window.routePath = function () { return stripChainFromHash(window.location.hash); };
window.withChain = withChain;
...
// Endpoints that failed STRUCTURALLY (fetch itself rejected: CORS preflight
// without ACAO, DNS, TLS). Those are a property of the node+browser pair, not
// a transient blip -- a 60s cooldown just re-runs the failure forever, and the
// probe loop re-runs it every 90s on top (rpc.mevblocker.io: 108 console
// errors per Trade session, Alexandr 08.08). They drop out of rotation for the
// rest of the page and come back on reload, where the node gets a fresh chance.
// HTTP-level faults (429, 5xx, garbage body) stay TRANSIENT and keep the old
// cooldown -- those really do recover mid-session (Nik msg 440).
const _rpcDead = new Set();
function _isStructuralRpcError(e) {
  // fetch() rejects with TypeError only for network-layer failures; HTTP
  // status codes resolve normally, and a probe timeout aborts as AbortError.
  return !!e && (e.name === 'TypeError' || e._structuralRpc === true);
}
let _rpcOrderTs = 0;
...
async function _probeRpc(url) {
  ...
  try {
    ...
    if (_isStructuralRpcError(e)) _rpcDead.add(url);
...
function _markRpcFail(url, structural) {
  if (structural) _rpcDead.add(url);
...
function getOrderedRpcs() {
  ...
  const unreliable = [];
  const cooled = [];
  const all = _getActiveChainRpcs();
  // Never hand back an empty list: if every endpoint of the chain died, the
  // set is worthless as a filter and we fall back to trying them all.
  const pool = all.some(u => !_rpcDead.has(u)) ? all.filter(u => !_rpcDead.has(u)) : all;
  for (const u of pool) {
    const m = _rpcMeta.get(u);
...
// USD value of ONE LP token. Numerator and denominator must come from the SAME
// snapshot: dividing a cached TVL by a freshly-read on-chain totalSupply drifts
// the moment anybody deposits — frxUSD/sUSDe on 06.08 had 97 054.85 LP in the
// API against 111 971.97 on chain, and every position there read 13% short
// (Alexandr crvecodev/1812). The API hands totalSupply as a wei string, hence
// the 1e18. virtualPrice is the fallback for snapshots that carry no supply.
function lpUsdPrice(pool) {
  if (!pool) return 0;
  const supply = Number(pool.totalSupply) / 1e18;
  if (pool.tvl > 0 && supply > 0) return pool.tvl / supply;
  const vp = Number(pool.virtualPrice) / 1e18;
  return vp > 0 ? vp : 0;
}
window.lpUsdPrice = lpUsdPrice;
...
let merklAprSeries = null;
let bribesSeries = null;      // Bribes chart tab (weekly $ histogram)
...
const HIGH_VOLUME_MIN_USD = 100000;   // порог чипа «High Vol»: $100k за сутки
// Утилизация (объём/TVL) — отдельный чип: у крупного пула объём почти никогда
// не превышает собственный TVL, поэтому смешивать её с «большим объёмом» нельзя.
let highUtilOnly = false;
...
// Single owner of "what a platform total becomes at a simulated deposit". Takes ANY total
// (Curve native, Cx/Sd best, or one platform's own) and dilutes it: base (trading fees) by
// pool TVL, everything gauge-driven (CRV / extra rewards / Merkl) by gauge TVL. Decompose:
// total = base + crv + extra + merkl; crv is the residual because the platform totals don't
// expose a component breakdown. Cx/Sd LP sit in the same Curve gauge, just boosted via veCRV,
// so gauge-based dilution is the correct math for them too.
function dilutedPlatformApy(pool, totalVal, deposit) {
  ...
  if (!pool) return totalVal;
  if ((!deposit || deposit <= 0) && _crvRatio === 1) return totalVal;
  ...
  const crvPortion = Math.max(0, totalVal - baseApy - extraSum - merklApr) * _crvRatio;
...
// Displayed TOTAL: Cx/Sd best when we have it, Curve native otherwise.
function dilutedTotalApy(pool, deposit) {
  ...
  return dilutedPlatformApy(pool, raw, deposit);
}
  ...
  for (let pass = 0; pass < PASSES; pass++) {
    ...
    for (let i = 0; i < tries; i++) {
      ...
      try {
        ...
        _markRpcFail(url, _isStructuralRpcError(e));
      }
    }
  }
...
// Цена из ручки бывает ПРОТУХШЕЙ, и у неё для этого есть отметка last_updated:
// по неликвидному токену она отстаёт на сутки-двое (замер 08.09: sdYB обновлён
// 06.09 22:28 UTC при живом YB). Показанная под суммой, такая цена не
// «примерная», а неверная — у тестировщика вход стоил дешевле выхода при 0.93%
// price impact.
const _USD_PRICE_MAX_AGE = 6 * 3600 * 1000;
// Запасной путь для протухшей или отсутствующей цены — цена этой же монеты,
// посчитанная ПО ПУЛУ (coinsDetailed[].usdPrice): так её выводит и curve.finance,
// когда прямой цены нет. Берём из самого глубокого пула с этой монетой.
function _poolCoinUsdPrice(addr) {
  const lower = String(addr || '').toLowerCase();
  if (!lower || !Array.isArray(allPools)) return 0;
  let best = 0, bestTvl = -1;
  for (const p of allPools) {
    for (const c of (p.coinsDetailed || [])) {
      if (String(c.address || '').toLowerCase() !== lower) continue;
      const price = parseFloat(c.usdPrice);
      const tvl = p.tvl || p.usdTotal || 0;
      if (isFinite(price) && price > 0 && tvl > bestTvl) { best = price; bestTvl = tvl; }
    }
  }
  return best;
}
...
async function _fetchUsdPrice(addr) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Отметка приходит в UTC без зоны — без 'Z' браузер прочтёт её как местное
      // время и возраст уедет на смещение пользователя.
      const rawTs = String(j?.data?.last_updated || j?.last_updated || '');
      const stamp = rawTs ? Date.parse(/[zZ]|[+-]\d\d:?\d\d$/.test(rawTs) ? rawTs : rawTs + 'Z') : NaN;
      const fresh = !isFinite(stamp) || (Date.now() - stamp) < _USD_PRICE_MAX_AGE;
      let safe = isFinite(price) && price > 0 && fresh ? price : 0;
      if (!safe) safe = _poolCoinUsdPrice(lower); // протухла или её нет — считаем по пулу
      // Долгий miss ставим только когда цены НЕТ вовсе: протухшую перепроверяем
      // на обычном TTL, иначе свежая появится у нас через сутки.
      _usdPriceCache.set(lower, { price: safe, ts: Date.now(), miss: safe === 0 && !(price > 0) });
      return safe;
    }
    // Non-ok (typically 404 for unindexed tokens) — сперва пул, и только потом
    // долгий miss: у монеты может не быть своей цены, но быть пул с ценой.
    const viaPool = _poolCoinUsdPrice(lower);
    _usdPriceCache.set(lower, { price: viaPool, ts: Date.now(), miss: viaPool === 0 });
    return viaPool;
    ...
    if (token._isNativeETH || token.address === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        try {
          ...
          _markRpcFail(url);
        } catch (e) { _markRpcFail(url, _isStructuralRpcError(e)); }
      }
      return 0;
    }
...
function handleRoute() {
  ...
  if (rawHash !== '#/' && rawHash !== '#') {
    const canon = canonicalHash(rawHash);
    if (canon !== rawHash) {
      // replaceState, not location.hash: rewriting the hash would fire
      // hashchange and re-enter this handler.
      try { history.replaceState(null, '', location.pathname + location.search + canon); } catch (e) {}
      rawHash = canon;
    }
  }
  const _m = rawHash.match(/^#\/([^/]+)(\/.*)?$/);
  const routeChain = (_m && isChainSlug(_m[1])) ? _m[1] : null;
  // A route that names another chain switches to it; setActiveChain reloads,
  // and after the reload the slug matches so it does not switch again.
  if (routeChain && typeof activeChainKey !== 'undefined' && routeChain !== activeChainKey) {
    let _t = 0;
    const applyRouteChain = () => {
      if (CHAINS_CONFIG && CHAINS_CONFIG.chains && CHAINS_CONFIG.chains[routeChain]) {
        setActiveChain(routeChain, { keepRoute: true });
      } else if (_t++ < 30) {
        setTimeout(applyRouteChain, 200);   // config may still be loading
      }
    };
    applyRouteChain();
  }
  const hash = stripChainFromHash(rawHash);
  const parts = hash.replace('#/', '').split('/').filter(Boolean);

  let poolAddr = null;
  let tradePairRoute = null;
  let view = 'trade'; // default for unrecognized deep routes

  if (parts.length === 0) {
    view = 'home'; // landing screen (Nik crvecodev/1217)
  ...
  if (view === 'trade' && tradePairRoute) {
    ...
    const trySelect = () => {
      if (poolsByAddress.has(poolAddr.toLowerCase())) {
        ...
        const baseRoute = withChain((view === 'yield') ? '#/yield' : '#/pools');
...
function switchView(view) {
  ...
  document.body.classList.remove('page-home', 'page-trade', 'page-swap', 'page-pools', 'page-yield', 'page-portfolio');
  ...
  if (view === 'home' && window.Home) {
    // Same two-rAF trick as Portfolio: let the display flip paint first.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => window.Home.open());
    });
  }
  ...
  mainLayout.classList.toggle('no-sidebar', view === 'swap' || view === 'portfolio' || view === 'home');
  ...
  mainLayout.classList.toggle('home-active', view === 'home');
  ...
  if (mobileToggle) {
    ...
    mobileToggle.style.display = (view === 'portfolio' || view === 'home') ? 'none' : '';
  }
...
function reorderTokenBalancesForViewport() {
  ...
  for (const it of items) {
    ...
    } else if (it.rightAnchorId) {
      // Pools page: Token Balances lives in the right sidebar under Pool Stats,
      // so the center column is chart + Recent Activity only (Alexandr 05.08).
      const anchor = document.getElementById(it.rightAnchorId);
      if (anchor && (el.parentElement !== anchor.parentElement || el.previousElementSibling !== anchor)) {
...
function updateHash() {
  if (!selectedPool && currentView !== 'trade' && currentView !== 'swap') {
    if (currentView === 'yield') window.location.hash = withChain('#/yield');
    else if (currentView === 'pools') window.location.hash = withChain('#/pools');
    else window.location.hash = withChain('#/trade');
    return;
  }
  if (currentView === 'swap') {
    window.location.hash = withChain('#/swap');
    return;
  }
  if (currentView === 'trade') {
    if (selectedPair) {
      window.location.hash = withChain('#/trade/' + encodeURIComponent(selectedPair.name));
    } else {
      window.location.hash = withChain('#/trade');
    }
    return;
  }
  const addr = selectedPool.address;
  if (currentView === 'yield') {
    window.location.hash = withChain('#/yield/' + addr);
  } else {
    window.location.hash = withChain('#/pool/' + addr);
  }
}
...
// Curve API returns `coins` either as objects (/getPools: address, symbol,
// decimals, poolBalance, usdPrice) or as plain symbols (/getVolumes). Return the
// object form when that is what we got, else null — callers store it as
// coinsDetailed.
function _detailedCoins(p) {
  const c = Array.isArray(p.coins) ? p.coins : null;
  return (c && c.length && c[0] && typeof c[0] === 'object' && c[0].address) ? c : null;
}
...
async function loadFromCache(cacheData) {
  ...
  updateHeaderStats();
  renderPoolList();
  _linkBridgedTokenGrades();
  detectSuspectOraclePools();
  loadOraclePoolFilters();
}
...
// === Curve official pool-filter list (Alexandr crvecodev/1407-1410) ===
// The canonical curve.finance UI hides pools whose oracle asset-type token
// points at an oracle contract that is NOT verified on Etherscan (a curated
// safety list). Source of truth: prices.curve.finance/v1/chains/pool_filters.
// We mark those pools p.oracleFiltered → they drop into the same "Show hidden"
// bucket as suspectOracle/gauge-killed pools (hidden by default, still findable
// for withdraw — deprecated_chain_withdraw_only). Complements our own on-chain
// stored_rates() heuristic; this is Curve's authoritative curated set.
let _oracleFilterInflight = null;
const _ORACLE_FILTER_LS = 'curvedex.oracleFilters';
const _ORACLE_FILTER_TTL = 6 * 3600 * 1000;
async function loadOraclePoolFilters() {
  if (_oracleFilterInflight) return _oracleFilterInflight;
  _oracleFilterInflight = (async () => {
    const chain = (typeof getChainKey === 'function') ? getChainKey() : 'ethereum';
    let byChain = null;
    try {
      const raw = localStorage.getItem(_ORACLE_FILTER_LS);
      if (raw) {
        const c = JSON.parse(raw);
        if (c && c.ts && (Date.now() - c.ts) < _ORACLE_FILTER_TTL && c.data) byChain = c.data;
      }
    } catch (e) { /* fall through to fetch */ }
    if (!byChain) {
      try {
        const resp = await fetch(PRICES_BASE + '/chains/pool_filters');
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        const rows = (await resp.json()).data || [];
        byChain = {};
        for (const r of rows) byChain[r.chain] = (r.pools || []).map(p => (p.address || '').toLowerCase());
        try { localStorage.setItem(_ORACLE_FILTER_LS, JSON.stringify({ ts: Date.now(), data: byChain })); } catch (e) {}
      } catch (e) {
        console.warn('[oracleFilter] fetch failed:', e && e.message);
        return;
      }
    }
    const hide = new Set(byChain[chain] || []);
    if (!hide.size) return;
    let n = 0;
    for (const p of allPools) {
      if (hide.has((p.address || '').toLowerCase())) { p.oracleFiltered = true; n++; }
    }
    if (n) {
      console.log('[oracleFilter]', n, 'pools hidden by Curve pool_filters (', chain, ')');
      if (typeof renderPoolList === 'function') renderPoolList();
    }
  })().finally(() => { _oracleFilterInflight = null; });
  return _oracleFilterInflight;
}
...
function updateHeaderStats() {
  ...
  // Home tab re-renders on every data tick (no-op unless Home is visible).
  if (window.Home) window.Home.refresh();
}
      ...
      if (!cacheMode) {
        ...
        pools.forEach(p => {
          ...
          if (existing) {
            ...
            // /getPools carries poolBalance + usdPrice per coin on EVERY chain.
            // Cache mode gets them pre-merged; the direct-API path (every
            // non-Ethereum chain) used to drop them, so anything reading
            // coinsDetailed went silent there — Risk Signals on Home showed no
            // pools at all (Alexandr crvecodev/2314).
            if (_detailedCoins(p)) existing.coinsDetailed = _detailedCoins(p);
            ...
            const newPool = {
              ...
              coinsDetailed: _detailedCoins(p),
...
function getFilteredPools() {
  ...
  if (!window.showHidden) {
    ...
    list = list.filter(p => {
      ...
      if (p.oracleFiltered) return false; // Curve pool_filters — unverified oracle contract (crvecodev/1407)
  ...
  // High Volume — абсолютный суточный объём от порога.
  if (highVolumeOnly) {
    list = list.filter(p => {
      const vol = (typeof p.volumeUSD === 'number' && isFinite(p.volumeUSD)) ? p.volumeUSD : 0;
      return vol >= HIGH_VOLUME_MIN_USD;
    });
  }
  // High Util — объём за сутки ≥ TVL (утилизация ≥ 100%).
  if (highUtilOnly) {
    list = list.filter(p => {
      const vol = (typeof p.volumeUSD === 'number' && isFinite(p.volumeUSD)) ? p.volumeUSD : 0;
  ...
  // 🅲 Grade ≥ C — strict subset of Stablecoins. Pool passes if EVERY coin has
  // a grade of C- or better (weakest-token rule). Yearn letter used where no Pharos.
  if (window.minGradeCOnly) {
    const grades = window._ratingsGrades;
    if (grades && grades.size > 0) {
      const ok = new Set(['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-']);
      list = list.filter(p => {
        const addrs = p.coinsAddresses;
        if (!Array.isArray(addrs) || !addrs.length) return false;
        return addrs.every(a => {
  ...
  // ⏳ Incentives > 1w (Alexandr crvecodev/1328) — pools whose paid Votemarket
  // runway exceeds one more week. Data = incentive_runway.json via yield.js
  // loadRunway()/_runwayCache; the toggle handler kicks the load, so an empty
  // cache here just means "still loading" and the filter passes nothing yet.
  if (window.runwayOnly) {
    const rw = (typeof _runwayCache !== 'undefined' && _runwayCache && _runwayCache !== 'missing')
      ? _runwayCache.gauges : null;
    list = list.filter(p => {
      if (!rw || !p.gaugeAddress) return false;
      const r = rw[p.gaugeAddress.toLowerCase()];
      return !!(r && r.weeks > 1);
    });
  }
  ...
  if (searchQuery) {
    ...
    // Paste a contract, find the pool (Alexandr crvecodev/1898). Only a 0x…
    // query is read as an address: a plain substring over addresses would make
    // any two hex characters match half the list.
    const addrQuery = /^0x[0-9a-f]{2,}$/.test(q) ? q : null;
    ...
    list = list.filter(p => {
      if (addrQuery) {
        // The pool itself, its LP token, or any coin inside it.
        if ((p.address || '').toLowerCase().startsWith(addrQuery)) return true;
        if ((p.lpTokenAddress || '').toLowerCase().startsWith(addrQuery)) return true;
        const ca = p.coinsAddresses;
        return !!(ca && ca.some(a => (a || '').toLowerCase().startsWith(addrQuery)));
      }
...
// Bridged wrappers (USDC.e on Arbitrum и т.п.) are the SAME asset as the
// catalog's canonical token but sit at their own address, so an address-only
// lookup leaves their pools unrated — Alexandr crvecodev/2330 «приравняй их как
// один токен». Two tiers, strongest first (Chado crvecodev/2333):
//   1. ON-CHAIN: a token minted by the standard bridge stores its origin in
//      l1Address(); USDC.e 0xff97…5cc8 answers 0xa0b8…eb48, which is the very
//      USDC the catalog grades A-. Address to address, no strings involved.
//   2. NAMES, only where the call reverts or returns zero (Arbitrum USDT goes
//      through a custom gateway and answers zeros): the coin's NAME must equal
//      a catalog asset's name once the chain qualifier in brackets is dropped
//      ("USD Coin (Arb1)" -> "USD Coin") AND its SYMBOL must start with that
//      asset's symbol (USDC.e -> USDC). Weaker on purpose — anybody can write a
//      name — so it never overrides tier 1.
// Answers are immutable, so they are cached per chain in localStorage; only
// coins the catalog does not already know are ever asked about.
const L1_ADDRESS_SELECTOR = '0xc2eeeebd';   // l1Address()
const ZERO_ADDR = '0x0000000000000000000000000000000000000000';

// Dust floor for the sweep: the same $50K the Home showcases use. Without it
// the sweep asks about every coin of all 684 Arbitrum pools (330 addresses,
// ~40 s and a wall of 429s from the public node) to decorate rows nobody can
// see — see pace_requests_to_third_party_sites.
const BRIDGE_LINK_MIN_TVL = 50000;
let _bridgeLinkRun = null;

async function _linkBridgedTokenGrades() {
  if (_bridgeLinkRun) return _bridgeLinkRun;        // one sweep at a time
  _bridgeLinkRun = _linkBridgedTokenGradesInner().finally(() => { _bridgeLinkRun = null; });
  return _bridgeLinkRun;
}

async function _linkBridgedTokenGradesInner() {
  const grades = window._ratingsGrades, tokens = window._ratingsTokens;
  if (!grades || !grades.size) return 0;
  if (typeof allPools === 'undefined' || !Array.isArray(allPools) || !allPools.length) return 0;
  const chain = (typeof getChainKey === 'function') ? getChainKey() : 'ethereum';
  if (chain === 'ethereum') return 0;          // no wrappers to resolve on L1

  // Unknown coins of pools worth showing, with their metadata.
  const unknown = new Map();
  for (const p of allPools) {
    if ((p.tvl || 0) < BRIDGE_LINK_MIN_TVL) continue;
    for (const c of (Array.isArray(p.coinsDetailed) ? p.coinsDetailed : [])) {
      const addr = String((c && c.address) || '').toLowerCase();
      if (addr && !grades.has(addr) && !unknown.has(addr)) unknown.set(addr, c);
    }
  }
  if (!unknown.size) return 0;

  const LS_KEY = 'curvedex.l1link.' + chain;
  let cache = {};
  try { cache = JSON.parse(localStorage.getItem(LS_KEY) || '{}') || {}; } catch (_) {}

  // Ask in PARALLEL (rpcCall has its own in-flight gate). Sequentially this is
  // minutes on a rate-limited public node, and the answer is only needed to
  // decorate rows — nothing may wait on it.
  const toAsk = [...unknown.keys()].filter(a => !(a in cache));
  if (toAsk.length) {
    await Promise.allSettled(toAsk.map(async (addr) => {
      let origin = '';
      try {
        const raw = await rpcCall(L1_ADDRESS_SELECTOR, addr);
        if (typeof raw === 'string' && raw.length >= 66) {
          const cand = '0x' + raw.slice(-40);
          if (cand !== ZERO_ADDR) origin = cand;
        }
      } catch (_) { /* token has no such method, or the node refused — tier 2 */ }
      cache[addr] = origin;
    }));
    try { localStorage.setItem(LS_KEY, JSON.stringify(cache)); } catch (_) {}
  }

  const byName = window._ratingsByName;
  const norm = (x) => String(x || '').toLowerCase().replace(/\s*\([^)]*\)/g, ' ')
    .replace(/\s+/g, ' ').trim();
  let linked = 0;
  for (const [addr, coin] of unknown) {
    if (grades.has(addr)) continue;
    const origin = cache[addr];
    let grade = origin ? grades.get(origin) : undefined;
    if (!grade && byName && byName.size) {
      const cand = byName.get(norm(coin && coin.name));
      const sym = String((coin && coin.symbol) || '').toUpperCase();
      if (cand && cand.grade && sym && sym.startsWith(String(cand.symbol).toUpperCase())) {
        grade = cand.grade;
      }
    }
    if (!grade) continue;
    grades.set(addr, grade);
    if (tokens) tokens.add(addr);
    linked++;
  }
  if (linked && typeof renderPoolList === 'function') renderPoolList();
  if (linked && window.Home && typeof window.Home.refresh === 'function') window.Home.refresh();
  return linked;
}
...
// Legacy assets (Alexandr crvecodev/1229): tokens whose issuer moved to a new
// contract and closed the official conversion path. Their pools keep working
// (trade/withdraw stay available) but must not be promoted: Home showcases
// skip them, the yield header shows a warning banner, the sidebar row gets
// the ⚠ badge. Address (lowercase) -> short reason used in UI copy.
const LEGACY_ASSETS = {
  // Legacy Frax Dollar: the stablecoin was upgraded to frxUSD and the official
  // 1:1 conversion window is closed — legacy FRAX trades at a discount.
  '0x853d955acef822db058eb8505911ed77f175b99e':
    'FRAX here is the legacy Frax Dollar: it was upgraded to frxUSD and the official 1:1 conversion is closed',
};
// Reason string if the pool holds a legacy asset directly or via a base-pool
// LP coin (metapools carrying FRAXBP etc. — same lpMap as grade inheritance).
// Depegged dollars (Alexandr crvecodev/2268 — MIM). The set is built server-side
// by collector/depeg_snapshot.py: a coin the stablecoin catalog calls a DOLLAR
// (pegType peggedUSD) whose aggregated market price sits below the band. No list
// of symbols lives here — the catalog decides what was meant to be $1, the market
// decides whether it still is.
let _depegCoins = null, _depegBand = 0.05;
(async function _loadDepegSet() {
  try {
    const r = await fetch((window.__DYNAMIC_BASE || '') + '/curvedex/depeg.json?v=' + (window.__APP_VERSION__ || ''), { cache: 'no-cache' });
    if (!r.ok) return;
    const d = await r.json();
    if (!d || !d.coins) return;
    if (typeof d.band === 'number' && d.band > 0) _depegBand = d.band;
    const m = new Map();
    for (const [a, e] of Object.entries(d.coins)) m.set(String(a).toLowerCase(), e);
    _depegCoins = m;
    window._depegCoins = m;
    console.log('[depeg]', m.size, 'addresses below peg (band', _depegBand + ')');
    if (typeof renderPoolList === 'function') renderPoolList();
    if (typeof updateYieldHeader === 'function' && typeof selectedPool !== 'undefined' && selectedPool) updateYieldHeader();
  } catch (e) { console.warn('[depeg] load failed:', e && e.message); }
})();

// Depeg note for a pool, or null. Curve's own price for the coin must not
// CONTRADICT the market feed: one illiquid pool can misprice a coin (yUSD reads
// $0.64 inside a pool and $1.00 on the market). A missing price contradicts
// nothing — absence is not a measurement.
function _poolDepeg(p) {
  if (!_depegCoins || !_depegCoins.size || !p) return null;
  const cd = Array.isArray(p.coinsDetailed) && p.coinsDetailed.length
    ? p.coinsDetailed
    : (Array.isArray(p.coinsAddresses) ? p.coinsAddresses.map(a => ({ address: a })) : []);
  for (const c of cd) {
    const a = String(c.address || '').toLowerCase();
    const hit = a && _depegCoins.get(a);
    if (!hit) continue;
    const own = parseFloat(c.usdPrice);
    if (isFinite(own) && own > 0 && own >= 1 - _depegBand) continue;
    const price = (isFinite(own) && own > 0) ? Math.min(own, hit.price) : hit.price;
    return { symbol: hit.symbol || c.symbol || 'asset', price };
  }
  return null;
}

function _depegText(d) {
  const price = d.price < 0.1 ? d.price.toFixed(4) : d.price.toFixed(3);
  return d.symbol + ' trades at $' + price + ' (' + ((d.price - 1) * 100).toFixed(1) + '% from its $1 peg)';
}

function _poolLegacyAsset(p) {
  const addrs = Array.isArray(p && p.coinsAddresses) ? p.coinsAddresses : [];
  const lpMap = _baseLpCoinsMap();
  for (const a of addrs) {
    const k = String(a || '').toLowerCase();
    if (LEGACY_ASSETS[k]) return LEGACY_ASSETS[k];
    const under = lpMap.get(k);
    if (under) {
      for (const u of under) if (LEGACY_ASSETS[u]) return LEGACY_ASSETS[u];
    }
  }
  return null;
}
...
function renderPoolList() {
  ...
  container.innerHTML = show.map(p => {
    ...
    const coinIcons = (p.coinsAddresses || []).filter(a => a && a !== '0x0000000000000000000000000000000000000000').map(a =>
      `<img class="token-icon" src="${_tokenIconUrl(a)}" alt="" width="18" height="18" loading="lazy" onerror="this.style.visibility='hidden'">`
    ).join('');
    ...
    const legacyNote = _poolLegacyAsset(p);
    const depegNote = _poolDepeg(p);
    const deprTitle = depegNote
      ? `Depegged asset — ${_depegText(depegNote)}. Pool price, TVL and APY are distorted; do not deposit.`
      : (p.deprecated
      ...
      : (p.suspectOracle
        ...
        : (p.oracleFiltered
          ? 'Unverified oracle contract — this pool uses a price oracle that is not a verified contract on Etherscan. Curve hides it by default. Deposit only if you know exactly what this is.'
          : (p.gaugeIsKilled
            ? 'Gauge killed by Curve DAO — protocol deprecated, deposits disabled on curve.finance. Avoid this pool.'
            : `Legacy asset: ${legacyNote}. New deposits are not recommended.`))));
    const killedBadge = (p.gaugeIsKilled || p.deprecated || p.suspectOracle || p.oracleFiltered || legacyNote || depegNote)
...
async function selectPool(address) {
  ...
  selectedPool = pool;
  // Пара и направление графика — из памяти браузера по ЭТОМУ пулу; ничего не
  // запомнено — канонический порядок (Alexandr crvecodev/2370).
  if (typeof _restorePoolPairPref === 'function') _restorePoolPairPref(pool);
  else if (typeof poolPriceInverted !== 'undefined') poolPriceInverted = false;
  if (typeof _syncPoolPriceInvertBtn === 'function') _syncPoolPriceInvertBtn();
  // Курсы всех пар пула в шапке: свой ряд у каждого пула.
  if (typeof loadPoolPairPrices === 'function') loadPoolPairPrices(pool);
  // Вкладка «Virtual Price» открыта — ряд у нового пула свой, перерисовать.
  if (typeof poolChartMode !== 'undefined' && poolChartMode === 'vp' &&
      typeof loadPoolVpChart === 'function') loadPoolVpChart();
...
async function loadViewData() {
  ...
  if (currentView === 'pools') {
    ...
    initYieldChart();
    if (currentChartMode === 'bribes') await loadBribesChart();
    else await loadApyHistory();
...
async function connectWallet() {
  ...
  try {
    ...
    if (btn && !walletAddress) {
      btn.disabled = false;
      btn.innerHTML = '<span class="wallet-btn-icon" aria-hidden="true"><svg class="icon"><use href="#icon-wallet"/></svg></span><span class="wallet-btn-text">Connect<span class="wallet-btn-word"> Wallet</span></span>';
    }
  } finally {
...
if (window.ethereum) {
  window.ethereum.on('accountsChanged', accounts => {
    if (accounts.length === 0) {
      ...
      wbtn.innerHTML = '<span class="wallet-btn-icon" aria-hidden="true"><svg class="icon"><use href="#icon-wallet"/></svg></span><span class="wallet-btn-text">Connect<span class="wallet-btn-word"> Wallet</span></span>';
      ...
      if (window.Portfolio) {
        ...
        if (stripChainFromHash(location.hash) === '#/portfolio') window.Portfolio.open();
...
async function loadBaseApyVp() {
  ...
  if (applied > 0) {
    updateHeaderStats();   // Avg APY in the header is computed from dailyApy —
    if (typeof renderPoolList === "function") renderPoolList();
  }
}

// Trading volume for Curve Lite chains, indexed by our own collector
// (Lite exposes no getVolumes — api-core serves the pool list only). Reads the
// 24h aggregate our collector writes and fills volumeUSD so the volume column,
// the header stats and the activity widgets count Lite pools like any other.
// No-op on non-Lite chains. (Alexandr 2026-09-22)
async function loadLiteVolumes() {
  const ck = getChainKey();
  const cc = CHAINS_CONFIG && CHAINS_CONFIG.chains && CHAINS_CONFIG.chains[ck];
  if (!cc || !cc.lite) return;
  let feed;
  try {
    const base = (window.__DYNAMIC_BASE || "");
    const resp = await fetch(base + "/curvedex/volumes_" + ck + ".json?v=" + (window.__APP_VERSION__ || ""), { cache: "no-cache" });
    if (!resp.ok) return;
    feed = await resp.json();
  } catch (e) { console.warn("[lite-vol] fetch failed:", e && e.message); return; }
  const rows = feed && feed.data && Array.isArray(feed.data.pools) ? feed.data.pools : null;
  if (!rows) return;
  let applied = 0;
  for (const row of rows) {
    const pool = poolsByAddress.get(String(row.address || "").toLowerCase());
    if (!pool) continue;
    const vol = Number(row.volumeUSD);
    if (!isFinite(vol) || vol < 0) continue;
    pool.volumeUSD = vol;
    pool.txCount24h = Number(row.txCount) || 0;
    applied++;
  }
  if (applied > 0) {
    updateHeaderStats();
    if (typeof renderPoolList === "function") renderPoolList();
    // The open pool card was rendered BEFORE this feed arrived (deep link to a
    // pool renders at registry-load time), so its 24h Volume row still holds the
    // zero it was born with. Re-render it, not just the list.
    if (typeof selectedPool !== "undefined" && selectedPool &&
        typeof updateTradePoolInfo === "function") {
      try { updateTradePoolInfo(); } catch (e) {}
    }
    // ...и панель на Yield: у неё свой набор строк, и открытая по прямой ссылке
    // страница держала ноль, с которым родилась (Alexandr crvecodev/2399).
    if (typeof selectedPool !== "undefined" && selectedPool &&
        typeof updateYieldPoolInfo === "function") {
      try { updateYieldPoolInfo(); } catch (e) {}
    }
  }
}

// Campaign page on Merkl, e.g.
// app.merkl.xyz/opportunities/ethereum/ERC20LOGPROCESSOR/0x625E92...6200
// (the slug is the chain NAME lowercased - the numeric chainId 404s).
function merklOpportunityUrl(opp) {
  const chain = ((opp.chain && opp.chain.name) || '').toLowerCase();
  const type = opp.type || '';
  const id = opp.identifier || '';
  if (!chain || !type || !id) return '';
  return `https://app.merkl.xyz/opportunities/${chain}/${type}/${id}`;
}

// Shared by the /yield header renderer and the 60s KPI refresh so the tile
// never drifts between the two.
function renderMerklKpi(pool) {
  const wrap = document.getElementById('kpiMerklWrap');
  const el = document.getElementById('kpiMerklApy');
  if (!wrap || !el) return;
  if (!(pool && pool.merklApr > 0)) { wrap.style.display = 'none'; return; }
  const text = '+' + fmtPct(pool.merklApr);
  if (pool.merklUrl) {
    el.innerHTML = '<a href="' + pool.merklUrl + '" target="_blank" rel="noopener"' +
      ' title="Open Merkl campaign" style="color:inherit;text-decoration:none;' +
      'border-bottom:1px dotted currentColor">' + text + '</a>';
  } else {
    el.textContent = text;
  }
  wrap.style.display = '';
  ...
  try {
    ...
    for (const opp of data) {
      ...
      if (addr && isFinite(apr) && apr > 0) {
        ...
        if (!existing || apr > existing.apr) newMap.set(addr, { apr, url: merklOpportunityUrl(opp) });
      }
    }
...
function applyMerklToPools() {
  for (const pool of allPools) {
    ...
    const merkl = merklRewardsMap.get(poolAddr) || merklRewardsMap.get(gaugeAddr) || null;
    pool.merklApr = merkl ? merkl.apr : 0;
    pool.merklUrl = merkl ? merkl.url : '';
  }
  // Merkl lands async, after the header is drawn - repaint the tile so the
  // campaign link shows up without waiting for the 60s KPI refresh.
  if (selectedPool && typeof renderMerklKpi === 'function') {
    const cur = poolsByAddress.get(selectedPool.address.toLowerCase()) || selectedPool;
    renderMerklKpi(cur);
  }
}
...
// ============================================================
// Yield-source filter (Alexandr crvecodev/1869) — which staking platforms may win
// a pool's Total APY. Same three sources as the Home chips, here as checkboxes in
// list settings. At least one stays on: with none on, every Total would read base
// only and the whole ranking would be meaningless.
const yieldSourceOn = { native: true, convex: true, stakedao: true };
// ОДНО состояние на весь сайт: чипы Home и галочки в настройках списка —
// два вида одного переключателя, а не две независимые настройки
// (Alexandr crvecodev/2018: «на Home отключение Convex ни на что не влияет»).
window.yieldSourceOn = yieldSourceOn;
const YIELD_SRC_OF = { C: 'native', Cx: 'convex', Sd: 'stakedao' };

// Re-decides pool.bestSrc / bestTotalApy from the bonuses stored at enrich time.
function pickBestYieldSource(pool) {
  const base = _safe(pool._baseUniform);
  const by = pool._bonusBySrc || {};
  let bestBonus = 0, src = 'C', found = false;
  for (const key of ['C', 'Cx', 'Sd']) {
    const v = by[key];
    if (v == null || !yieldSourceOn[YIELD_SRC_OF[key]]) continue;
    if (!found || v > bestBonus) { bestBonus = v; src = key; found = true; }
  }
  pool.bestBonus = bestBonus;
  pool.bestTotalApy = base + bestBonus;
  pool.bestSrc = src;
}

// Called by the checkboxes: recompute every pool, then repaint what is on screen.
function applyYieldSources() {
  if (Array.isArray(allPools)) allPools.forEach(pickBestYieldSource);
  renderPoolList();
  if (selectedPool && currentView === 'yield') updateYieldHeader();
  if (window.Home && typeof window.Home.refresh === 'function') window.Home.refresh();
}
window.applyYieldSources = applyYieldSources;
...
// Native CRV bonus = BASE (unboosted, 1× veCRV) = gaugeCrvApy[0], NOT the
// base/max-boost average. Verdict Alexandr + Nik (crvecodev/1381-1385): ~99% of
// LPs have no veCRV self-boost, so the mid-boost average (1.75×) systematically
// overstated the number a typical depositor actually earns. The max-boost end
// ([1], 2.5×) is still surfaced as the upper bound of the CRV range in the KPI
// tile. Reverses the earlier uniform-base avg-CRV convention (msg 590).
function _gaugeCrvBase(pool) {
  return Array.isArray(pool.gaugeCrvApy) ? (Number(pool.gaugeCrvApy[0]) || 0) : 0;
}
...
function mergePlatformAprs() {
  ...
  for (const pool of allPools) {
    ...
    const curveBonusDirect = _gaugeCrvBase(pool) + curveExtra;
    ...
    // Keep the three bonuses; which one wins is decided by pickBestYieldSource so
    // the Yield-sources checkboxes can re-decide without re-fetching (crvecodev/1869).
    pool._baseUniform = baseUniform;
    pool._bonusBySrc = { C: curveBonus, Cx: cvx ? cvxBonus : null, Sd: sd ? sdBonus : null };
    pickBestYieldSource(pool);
  ...
  Promise.allSettled([fetchConvexYields(), fetchStakeDaoYields()])
    .then(() => {
      try {
        mergePlatformAprs();
        if (typeof renderPoolList === 'function') renderPoolList();
        if (window.Home) window.Home.refresh(); // Home APY = bestTotalApy, refresh once platform data lands
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  ...
  if (btn.id === 'myAssetsToggle' || btn.id === 'favoritesToggle' ||
      ...
      btn.id === 'highVolumeToggle' || btn.id === 'highUtilToggle' || btn.id === 'disbalanceToggle' ||
      btn.id === 'stablecoinsToggle' || btn.id === 'minGradeBToggle' || btn.id === 'minGradeCToggle' || btn.id === 'runwayToggle' ||
...
// «🔥 High Util» — прежний смысл High Vol: объём за сутки ≥ TVL.
const highUtilToggleEl = document.getElementById('highUtilToggle');
if (highUtilToggleEl) {
  highUtilToggleEl.addEventListener('click', () => {
    highUtilOnly = !highUtilOnly;
    highUtilToggleEl.classList.toggle('active', highUtilOnly);
    renderPoolList();
  });
}
...
(function initListSettings() {
  ...
  // Yield-source checkboxes — restore, then let each one re-decide the winner.
  [['native', 'srcToggleNative', 'cd_srcNative'],
   ['convex', 'srcToggleConvex', 'cd_srcConvex'],
   ['stakedao', 'srcToggleStakedao', 'cd_srcStakedao']].forEach(([key, id, store]) => {
    const chk = document.getElementById(id);
    if (!chk) return;
    let on = true;
    try { on = localStorage.getItem(store) !== '0'; } catch (e) {}
    yieldSourceOn[key] = on;
    chk.checked = on;
    chk.addEventListener('change', () => {
      // Refuse to switch off the last one — a ranking needs at least one source.
      if (!chk.checked && !Object.keys(yieldSourceOn).some(k => k !== key && yieldSourceOn[k])) {
        chk.checked = true;
        return;
      }
      yieldSourceOn[key] = chk.checked;
      try { localStorage.setItem(store, chk.checked ? '1' : '0'); } catch (e) {}
      applyYieldSources();
    });
  });
})();
...
// "🅲 Grade ≥ C" pill — strict subset: every coin's grade is C- or better.
const minGradeCToggleEl = document.getElementById('minGradeCToggle');
if (minGradeCToggleEl) {
  minGradeCToggleEl.addEventListener('click', () => {
    window.minGradeCOnly = !window.minGradeCOnly;
    minGradeCToggleEl.classList.toggle('active', window.minGradeCOnly);
    renderPoolList();
  });
}

// "⏳ Incentives > 1w" pill — paid Votemarket runway exceeds one more week.
// Runway JSON is lazy: first activation kicks loadRunway() (yield.js) and
// re-renders when it lands.
const runwayToggleEl = document.getElementById('runwayToggle');
if (runwayToggleEl) {
  runwayToggleEl.addEventListener('click', () => {
    window.runwayOnly = !window.runwayOnly;
    runwayToggleEl.classList.toggle('active', window.runwayOnly);
    if (window.runwayOnly && typeof loadRunway === 'function'
        && typeof _runwayCache !== 'undefined' && _runwayCache == null) {
      loadRunway().then(() => renderPoolList());
    }
    renderPoolList();
  });
}
...
// Chart tabs (yield)
// Только вкладки YIELD: класс .chart-tab переиспользован вкладками графика пула,
// и без этой привязки клик по ним уезжал в switchChartMode(undefined) и гасил
// подсветку, которую только что поставил свой обработчик (Alexandr crvecodev/2378).
document.querySelectorAll('#yieldChartTabs .chart-tab').forEach(btn => {
...
// Clear button inside the search field. Goes through the input event so the
// list, the query variable and the button itself all reset the one way.
function clearPoolSearch() {
  const inp = document.getElementById('poolSearch');
  if (!inp) return;
  inp.value = '';
  inp.dispatchEvent(new Event('input', { bubbles: true }));
  try { inp.focus(); } catch (e) {}
}
window.clearPoolSearch = clearPoolSearch;
...
function startRefresh() {
  setInterval(() => {
    ...
    if (selectedPool && currentView === 'pools') {
      // merge-only refresh: see refreshTradesHead in trade.js
      (window.refreshTradesHead || loadTrades)();
    }
  }, 10000);
...
/**
 * Суммы, на которые надо подписать approve, чтобы хватило ровно на операцию.
 *
 * Разрешение выдаём НА СУММУ ОПЕРАЦИИ, а не на весь баланс (Ник, 11.08):
 * бессрочный MaxUint256 оставляет контракту доступ ко всему остатку кошелька
 * навсегда. Цена — approve на каждую операцию.
 *
 * USDT и другие токены той же школы запрещают менять НЕнулевое разрешение
 * напрямую (внутри require allowance == 0), поэтому при 0 < have < need
 * сначала обнуляем, потом ставим нужное.
 *
 * @param {bigint|string|number} current текущий allowance
 * @param {bigint|string|number} needed  сколько нужно на операцию
 * @returns {bigint[]} [] — хватает; [need]; [0n, need] для USDT-подобных
 */
window.approveAmounts = function (current, needed) {
  const have = BigInt(current || 0);
  const need = BigInt(needed || 0);
  if (need <= 0n || have >= need) return [];
  return have > 0n ? [0n, need] : [need];
};
