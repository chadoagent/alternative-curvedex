...
async function fetchConvexYields() {
  ...
  try {
    ...
    for (const [poolId, a] of Object.entries(apys)) {
      ...
      const crv = Number(a.crvApy) || 0;
      const cvx = Number(a.cvxApy) || 0;
      const extras = Array.isArray(a.extraRewards) ? a.extraRewards : [];
      const extraSum = extras.reduce((s, r) => s + (Number(r.apy) || 0), 0);
...
function updateYieldHeader() {
  ...
  // Toggle deprecated banner visibility based on Curve API gauge.is_killed
  // (Алекс crvecodev/500). Mirrors yellow "PROTOCOL DEPRECATED" alert on
  // curve.finance/dex/.../deposit.
  const depBanner = document.getElementById('yieldDeprecatedBanner');
  if (depBanner) depBanner.style.display = pool && pool.gaugeIsKilled ? '' : 'none';
...
function initYieldChart() {
  ...
  if (currentChartMode === 'tvl') {
    ...
    apySeries = yieldChart.addLineSeries({
      color: '#0ecb81', lineWidth: 2, title: 'Daily APY',
      priceFormat: { type: 'custom', formatter: v => v.toFixed(4) + '%' },
    });
    weeklyApySeries = yieldChart.addLineSeries({
      color: '#f0b90b', lineWidth: 2, title: 'Weekly APY',
      priceFormat: { type: 'custom', formatter: v => v.toFixed(4) + '%' },
    });
    crvAprSeries = yieldChart.addLineSeries({
      color: '#e68a00', lineWidth: 2, title: 'CRV+Rewards APR', visible: false,
    ...
    tvlSeries = yieldChart.addLineSeries({
      color: '#f0b90b', lineWidth: 2, title: 'Virtual Price', visible: false,
      priceFormat: { type: 'custom', formatter: v => v.toFixed(4), minMove: 0.0001 },
      priceScaleId: 'right',
    });
  }
  ...
  try {
    ...
    if (currentChartMode === 'apy') {
      try {
        ...
        if (crvData.length > 0) {
          crvAprSeries.applyOptions({ visible: true, title: 'CRV+Rewards APR (DefiLlama)', lineStyle: LightweightCharts.LineStyle.Solid });
...
function updateYieldLegend(hasCrv, hasOther, llamaUsed = false, hasConvex = false, hasStakeDao = false) {
  ...
  if (!el) {
    ...
    el.style.cssText = 'position:absolute;top:8px;left:8px;z-index:10;display:flex;gap:12px;font-size:11px;font-family:var(--font-mono);pointer-events:none;flex-wrap:wrap;';
...
function updateUnstakeTabUI() {
  ...
  if (ga) ga.textContent = hasGauge ? shortAddr(gauge) : 'N/A';
...
async function handleUnstakeFromUnstakeTab() {
  ...
  try {
    await (await new ethers.Contract(gauge, GAUGE_ABI, signer).withdraw(ethers.parseUnits(amtVal, 18))).wait();
    ...
    if (e.code !== 'ACTION_REJECTED') alert('Unstake failed: ' + (e.shortMessage || e.message));
...
async function handleClaimExtrasFromYield() {
  ...
  try {
    ...
    await (await g.claim_rewards()).wait();
    await loadStakeData();
    updateClaimTabUI();
  } catch (e) {
...
function setDasMax(index) {
  if (!walletAddress || !selectedPool) return;
  const balEl = document.getElementById(`dasCoinBal_${index}`);
  if (balEl) {
    const t = balEl.textContent.replace('Bal: ', '');
    if (t !== '--') { document.getElementById(`dasAmount_${index}`).value = t; onDasAmountChange(); }
  }
}
...
function setDasPreset(index, pct) {
  ...
  if (balEl) {
    const t = balEl.textContent.replace('Bal: ', '');
    if (t !== '--') { document.getElementById(`dasAmount_${index}`).value = (parseFloat(t) * pct).toFixed(6); onDasAmountChange(); }
  }
}
...
async function getDasQuote() {
  ...
  try {
    const ifaceMap = {
      2: new ethers.Interface(['function calc_token_amount(uint256[2] amounts, bool deposit) view returns (uint256)']),
      3: new ethers.Interface(['function calc_token_amount(uint256[3] amounts, bool deposit) view returns (uint256)']),
      4: new ethers.Interface(['function calc_token_amount(uint256[4] amounts, bool deposit) view returns (uint256)']),
    };
    const iface = ifaceMap[n];
    if (!iface) return;
...
async function handleDepositAndStakeSubmit() {
  ...
  try {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (allowance < amounts[i]) { await (await token.approve(selectedPool.address, ethers.MaxUint256)).wait(); }
    }
    ...
    let minMint = 0n;
    if (calcIface) {
      const result = await rpcCall(calcIface.encodeFunctionData('calc_token_amount', [amounts, true]), selectedPool.address);
      minMint = BigInt(result) * BigInt(Math.round((1 - dasSlippage / 100) * 10000)) / 10000n;
    }
    const addIface = new ethers.Interface([`function add_liquidity(uint256[${n}] amounts, uint256 min_mint_amount) payable returns (uint256)`]);
    ...
    if (allowance < lpBal) {
      ...
      await (await lpToken.approve(gauge, ethers.MaxUint256)).wait();
    }
    ...
    let staked = false;
    try {
      await (await gaugeContract['deposit(uint256,address,bool)'](lpBal, walletAddress, false)).wait();
      staked = true;
    } catch (_) {
      try {
        await (await gaugeContract['deposit(uint256,address)'](lpBal, walletAddress)).wait();
        staked = true;
      } catch (__) {
...
function setDepositMax(index) {
  if (!walletAddress || !selectedPool) return;
  const balEl = document.getElementById(`depCoinBal_${index}`);
  if (balEl) {
    const balText = balEl.textContent.replace('Bal: ', '');
    if (balText !== '--') { document.getElementById(`depAmount_${index}`).value = balText; onDepositAmountChange(); }
  }
}
...
function setDepositPreset(index, pct) {
  ...
  if (balEl) {
    const balText = balEl.textContent.replace('Bal: ', '');
    if (balText !== '--') { document.getElementById(`depAmount_${index}`).value = (parseFloat(balText) * pct).toFixed(6); onDepositAmountChange(); }
  }
}
...
async function handleDepositSubmit() {
  ...
  try {
    for (let i = 0; i < n; i++) {
      ...
      const token = new ethers.Contract(addr, ERC20_ABI, signer);
      const allowance = await token.allowance(walletAddress, selectedPool.address);
      if (allowance < amounts[i]) { const tx = await token.approve(selectedPool.address, ethers.MaxUint256); await tx.wait(); }
    }
    const ifaceMap = {
      2: new ethers.Interface(['function calc_token_amount(uint256[2] amounts, bool deposit) view returns (uint256)']),
      3: new ethers.Interface(['function calc_token_amount(uint256[3] amounts, bool deposit) view returns (uint256)']),
    ...
    const tx = await poolContract.add_liquidity(amounts, minMint, { value: ethValue });
    await tx.wait();
    ...
    if (shouldStake && selectedPool._gaugeAddress) {
      try {
        ...
        if (lpBal > 0n) {
          ...
          if (gaugeAllowance < lpBal) { const aTx = await lpToken.approve(selectedPool._gaugeAddress, ethers.MaxUint256); await aTx.wait(); }
          const gauge = new ethers.Contract(selectedPool._gaugeAddress, ['function deposit(uint256)'], signer);
          const sTx = await gauge.deposit(lpBal);
          await sTx.wait();
        }
  ...
  try {
    ...
    if (withdrawMode === 'single') {
      ...
      try {
        ...
        await (await contract.remove_liquidity_one_coin(lpAmount, coinIdx, minOut)).wait();
      } catch (e) {
      ...
      const iface = new ethers.Interface([`function remove_liquidity(uint256 _amount, uint256[${n}] min_amounts) returns (uint256[${n}])`]);
      const contract = new ethers.Contract(selectedPool.address, iface, signer);
      await (await contract.remove_liquidity(lpAmount, minAmounts)).wait();
    }
...
async function handleStake() {
  ...
  try {
    const lpToken = new ethers.Contract(lpAddr, ERC20_ABI, signer);
    const allowance = await lpToken.allowance(walletAddress, gauge);
    if (allowance < amount) { await (await lpToken.approve(gauge, ethers.MaxUint256)).wait(); }
    await (await new ethers.Contract(gauge, GAUGE_ABI, signer).deposit(amount)).wait();
...
async function handleUnstake() {
  ...
  try {
    ...
    console.error('Unstake error:', e);
    if (e.code !== 'ACTION_REJECTED') alert('Unstake failed: ' + e.message);
  }
}
