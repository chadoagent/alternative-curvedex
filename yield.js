...
async function fetchStakeDaoYields() {
  ...
  // Server snapshot first (collector/stakedao_snapshot.py → stakedao.json):
  // same-origin, instant, no r.jina.ai dependency. hub.stakedao.org has no CORS
  // and sits behind Cloudflare, so the live proxy path below occasionally misses
  // on page open — leaving pools whose only source is the proxy (GitHub fallback
  // covers ~78 legacy vaults) showing "—". The snapshot removes that fragility;
  // the proxy path stays as a live fallback when the snapshot is missing/stale.
  try {
    const base = (typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '';
    const ver = (typeof window !== 'undefined' && window.__APP_VERSION__) || '';
    const sr = await fetch(base + '/curvedex/stakedao.json?v=' + ver, { cache: 'no-cache' });
    if (sr.ok) {
      const snap = await sr.json();
      const rows = Array.isArray(snap && snap.vaults) ? snap.vaults : [];
      const fresh = snap && snap.updated_ts && (Date.now() / 1000 - snap.updated_ts) < 43200; // <12h
      if (fresh && rows.length) {
        const expectedChainId = (typeof window !== 'undefined' && window.getChainId ? window.getChainId() : 1);
        const sLp = new Map(), sG = new Map();
        for (const r of rows) {
          if (r.chainId !== expectedChainId) continue;
          const entry = { total: r.total, boost: r.boost || 1, vault: _isAddr(r.vault) ? r.vault : null,
            extras: r.extras || 0, tradingFees: r.tradingFees || 0, crv: r.crv || 0, bonus: r.bonus || 0 };
          if (_isAddr(r.lp)) sLp.set(r.lp.toLowerCase(), entry);
          if (_isAddr(r.gauge)) sG.set(r.gauge.toLowerCase(), entry);
        }
        if (sLp.size || sG.size) {
          _stakedaoCache = { ts: Date.now(), byLpAddr: sLp, byGaugeAddr: sG };
          return _stakedaoCache;
        }
      }
    }
  } catch (e) { /* snapshot missing/stale — fall through to live proxy path */ }
...
// The simulated deposit, read in one place so every header tile asks the same question.
function _simDeposit() {
  return (typeof simDepositAmount !== 'undefined' && simDepositAmount > 0) ? simDepositAmount : 0;
}
// CRV range tile. Honours the same simulated deposit TOTAL does: a new unboosted LP dilutes the
// gauge emission it joins; the raw range lives in the tooltip (Alexandr crvecodev/2021 — TOTAL
// read as smaller than the "unboosted" CRV standing next to it, because only TOTAL was diluted).
// Own function because gauge TVL arrives after the first header render — updateThirdPartyYields
// redraws this tile the same way it redraws TOTAL.
function _renderGaugeApyKpi(pool) {
  const el = document.getElementById('kpiGaugeApy');
  if (!el || !pool) return;
  const raw = Array.isArray(pool.gaugeCrvApy) ? pool.gaugeCrvApy : [0, 0];
  if (!((raw[0] + raw[1]) / 2 > 0)) { el.textContent = '--'; el.title = ''; return; }
  const dep = _simDeposit();
  const diluting = dep > 0 && pool.gaugeTvlUsd > 0;
  const shown = diluting
    ? [gaugeDilutedApy(raw[0], pool.gaugeTvlUsd, dep), gaugeDilutedApy(raw[1], pool.gaugeTvlUsd, dep)]
    : raw;
  el.textContent = `${fmtPct(shown[0]).replace('%', '')}-${fmtPct(shown[1])}`;
  el.title = diluting
    ? `CRV emission APR, 1x - 2.5x boost. Diluted by the $${dep.toLocaleString('en-US')} simulated deposit into a $${Math.round(pool.gaugeTvlUsd).toLocaleString('en-US')} gauge \u2014 raw ${fmtPct(raw[0]).replace('%', '')}-${fmtPct(raw[1])}.`
    : 'CRV emission APR, 1x - 2.5x boost.';
}
async function updateThirdPartyYields() {
  ...
  try {
    ...
    // Platform totals ride the same rule as TOTAL: shown at the simulated deposit, undiluted
    // value in the tooltip (Alexandr crvecodev/2021 — only TOTAL was honouring the simulation).
    const _dep = _simDeposit();
    const _undil = (v) => (_dep > 0 && v != null)
      ? ` | ${fmtPct(v)} before the $${_dep.toLocaleString('en-US')} simulated deposit`
      : '';
    const cvxShown = (_dep > 0 && cvxUniform != null) ? dilutedPlatformApy(pool, cvxUniform, _dep) : cvxUniform;
    const sdShown = (_dep > 0 && sdUniform != null) ? dilutedPlatformApy(pool, sdUniform, _dep) : sdUniform;
    cvxEl.textContent = cvxShown != null ? fmtPct(cvxShown) : '—';
    cvxEl.title = cvx
      ? `Base ${fmtPct(baseUniform)} (Curve 7d) + CRV ${fmtPct(cvx.crv)} (boost ${cvx.boost.toFixed(2)}x) + CVX ${fmtPct(cvx.cvx)}${cvx.extra ? ' + Extra ' + fmtPct(cvx.extra) : ''} | Convex raw ${fmtPct(cvx.total)}${_undil(cvxUniform)}`
      : 'Pool not listed on Convex';
    sdEl.textContent = sdShown != null ? fmtPct(sdShown) : '—';
    sdEl.title = sd
      ? `Base ${fmtPct(baseUniform)} (Curve 7d) + CRV ${fmtPct(sd.crv || 0)} (boost ${sd.boost.toFixed(2)}x)${sd.extras ? ' + Extras ' + fmtPct(sd.extras) : ''} | StakeDAO raw ${fmtPct(sd.total)}${_undil(sdUniform)}`
      : 'Pool not listed on StakeDAO';
    // Gauge TVL is populated by the gauge enrichment, which can land after the first header
    // render — redraw the CRV tile here, where TOTAL is already refreshed post-fetch.
    _renderGaugeApyKpi(pool);
...
// ============================================================
let _llamaPoolMap = null;   // { ts, chain, maps: { curve, convex, stakedao } }
const LLAMA_CACHE_TTL = 10 * 60 * 1000; // 10 min
// getChainKey() → DefiLlama chain label. Unknown chain → no maps (as before).
const _LLAMA_CHAIN_LABELS = {
  ethereum: 'Ethereum', arbitrum: 'Arbitrum', optimism: 'OP Mainnet',
  base: 'Base', polygon: 'Polygon', fraxtal: 'Fraxtal', xdai: 'Gnosis',
  gnosis: 'Gnosis', avalanche: 'Avalanche', fantom: 'Fantom', kava: 'Kava',
  sonic: 'Sonic', monad: 'Monad', plasma: 'Plasma',
};
...
async function getLlamaPoolMaps() {
  // Chart lines were Ethereum-only (crvecodev/1358) — map the active chain.
  const chainLabel = _LLAMA_CHAIN_LABELS[getChainKey()] || null;
  if (!chainLabel) return null;
  if (_llamaPoolMap && _llamaPoolMap.chain === chainLabel && Date.now() - _llamaPoolMap.ts < LLAMA_CACHE_TTL) return _llamaPoolMap.maps;
  try {
    const resp = await fetch('https://yields.llama.fi/pools');
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const json = await resp.json();
    const all = (json.data || []).filter(p => p.chain === chainLabel);
    const maps = {
      curve: new Map(),
      convex: new Map(),
      stakedao: new Map(),
    ...
    _llamaPoolMap = { ts: Date.now(), chain: chainLabel, maps };
    return maps;
  } catch (e) {
...
// First-party Convex/StakeDAO APR history (collector/platform_apr_history.py,
// crvecodev/1358): DefiLlama misses ~45 Convex / ~73 StakeDAO vaults deployed
// on our TVL≥50k pools, so the chart falls back to our own daily snapshots.
// Convex keyed by curve pool address, StakeDAO by gauge address.
let _ownAprHistory = null; // { ts, data } | data === 'missing'
async function getPlatformAprHistory() {
  if (_ownAprHistory && Date.now() - _ownAprHistory.ts < LLAMA_CACHE_TTL) {
    return _ownAprHistory.data === 'missing' ? null : _ownAprHistory.data;
  }
  let data = 'missing';
  try {
    const base = (typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '';
    const ver = (typeof window !== 'undefined' && window.__APP_VERSION__) || '';
    const r = await fetch(base + '/curvedex/platform_apr_history.json?v=' + ver, { cache: 'no-cache' });
    if (r.ok) {
      const j = await r.json();
      if (j && Array.isArray(j.dates) && j.dates.length) data = j;
    }
  } catch (e) { /* fallback stays unavailable */ }
  _ownAprHistory = { ts: Date.now(), data };
  return data === 'missing' ? null : data;
}

// dates[] + aligned row[] → lightweight-charts series (nulls skipped).
// Merkl is sampled every 4h (no history API), but every OTHER platform line on
// this chart comes from DefiLlama at one point a day — measured 01.08, spacing
// 23.997h. Two resolutions side by side read as a discrepancy, so the samples
// are averaged into one point per UTC day (Alexandr crvecodev/1730). The mean
// also beats a single morning reading: Merkl APR drifts with TVL all day.
function _merklDailyFromSamples(stamps, row) {
  if (!Array.isArray(row) || !Array.isArray(stamps)) return [];
  const buckets = new Map();
  for (let i = 0; i < stamps.length; i++) {
    const v = row[i];
    const ts = Number(stamps[i]);
    if (v == null || !isFinite(v) || !isFinite(ts)) continue;
  ...
  return [...buckets.entries()]
    .sort((a, b) => a[0] - b[0])
    .map(([day, b]) => ({ time: day, value: b.sum / b.n }));
}

function _ownRowToSeries(dates, row) {
  const out = [];
  if (!Array.isArray(row)) return out;
  for (let i = 0; i < dates.length; i++) {
    const v = row[i];
    if (v == null || !isFinite(v)) continue;
    const ts = Math.floor(Date.parse(dates[i] + 'T00:00:00Z') / 1000);
    if (isFinite(ts)) out.push({ time: ts, value: v });
  }
  return out;
}
...
function updateYieldHeader() {
  ...
  if (depBanner) {
    const depeg = (typeof _poolDepeg === 'function') ? _poolDepeg(pool) : null;
    if (depeg) {
      depBanner.innerHTML = '<strong>\u26a0\ufe0f Depegged asset</strong> \u2014 ' + _depegText(depeg)
        + '. Price, TVL and APY shown for this pool are distorted while the peg is broken: the pool fills up with the cheap side. Exits keep working; new deposits are not recommended.';
      depBanner.style.display = '';
    } else if (pool && pool.deprecated) {
      ...
      depBanner.style.display = '';
    } else if (pool && pool.oracleFiltered) {
      depBanner.innerHTML = '<strong>⚠️ Unverified oracle</strong> — This pool uses a price oracle whose contract is not verified on Etherscan. Curve hides such pools by default (official pool_filters list). An unverified oracle can be changed by its deployer to misprice deposits — do not deposit unless you know exactly what this pool is.';
      depBanner.style.display = '';
      ...
      depBanner.style.display = '';
    } else if (pool && typeof _poolLegacyAsset === 'function' && _poolLegacyAsset(pool)) {
      // Legacy asset in pool composition (Alexandr crvecodev/1229) — pool keeps
      // working for exits, but parking new money in a sunset asset is a trap.
      depBanner.innerHTML = '<strong>⚠️ Legacy asset</strong> — ' + _poolLegacyAsset(pool) + '. Trading and withdrawals keep working, but new deposits are not recommended.';
      depBanner.style.display = '';
    } else {
  ...
  // Merkl KPI (tile links to the campaign - renderer shared with app.js)
  renderMerklKpi(pool);
...
// Incentive runway (tester crvecodev/1300 A): «стимул ещё ~N нед, до DD.MM» рядом с
// gauge-weight (эмиссионный KPI). Источник — incentive_runway.json (collector: Votemarket
// campaign endTimestamp per gauge). Оценка по добавленным взяткам, не гарантия.
let _runwayCache = null; // {gauges:{...}} | 'missing'
async function loadRunway() {
  if (_runwayCache != null) return _runwayCache;
  try {
    const base = (typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '';
    const ver = (typeof window !== 'undefined' && window.__APP_VERSION__) || '';
    const r = await fetch(base + '/curvedex/incentive_runway.json?v=' + ver, { cache: 'no-cache' });
    _runwayCache = r.ok ? await r.json() : 'missing';
  } catch (e) { _runwayCache = 'missing'; }
  return _runwayCache;
}
// Own KPI tile, same label/value/meta format as the rest of the row (Alexandr
// crvecodev/1320 — the long inline sentence broke the compact card format).
function _updateRunwayKpi(pool) {
  const wrap = document.getElementById('kpiRunwayWrap');
  const valEl = document.getElementById('kpiRunway');
  const metaEl = document.getElementById('kpiRunwayMeta');
  if (!wrap || !valEl || !metaEl) return;
  const gauge = pool && pool.gaugeAddress;
  const r = (gauge && _runwayCache && _runwayCache !== 'missing' && _runwayCache.gauges)
    ? _runwayCache.gauges[gauge.toLowerCase()] : null;
  if (!r || !r.weeks) { wrap.style.display = 'none'; return; }
  const d = new Date(r.end_ts * 1000);
  const dd = String(d.getUTCDate()).padStart(2, '0') + '.' + String(d.getUTCMonth() + 1).padStart(2, '0');
  wrap.style.display = '';
  valEl.textContent = `~${r.weeks}w`;
  metaEl.textContent = `until ${dd}`;
  wrap.title = `Estimate from added bribes (Votemarket): incentive lasts ~${r.weeks} more week(s), until ${dd}. Emission may persist ~1 week longer via vote inertia. Not guaranteed — if bribes are extended, the date shifts.`;
}
...
function _updateGaugeWeightKpi(pool) {
  ...
  if (!pool || !pool.gaugeAddress || pool.gaugeAddress === '0x0000000000000000000000000000000000000000') {
    ...
    _updateRunwayKpi(pool); // hides the Incentive tile for gaugeless pools
    return;
  }
  ...
  // Incentive tile: fill from cache now, or once the runway JSON lands.
  _updateRunwayKpi(pool);
  if (_runwayCache == null) {
    loadRunway().then(() => {
      if (selectedPool && selectedPool.gaugeAddress === pool.gaugeAddress) _updateRunwayKpi(pool);
    });
  }
...
// Per-coin contract addresses. Shared by the Yield modal and the Pools page
// sidebar so both stay identical (Alexandr 05.08).
function renderCoinContracts(pool, wrapId, listId) {
  const wrap = document.getElementById(wrapId);
  const list = document.getElementById(listId);
  if (!wrap || !list || !pool) return;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const iconHtml = `<img class="token-icon" src="${_tokenIconUrl(a)}" alt="" width="14" height="14" loading="lazy" onerror="this.style.visibility='hidden'">`;
...
window.renderCoinContracts = renderCoinContracts;
...
function updateYieldPoolInfo() {
  ...
  // of the row per tester @Alexandr_Petryashev msg 245). Wording mirrors Curve's
  // own UI — see poolTypeLabel in app.js.
  renderPoolTypeBadge('yieldInfoType', pool, true);
  ...
  // На Lite-сетях та же оговорка, что на странице пула: ставки комиссии там нет
  // ни у кого, произведение «объём × плоская ставка» не печатаем вовсе
  // (Alexandr crvecodev/2360). Сам объём на Lite приходит из нашего индекса —
  // его вливает loadLiteVolumes в app.js, единственный автор этого поля.
  const _ccY = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains;
  const _liteY = !!(_ccY && _ccY[getChainKey()] && _ccY[getChainKey()].lite);
  const _yFeesEl = document.getElementById('yieldInfoFees');
  _yFeesEl.textContent = (!_liteY && pool.volumeUSD > 0) ? '~' + fmt$(estFees) : '--';
  if (_liteY) _yFeesEl.dataset.blankByDesign = 'lite-no-fee-rate';
  else delete _yFeesEl.dataset.blankByDesign;
  ...
  renderCoinContracts(pool, 'yieldInfoCoins', 'yieldInfoCoinsList');
...
function initYieldChart() {
  ...
  yieldChart = LightweightCharts.createChart(container, {
    ...
    rightPriceScale: {
      ...
      // Bribes mode "$12.3K" needs the wide gutter too.
      minimumWidth: (currentChartMode === 'tvl' || currentChartMode === 'bribes') ? (_isMobile ? 70 : 90) : (_isMobile ? 48 : 70),
      autoScale: true,
  ...
  // Кнопка копирования графика берёт его отсюда (chart_share.js).
  if (window.registerChart) registerChart('yield-chart-container', yieldChart);
  ...
  if (currentChartMode === 'bribes') {
    // Bribes mode: single weekly-$ histogram, bar color encodes the source
    // (Votemarket / Votium / both). No APY/VP series at all — loadBribesChart
    // is the only data path in this mode.
    bribesSeries = yieldChart.addHistogramSeries({
      priceFormat: { type: 'custom', formatter: _fmtUsdShort },
      priceScaleId: 'right',
    });
    ...
    merklAprSeries = yieldChart.addLineSeries({
      color: '#b07aff', lineWidth: 2, title: '', visible: false,
      // Dashed so it never reads as the StakeDAO line - both are violet.
      lineStyle: LightweightCharts.LineStyle.Dashed,
      priceFormat: { type: 'custom', formatter: v => v.toFixed(1) + '%' },
      priceLineVisible: false,
      crosshairMarkerVisible: true, lastValueVisible: false,
...
async function loadApyHistory() {
  ...
  // Lite-сети ручка снапшотов Curve не обслуживает — там этот график был пуст
  // целиком, хотя на странице пула та же вкладка Virtual Price рисуется
  // (Alexandr crvecodev/2399). Ряд цены пая берём из ТОГО ЖЕ источника, что и
  // Pools: _fetchPoolVpSeries (trade.js) сама уходит в наш суточный снимок
  // vp_history.json на lite-сетях. APY-ряда у нас для них нет.
  const _liteCfg = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG
    && CHAINS_CONFIG.chains && CHAINS_CONFIG.chains[getChainKey()];
  if (_liteCfg && _liteCfg.lite && typeof _fetchPoolVpSeries === 'function') {
    const liteVp = await _fetchPoolVpSeries(selectedPool);
    apySeries.setData([]); weeklyApySeries.setData([]);
    crvAprSeries.setData([]); otherAprSeries.setData([]);
    tvlSeries.setData(liteVp);
    updateYieldLegend(false, false);
    if (currentChartMode === 'tvl') {
      showChartNoData(liteVp.length ? null : 'No virtual price history for this pool');
    } else {
  ...
  try {
    ...
    merklAprSeries.applyOptions({ visible: false });
    merklAprSeries.setData([]);
    ...
    if (currentChartMode === 'apy') {
      try {
        ...
        // DefiLlama miss → first-party daily snapshots (crvecodev/1358).
        // Row presence in the collector file = vault actually deployed (it is
        // built from the platforms' own APIs), so an absent platform still
        // draws nothing. NB: don't gate on pool.cvxApr/sdApr — those land
        // asynchronously via mergePlatformAprs and are usually null when the
        // chart first renders.
        // Merkl has no history endpoint at all (its aprRecord is one current
        // point), so its line is always first-party (Alexandr crvecodev/1686).
        let cvxFinal = cvxData, sdFinal = sdData, merklFinal = [];
        {
          const own = await getPlatformAprHistory();
          if (own) {
            const mRow = own.merkl?.[pool.address?.toLowerCase()] ||
                         own.merkl?.[(pool.gaugeAddress || '').toLowerCase()];
            merklFinal = _merklDailyFromSamples(own.merkl_ts, mRow);
            if (cvxData.length === 0) {
              const row = own.convex?.[pool.address?.toLowerCase()] ||
                          own.convex?.[(pool.lpTokenAddress || '').toLowerCase()];
              cvxFinal = _ownRowToSeries(own.dates, row);
            }
            if (sdData.length === 0) {
              const row = own.stakedao?.[(pool.gaugeAddress || '').toLowerCase()];
              sdFinal = _ownRowToSeries(own.dates, row);
            }
          }
        }
        if (cvxFinal.length > 0) {
          convexAprSeries.applyOptions({ visible: true });
          convexAprSeries.setData(cvxFinal);
          hasConvex = true;
        }
        if (sdFinal.length > 0) {
          stakedaoAprSeries.applyOptions({ visible: true });
          stakedaoAprSeries.setData(sdFinal);
          hasStakeDao = true;
        }
        if (merklFinal.length > 0) {
          merklAprSeries.applyOptions({ visible: true });
          merklAprSeries.setData(merklFinal);
          hasMerkl = true;
        }
      } catch (e) {
    ...
    updateYieldLegend(hasCrv, false, llamaUsed, hasConvex, hasStakeDao, hasMerkl);
...
function switchChartMode(mode) {
  ...
  document.querySelectorAll('#yieldChartTabs .chart-tab').forEach(t => t.classList.toggle('active', t.dataset.chart === mode));
  ...
  initYieldChart();
  if (mode === 'bribes') loadBribesChart();
  else loadApyHistory();
  const legend = document.getElementById('yieldChartLegend');
  if (legend) legend.style.display = (mode === 'apy' || mode === 'bribes') ? '' : 'none';
  showChartNoData(null);
}

// ============================================================
// YIELD: Bribes chart tab (Alexandr crvecodev/1300 C, /1306 в $, /1328 go)
// Weekly bribe history for the selected pool's gauge from bribes_gauge.json
// (collector/bribes_gauge_snapshot.py: Votium rounds via Llama Airforce $,
// Votemarket campaigns on-chain priced via Curve pool symbol map).
// One histogram, bar color encodes the source; weeks with both sources are
// summed and colored separately.
// ============================================================
const _BRIBE_COLORS = { votemarket: '#22d3ee', votium: '#a855f7', both: '#0ecb81' };
let _bribesGaugeCache = null; // parsed bribes_gauge.json | 'missing'
...
async function loadBribesGauge() {
  if (_bribesGaugeCache != null) return _bribesGaugeCache;
  try {
    const base = (typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '';
    const ver = (typeof window !== 'undefined' && window.__APP_VERSION__) || '';
    const r = await fetch(base + '/curvedex/bribes_gauge.json?v=' + ver, { cache: 'no-cache' });
    _bribesGaugeCache = r.ok ? await r.json() : 'missing';
  } catch (e) { _bribesGaugeCache = 'missing'; }
  return _bribesGaugeCache;
}
async function loadBribesChart() {
  if (!selectedPool || !bribesSeries) return;
  showChartNoData(null);
  const legend = document.getElementById('yieldChartLegend');
  if (legend) {
    legend.style.display = '';
    legend.innerHTML =
      `<span style="color:${_BRIBE_COLORS.votemarket};">■ Votemarket</span>` +
      `<span style="color:${_BRIBE_COLORS.votium};">■ Votium</span>` +
      `<span style="color:${_BRIBE_COLORS.both};">■ Both</span>` +
      `<span style="color:#848e9c;">weekly bribes added, $</span>`;
  }
  const data = await loadBribesGauge();
  if (currentChartMode !== 'bribes' || !bribesSeries) return; // user switched away mid-fetch
  const gauge = (selectedPool.gaugeAddress || '').toLowerCase();
  const rows = (data && data !== 'missing' && data.gauges && data.gauges[gauge]) || [];
  if (!rows.length) {
    bribesSeries.setData([]);
    showChartNoData('No bribe history for this gauge (Votium + Votemarket)');
    return;
  }
  const WEEK = 604800;
  const byWeek = new Map(); // weekTs -> {usd, srcs:Set}
  for (const r of rows) {
    const wk = Math.floor(r.ts / WEEK) * WEEK;
    const rec = byWeek.get(wk) || { usd: 0, srcs: new Set() };
    rec.usd += r.usd;
    rec.srcs.add(r.src);
    byWeek.set(wk, rec);
  }
  const bars = [...byWeek.entries()].sort((a, b) => a[0] - b[0]).map(([wk, rec]) => ({
    time: wk,
    value: Math.round(rec.usd),
    color: rec.srcs.size > 1 ? _BRIBE_COLORS.both
      : (rec.srcs.has('votium') ? _BRIBE_COLORS.votium : _BRIBE_COLORS.votemarket),
  }));
  bribesSeries.setData(bars);
  yieldChart.timeScale().fitContent();
}
...
// YIELD: Chart Legend
// ============================================================
function updateYieldLegend(hasCrv, hasOther, llamaUsed = false, hasConvex = false, hasStakeDao = false, hasMerkl = false) {
  ...
  if (hasMerkl) {
    html += `<span style="color:#b07aff;">${legDash} Merkl APR</span>`;
  }
  el.innerHTML = html;
}
...
const _SUB_TAB_PANEL_IDS = ['tab-deposit', 'tab-zap', 'tab-stake', 'tab-deposit-and-stake', 'tab-withdraw', 'tab-unstake', 'tab-claim-rewards', 'tab-info'];
...
function showActiveYieldFormPanel() {
  ...
  else if (_activeSubTab === 'zap') {
    if (typeof resetZapUI === 'function') resetZapUI();
  }
    ...
    <div class="coin-input-group">
      <div class="coin-input-box">
        <div class="coin-input-header">
          <span class="coin-input-label"><img class="token-icon" src="${_tokenIconUrl(selectedPool.coinsAddresses[i])}" alt="" width="20" height="20" loading="lazy" onerror="this.style.visibility='hidden'"> ${coin}</span>
...
// СУММА ИЗ ПОЛЯ — СТРОКОЙ, БЕЗ float. `parseFloat(x).toFixed(8)` ОКРУГЛЯЕТ:
// на «весь баланс» это даёт сумму БОЛЬШЕ остатка (транзакция падает уже в
// кошельке), а у 18-знакового токена молча срезает хвост. Обрезаем лишние
// знаки, не округляя (канон Михаила: суммы только в децималах, не во float).
function _parseAmountStrict(str, decimals) {
  const s = String(str == null ? '' : str).trim().replace(',', '.');
  if (!s || !/^[0-9]*\.?[0-9]*$/.test(s)) return 0n;
  const dot = s.indexOf('.');
  const whole = dot < 0 ? s : s.slice(0, dot);
  const dec = Number(decimals) || 18;
  const frac = dot < 0 ? '' : s.slice(dot + 1, dot + 1 + dec);
  try { return ethers.parseUnits((whole || '0') + (frac ? '.' + frac : ''), dec); } catch (e) { return 0n; }
}
...
async function getDasQuote() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const decimals = parseInt(selectedPool.decimals[i]) || 18;
    const wei = _parseAmountStrict(el ? el.value : '', decimals);
    if (wei > 0n) hasAny = true;
    amounts.push(wei);
  }
...
async function handleDepositAndStakeSubmit() {
  ...
  try {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Разрешение ровно на сумму вклада; при 0 < allowance < need сперва обнуление (USDT).
      for (const amt of window.approveAmounts(allowance, amounts[i])) {
        await (await window.sendContractTxWithDynamicGas(token, 'approve', [selectedPool.address, amt])).wait();
      }
    }
    ...
    if (allowance < lpBal) {
      ...
      for (const amt of window.approveAmounts(allowance, lpBal)) {
        await (await window.sendContractTxWithDynamicGas(lpToken, 'approve', [gauge, amt])).wait();
      }
    }
...
async function handleDepositSubmit() {
  ...
  try {
    for (let i = 0; i < n; i++) {
      ...
      const allowance = await token.allowance(walletAddress, selectedPool.address);
      for (const amt of window.approveAmounts(allowance, amounts[i])) {
        const tx = await window.sendContractTxWithDynamicGas(token, 'approve', [selectedPool.address, amt]); await tx.wait();
      }
    }
    const liqAbi = window.poolLiquidityAbi(selectedPool, n);
    let minMint = 0n;
    ...
    if (shouldStake && selectedPool._gaugeAddress) {
      try {
        ...
        if (lpBal > 0n) {
          ...
          for (const amt of window.approveAmounts(gaugeAllowance, lpBal)) {
            const aTx = await window.sendContractTxWithDynamicGas(lpToken, 'approve', [selectedPool._gaugeAddress, amt]); await aTx.wait();
          }
...
function setWithdrawMode(mode) {
  ...
  const zapWrap = document.getElementById('withdrawZapWrap');
  if (zapWrap) zapWrap.style.display = mode === 'any' ? '' : 'none';
  const gaugeRow = document.getElementById('zapOutFromGaugeRow');
  if (gaugeRow) gaugeRow.style.display = (mode === 'any' && _zapGaugeAddress()) ? 'flex' : 'none';
  if (mode !== 'any') {
    _zapHide('zapOutPreview');
    _zapOutResetRows();
    const cb = document.getElementById('zapOutFromGauge');
    if (cb) cb.checked = false;
  }
  _zapRefreshWithdrawBalanceLabel();
  onWithdrawAmountChange();
}

function setMaxWithdrawLP() {
  setWithdrawPreset(1.0);
}

function setWithdrawPreset(pct) {
  const basis = (withdrawMode === 'any') ? _zapOutSourceBalance() : lpBalanceRaw;
  if (basis === 0n || typeof ethers === 'undefined') return;
  document.getElementById('withdrawLPAmount').value = ethers.formatUnits((basis * BigInt(Math.round(pct * 10000))) / 10000n, 18);
  onWithdrawAmountChange();
}
...
async function getWithdrawQuote() {
  ...
  if (withdrawMode === 'any') {
    // The amount lives in the Receive box; the native Expected Output row is
    // written by the single-coin path and would linger with its number.
    document.getElementById('withdrawPreview').style.display = 'none';
    await fetchZapOutQuote(lpVal);
    return;
  }
...
function updateWithdrawButton() {
  ...
  if (typeof ethers !== 'undefined') {
    try {
      const lpAmount = ethers.parseUnits(lpVal, 18);
      if (withdrawMode === 'any' && _zapOutFromGauge()) {
        if (lpAmount > stakedLPRaw) {
          btn.textContent = 'More than you have staked';
          btn.className = 'action-btn disabled';
          return;
        }
      } else if (lpAmount > lpBalanceRaw) {
  ...
  if (withdrawMode === 'any') {
    if (!_zapOutToken) { btn.textContent = 'Select a token'; btn.className = 'action-btn disabled'; return; }
    if (!_zapOutQuote) { btn.textContent = 'Quoting…'; btn.className = 'action-btn disabled'; return; }
    btn.textContent = `Withdraw to ${_zapOutToken.symbol || 'token'}`;
...
async function handleWithdrawSubmit() {
  ...
  if (withdrawMode === 'any') return handleZapOutSubmit();
  const lpAmount = ethers.parseUnits(lpVal, 18);
...
async function handleStake() {
  ...
  try {
    const lpToken = new ethers.Contract(lpAddr, ERC20_ABI, signer);
    const allowance = await lpToken.allowance(walletAddress, gauge);
    for (const amt of window.approveAmounts(allowance, amount)) {
      await (await window.sendContractTxWithDynamicGas(lpToken, 'approve', [gauge, amt])).wait();
    }
...
async function updateDepositGas() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const wei = _parseAmountStrict(inp ? inp.value : '', dec);
    if (wei > 0n) hasAny = true;
    amounts.push(wei);
  }
    ...
    if (walletAddress) {
      for (let i = 0; i < n; i++) {
        ...
        steps.push({
          ...
          data: window._buildApproveCalldata(selectedPool.address, amounts[i]),
...
async function updateWithdrawGas() {
  ...
  const el = document.getElementById(elId);
  if (!el || !selectedPool) return;
  if (withdrawMode === 'any') return;   // the route quote reports its own gas
  if (typeof window.estimateMultiStepGas !== 'function') return;
  if (typeof ethers === 'undefined') { try { await loadEthers(); } catch { return; } }
  const lpVal = document.getElementById('withdrawLPAmount')?.value;
    ...
    if (walletAddress) {
      ...
      if (allowance < amount) {
        steps.push({
          label: 'Approve LP',
          to: lpAddr,
          data: window._buildApproveCalldata(gauge, amount),
          fallback: window._GAS_FALLBACK.approve,
        });
      }
    }
...
async function updateDasGas() {
  ...
  try {
    ...
    steps.push({
      ...
      // Оценка газа: LP ещё не существует, сумма неизвестна. Важен только класс
      // записи (нулевое → ненулевое), поэтому 1n, а не бессрочный максимум.
      data: window._buildApproveCalldata(gauge, 1n),
      fallback: window._GAS_FALLBACK.approve,
    });
...
// ============================================================
// YIELD: Zap tab — deposit into a pool with ANY token (Alexandr crvecodev/1690).
// Quotes come from our own endpoint; the Enso key never reaches the browser.
// No token list is shipped: symbols/addresses are looked up upstream on demand.
// ============================================================
const ZAP_MAX_SLIPPAGE = 5.0;      // server refuses more, keep the UI honest
const ZAP_LOSS_WARN = 1.0;         // % of входа — above this we warn out loud
let _zapToken = null;              // {address, symbol, decimals}
let _zapQuote = null;
let _zapQuoteTimer = null;
let _zapSeq = 0;

function _zapBase() {
  return ((typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '') + '/curvedex/enso';
}

function resetZapUI() {
  _zapToken = null;
  _zapQuote = null;
  _zapExtraTokens.clear();
  const amount = document.getElementById('zapAmount');
  if (amount) amount.value = '';
  _zapPaintToken(null);
  _zapHide('zapPreview');
  _zapHide('zapWarn');
  const gas = document.getElementById('zapGas');
  if (gas) gas.textContent = '--';
  updateZapButton();
}

function _zapHide(id) { const el = document.getElementById(id); if (el) el.style.display = 'none'; }
function _zapShow(id) { const el = document.getElementById(id); if (el) el.style.display = ''; }

// Token picker: the very modal the Swap view uses -- wallet balances first,
// search by symbol or address. Tokens outside Curve pools come from the router.
let _zapExtraTokens = new Map();   // lower address -> {address, symbol, decimals}
let _zapBalance = 0;

let _zapOutToken = null;           // Withdraw -> any token

window._zapPickers = {
  zap:    { select: 'selectZapToken',    current: () => _zapToken },
  zapout: { select: 'selectZapOutToken', current: () => _zapOutToken },
};

function _openZapPicker(target) {
...
function openZapTokenPicker() { _openZapPicker('zap'); }
function openZapOutTokenPicker() { _openZapPicker('zapout'); }

let _zapSearchTimer = null;

// Debounced: every keystroke re-renders the list, and a burst of lookups would
// hit the router's rate limit -- the last answer then arrives empty.
function zapEnsoSearchInto(list, query) {
  clearTimeout(_zapSearchTimer);
  _zapSearchTimer = setTimeout(() => _zapEnsoFetchInto(list, query), 300);
}

async function _zapEnsoFetchInto(list, query) {
  const term = String(query || '').trim();
  if (term.length < 2 || !list) return;
  const chainId = typeof getChainId === 'function' ? getChainId() : 1;
  try {
    const r = await fetch(`${_zapBase()}/tokens?chainId=${chainId}&q=${encodeURIComponent(term)}`);
    const j = await r.json();
    const rows = (j.tokens || []).slice(0, 30);
    if (!rows.length) return;
    const search = document.getElementById('swapTokenSearchInput');
    if (search && search.value.trim() !== term) return;   // a newer query is on screen
    const picker = typeof _swapModalPicker === 'function' ? _swapModalPicker() : null;
    const onPick = picker ? picker.select : 'selectZapToken';
    list.innerHTML = rows.map(tk => {
      const addr = String(tk.address || '').toLowerCase();
      _zapExtraTokens.set(addr, { address: addr, symbol: tk.symbol || '?', decimals: Number(tk.decimals) || 18 });
      const icon = typeof _tokenIconUrl === 'function' ? _tokenIconUrl(addr) : '';
      return `<div class="token-modal-item" onclick="${onPick}('${addr}')">
        <img class="token-icon" src="${icon}" alt="" width="28" height="28" style="border-radius:50%;object-fit:cover" loading="lazy" onerror="this.style.visibility='hidden'">
        <div class="token-modal-info">
          <div class="token-name">${_esc(tk.symbol || '?')}</div>
          <div class="token-addr" title="${_esc(addr)}">${addr.slice(0, 6)}...${addr.slice(-4)}</div>
          <div class="token-pools">${_esc((tk.name || '').slice(0, 32))}</div>
        </div>
      </div>`;
    }).join('');
  } catch (e) { /* leave the "No tokens found" line in place */ }
}

function _zapResolveToken(address) {
  const addr = String(address || '').toLowerCase();
  const extra = _zapExtraTokens.get(addr);
  if (extra) return extra;
  const hit = typeof tradeTokenList !== 'undefined'
    ? (tradeTokenList || []).find(t => t.address.toLowerCase() === addr) : null;
  return hit
    ? { address: hit.address, symbol: hit.symbol, decimals: Number(hit.decimals) || 18, _isNativeETH: !!hit._isNativeETH }
    : null;
}

function selectZapOutToken(address) {
  const tk = _zapResolveToken(address);
  if (!tk) return;
  _zapOutToken = tk;
  _zapOutResetRows();               // the old numbers belong to the old token
  _zapPaintTokenInto('zapOutTokenName', 'zapOutTokenIcon', tk);
  if (typeof closeSwapTokenModal === 'function') closeSwapTokenModal();
  onWithdrawAmountChange();
}

function selectZapToken(address) {
  const tk = _zapResolveToken(address);
  if (!tk) return;
  _zapToken = tk;
  _zapPaintToken(tk);
...
function _zapPaintTokenInto(nameId, iconId, tk) {
  const name = document.getElementById(nameId);
  if (name) name.textContent = tk ? (tk.symbol || '?') : 'Select';
  if (tk && typeof _setTokenIcon === 'function') {
    _setTokenIcon(iconId, tk.address, tk.symbol);
  } else {
    const icon = document.getElementById(iconId);
    if (icon) { icon.style.backgroundImage = ''; icon.style.color = ''; icon.textContent = '?'; }
  }
}

function _zapPaintToken(tk) {
  _zapPaintTokenInto('zapTokenName', 'zapTokenIcon', tk);
  if (!tk) {
    _zapBalance = 0;
    const bal = document.getElementById('zapBalance');
    if (bal) bal.textContent = '';
  }
}

async function _zapLoadBalance(tk) {
  const el = document.getElementById('zapBalance');
  _zapBalance = 0;
  if (el) el.textContent = '';
  if (!tk || typeof walletAddress === 'undefined' || !walletAddress ||
      typeof getWalletTokenBalances !== 'function') return;
  const want = String(tk.address).toLowerCase();
  try {
    const map = await getWalletTokenBalances([tk], walletAddress);
    if (!_zapToken || String(_zapToken.address).toLowerCase() !== want) return;   // token changed meanwhile
    const e = map.get(want);
    _zapBalance = e && e.balance > 0 ? e.balance : 0;
    if (!el) return;
    if (_zapBalance > 0) {
      const usd = e && e.usdValue ? ` (${_fmtTokenUsd(e.usdValue)})` : '';
      el.textContent = `Balance: ${_fmtTokenBalance(_zapBalance)}${usd}`;
    } else {
      el.textContent = 'Balance: 0';
    }
  } catch (err) { /* leave it blank rather than lie */ }
}

function zapSetPreset(fraction) {
  const amt = document.getElementById('zapAmount');
  if (!amt || !(_zapBalance > 0)) return;
  let v = _zapBalance * fraction;
  const isNative = _zapToken && (_zapToken._isNativeETH ||
    String(_zapToken.address).toLowerCase() === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee');
  if (fraction >= 1 && isNative) v = Math.max(0, v - 0.002);   // leave gas money
  amt.value = v > 0 ? String(Number(v.toFixed(8))) : '';
  onZapInputChange();
}
...
function zapSlippagePct() {
  const btn = document.querySelector('#tab-zap .zap-slip.active');
  const v = btn ? parseFloat(btn.dataset.slip) : 0.5;
  return Math.min(isFinite(v) ? v : 0.5, ZAP_MAX_SLIPPAGE);
}

function onZapInputChange() {
  clearTimeout(_zapQuoteTimer);
  _zapQuote = null;
  updateZapButton();
  const amtEl = document.getElementById('zapAmount');
  const amount = amtEl ? parseFloat(amtEl.value) : 0;
  if (!_zapToken || !(amount > 0) || !selectedPool) { _zapHide('zapPreview'); return; }
  _zapQuoteTimer = setTimeout(fetchZapQuote, 400);
}

async function fetchZapQuote() {
  const seq = ++_zapSeq;
  const amtEl = document.getElementById('zapAmount');
  const amount = amtEl ? parseFloat(amtEl.value) : 0;
  if (!_zapToken || !(amount > 0) || !selectedPool) return;
  const stake = !!(document.getElementById('zapStakeLP') || {}).checked;
  const gauge = (selectedPool.gaugeAddress || '').trim();
  if (stake && !gauge) {
    _zapWarn('This pool has no gauge — uncheck "Stake LP".', true);
    return;
  }
  const tokenOut = stake ? gauge : selectedPool.address;
  const decimals = Number(_zapToken.decimals) || 18;
  let amountIn;
  try {
    amountIn = _zapToUnits(amount, decimals);
  } catch (e) { return; }
  const chainId = typeof getChainId === 'function' ? getChainId() : 1;
  const sender = (typeof walletAddress !== 'undefined' && walletAddress)
    ? walletAddress : '0x1111111111111111111111111111111111111111';
  const slippageBps = Math.round(zapSlippagePct() * 100);
  // Upstream routing takes seconds on busy pairs - say "working" instead of
  // leaving dashes that read as "nothing found".
  _setText('zapOut', 'quoting…');
  _setText('zapMinOut', '…');
  _setText('zapLoss', '…');
  _setText('zapRoute', '…');
  _zapShow('zapPreview');
  const url = `${_zapBase()}/route?chainId=${chainId}&tokenIn=${_zapToken.address}` +
    `&tokenOut=${tokenOut}&amountIn=${amountIn}&slippage=${slippageBps}` +
    `&fromAddress=${sender}&receiver=${sender}`;
  try {
    const r = await fetch(url);
    const j = await r.json();
    if (seq !== _zapSeq) return;  // a newer input already superseded this quote
    if (j.error) { _zapWarn(j.error, true); _zapHide('zapPreview'); _zapQuote = null; updateZapButton(); return; }
    _zapQuote = j;
    renderZapQuote(amount, decimals, stake);
  } catch (e) {
    if (seq === _zapSeq) { _zapWarn('Quote failed — try again', true); _zapQuote = null; updateZapButton(); }
  }
}

function _zapToUnits(amount, decimals) {
  // String math: floats lose money (Михаил: децималы/BigInt, никогда float).
  const [int, frac = ''] = String(amount).split('.');
  const padded = (frac + '0'.repeat(decimals)).slice(0, decimals);
  return (BigInt(int || '0') * (10n ** BigInt(decimals)) + BigInt(padded || '0')).toString();
}

function _zapFromUnits(raw, decimals) {
...
function renderZapQuote(amountIn, decimalsIn, stake) {
  const q = _zapQuote;
  if (!q) return;
  const lpDecimals = 18;
  const out = _zapFromUnits(q.amountOut || '0', lpDecimals);
  const minOut = _zapFromUnits(q.minAmountOut || '0', lpDecimals);
  const label = stake ? 'staked LP' : 'LP';
  _setText('zapOut', `${_fmtNum(out)} ${label}`);
  _setText('zapMinOut', `${_fmtNum(minOut)} ${label}`);

  _setText('zapLoss', '…');
  _zapRenderLoss('zapLoss', amountIn, _zapToken.address, out, _zapLpAddress(), _zapBandPct(q));
  _setText('zapRoute', (q.route || []).join(' → ') || '--');
  const gasEl = document.getElementById('zapGas');
  if (gasEl) gasEl.textContent = q.gas ? `${(Number(q.gas) / 1000).toFixed(0)}k gas` : '--';
  _zapShow('zapPreview');
  updateZapButton();
}

// How far the quote itself allows the output to drift, in percent.
function _zapBandPct(q) {
  const out = Number(q && q.amountOut);
  const min = Number(q && q.minAmountOut);
  return (isFinite(out) && out > 0 && isFinite(min) && min > 0) ? (out - min) / out * 100 : 0;
}

const _zapPriceCache = new Map();

// Our own LP price, the way the portfolio computes it: TVL over supply. Saves a
// round-trip and keeps the two zap directions on one number. Measured 31.07 on
// pyUSD/crvUSD: ours 1.00789, Curve prices API 1.00775, router 1.00842 - the
// three agree, so this is a shortcut, not a correction.
function _zapLpUsdPrice() {
  if (!selectedPool) return 0;
  const tvl = Number(selectedPool.tvl || selectedPool.usdTotal) || 0;
  const supply = selectedPool.totalSupply ? parseFloat(String(selectedPool.totalSupply)) / 1e18 : 0;
  return (tvl > 0 && supply > 0) ? tvl / supply : 0;
}

async function _zapPrice(address) {
  if (selectedPool && String(address).toLowerCase() === String(_zapLpAddress()).toLowerCase()) {
    const own = _zapLpUsdPrice();
    if (own > 0) return own;
  }
  const chainId = typeof getChainId === 'function' ? getChainId() : 1;
  const key = chainId + ':' + String(address).toLowerCase();
  const hit = _zapPriceCache.get(key);
  if (hit && Date.now() - hit.ts < 60000) return hit.price;
  try {
    const r = await fetch(`${_zapBase()}/price?chainId=${chainId}&address=${address}`);
    const j = await r.json();
    const price = Number(j.price);
    if (isFinite(price) && price > 0) {
      _zapPriceCache.set(key, { price, ts: Date.now() });
      return price;
    }
  } catch (e) { /* price is a nicety, the quote itself already stands */ }
  return 0;
}

// Enso returns priceImpact = 0 for every route, so the loss is ours to compute:
// what the input is worth vs what the LP (or staked LP) we receive is worth.
// bandPct: the quote's own slippage band (out vs minimum out). Below it a
// difference is indistinguishable from rounding, so it is not worth flagging.
async function _zapRenderLoss(elId, amountIn, addrIn, amountOut, addrOut, bandPct) {
  const el = document.getElementById(elId);
  if (!el) return;
  const [priceIn, priceOut] = await Promise.all([_zapUsdPrice(addrIn), _zapUsdPrice(addrOut)]);
  const label = document.getElementById(elId + 'Label');
  const _label = t => { if (label) label.textContent = t; };
  if (!(priceIn > 0) || !(priceOut > 0)) {
    _label('Route loss');
    el.textContent = 'n/a (no price for this token)';
    return;
  }
  const usdIn = amountIn * priceIn;
  const usdOut = amountOut * priceOut;
  const lossPct = (usdIn - usdOut) / usdIn * 100;
  const diff = _fmtNum(Math.abs(usdIn - usdOut));
  // A negative "loss" means the LP is worth more than what went in. A router
  // does not hand out money, so that is the price feed lagging the market, not
  // a bonus: keep the number, but say out loud it cannot be trusted.
  // Short on purpose: the amount paid is already on screen, and a long value
  // crowded the label on one row.
  // The direction lives in the label, so the number stays unsigned (Alexandr
  // crvecodev/1825: "Route loss: +0.03%" read like a bug). A gain is not
  // automatically a display error — a step that converts at par while the input
  // trades below it really does pay: 10 000 USDC -> 10 000.000000 frxUSD while
  // USDC sits at 0.99954. So no accusation in the value, just the fact in the
  // tooltip.
  const gain = lossPct < 0;
  _label(gain ? 'Route gain' : 'Route loss');
  el.textContent = `${Math.abs(lossPct).toFixed(2)}%  ($${diff})`;
  el.style.color = gain ? 'var(--green, #22c55e)' : '';
  el.title = gain
    ? `In $${_fmtNum(usdIn)} \u2192 out $${_fmtNum(usdOut)} \u2014 the output is worth more than the input, which happens when a step of the route converts at par while the input trades below it. Judge the trade by "Minimum received".`
    : `In $${_fmtNum(usdIn)} \u2192 out $${_fmtNum(usdOut)}`;
  if (elId !== 'zapLoss') return;   // the deposit tab owns the warning banner
  if (lossPct > ZAP_LOSS_WARN) {
    _zapWarn(`Route loss ${lossPct.toFixed(2)}% of your deposit — check the amount before signing.`, false);
  } else {
    _zapHide('zapWarn');
  }
}
...
// Every row of the Any Token preview is written by one quote; clear them as one
// too, otherwise a switched token or a dropped quote leaves half the rows
// showing the previous answer.
function _zapOutResetRows(placeholder) {
  const v = placeholder || '--';
  _setText('zapOutAmount', v);
  _setText('zapOutMin', v);
  _setText('zapOutLoss', v);
  _setText('zapOutRoute', v);
  _zapOutQuote = null;
}
...
// Most Curve pools mint the LP from the pool contract itself, older ones have
// a separate token: pick whichever the pool data says.
// The LP leg is priced by the pool itself, not by the router's feed: Enso
// quoted this pool's LP at 1.005991 against 1.005623 from the pool's own
// TVL/supply, and that 0.037% gap surfaced as a phantom "+0.05% profit" on
// every zap (Alexandr crvecodev/1812). Any other token keeps the market feed —
// USDC really does trade a few bp under $1, and hiding that would be a lie.
function _zapUsdPrice(addr) {
  const lp = _zapLpAddress();
  if (lp && addr && addr.toLowerCase() === lp.toLowerCase() && typeof lpUsdPrice === 'function') {
    const p = lpUsdPrice(selectedPool);
    if (p > 0) return Promise.resolve(p);
  }
  return _zapPrice(addr);
}

function _zapLpAddress() {
  return selectedPool ? (selectedPool.lpTokenAddress || selectedPool.address) : '';
}

function _zapWarn(text, isError, elId) {
  const el = document.getElementById(elId || 'zapWarn');
  if (!el) return;
  el.textContent = text;
  el.style.background = isError ? 'rgba(246,70,93,0.12)' : 'rgba(240,185,11,0.12)';
  el.style.color = isError ? 'var(--red,#f6465d)' : 'var(--yellow,#f0b90b)';
  el.style.display = '';
}

function updateZapButton() {
  const btn = document.getElementById('zapSubmit');
  if (!btn) return;
  ...
  if (!_zapToken) { btn.textContent = 'Select a token'; btn.className = 'action-btn'; btn.disabled = true; return; }
  const amtEl = document.getElementById('zapAmount');
  if (!(parseFloat(amtEl ? amtEl.value : '0') > 0)) {
    btn.textContent = 'Enter an amount'; btn.className = 'action-btn'; btn.disabled = true; return;
  }
  if (!_zapQuote) { btn.textContent = 'Quoting…'; btn.className = 'action-btn'; btn.disabled = true; return; }
  btn.textContent = (document.getElementById('zapStakeLP') || {}).checked ? 'Zap & Stake' : 'Zap in';
  btn.className = 'action-btn';
  btn.disabled = false;
}

// The router pulls tokenIn out of the wallet, so an ERC20 needs an allowance
// first or the route reverts. Native gas token needs none. Gas per the house
// rule: estimate x1.5, never a hardcoded limit.
async function _zapSendRoute(quote, tokenInAddr, amountUnits, warnId) {
  await loadEthers();
  if (!signer && provider) signer = await provider.getSigner();
  const tx = quote.tx;
  const native = String(tokenInAddr).toLowerCase() === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee';
  const gasOverrides = async () => (typeof window.computeMichwillGasParams === 'function'
    ? await window.computeMichwillGasParams(provider) : {});
  const withGas = async (req) => {
    if (typeof window.estimateGasWithBuffer === 'function') {
      req.gasLimit = await window.estimateGasWithBuffer(provider, req, walletAddress);
    }
    return { ...req, ...(await gasOverrides()) };
  };

  if (!native) {
    const token = new ethers.Contract(tokenInAddr, ERC20_ABI, signer);
    const allowance = await token.allowance(walletAddress, tx.to);
    if (allowance < BigInt(amountUnits)) {
      _zapWarn('Approving the router — confirm the first transaction…', false, warnId);
      for (const amt of window.approveAmounts(allowance, BigInt(amountUnits))) {
        const data = token.interface.encodeFunctionData('approve', [tx.to, amt]);
        const approve = await signer.sendTransaction(await withGas({ to: tokenInAddr, data, value: 0n }));
        await approve.wait();
      }
    }
  }

  const sent = await signer.sendTransaction(await withGas({
    to: tx.to,
    data: tx.data,
...
async function handleZapSubmit() {
  ...
  if (!_zapQuote || !_zapQuote.tx) { _zapWarn('No quote yet', true); return; }
  const btn = document.getElementById('zapSubmit');
  try {
    if (btn) { btn.disabled = true; btn.textContent = 'Confirm in wallet…'; }
    const amtEl = document.getElementById('zapAmount');
    const units = _zapToUnits(amtEl ? amtEl.value : '0', Number(_zapToken.decimals) || 18);
    const hash = await _zapSendRoute(_zapQuote, _zapToken.address, units, 'zapWarn');
    _zapWarn(`Sent: ${String(hash).slice(0, 12)}…`, false);
  } catch (e) {
    _zapWarn((e && e.message ? e.message : 'Transaction rejected').slice(0, 120), true);
  } finally {
    updateZapButton();
  }
}
...
// ============================================================
// WITHDRAW -> ANY TOKEN (reverse zap)
// ============================================================
let _zapOutQuote = null;
let _zapOutSeq = 0;

// Withdraw -> any token can start from the gauge: the router unstakes, redeems
// and swaps in one transaction, so no separate unstake step is needed.
function _zapOutFromGauge() {
  const cb = document.getElementById('zapOutFromGauge');
  return !!(cb && cb.checked && _zapGaugeAddress());
}

function _zapGaugeAddress() {
  const g = selectedPool && selectedPool.gaugeAddress;
  return (g && g !== '0x0000000000000000000000000000000000000000') ? g : '';
}

// What the route spends, and therefore what the amount is limited by.
function _zapOutSourceAddress() {
  return _zapOutFromGauge() ? _zapGaugeAddress() : _zapLpAddress();
}

function _zapOutSourceBalance() {
  return _zapOutFromGauge() ? stakedLPRaw : lpBalanceRaw;
}

function onZapOutSourceChange() {
  _zapRefreshWithdrawBalanceLabel();
  onWithdrawAmountChange();
}

function _zapRefreshWithdrawBalanceLabel() {
  const el = document.getElementById('withdrawLPBalance');
  if (!el || typeof ethers === 'undefined') return;
  const raw = _zapOutSourceBalance();
  const label = _zapOutFromGauge() ? 'Staked: ' : 'Balance: ';
  el.textContent = label + _formatLP(parseFloat(ethers.formatUnits(raw, 18)));
}

function _zapOutSlippagePct() {
  const btn = document.querySelector('#tab-withdraw .wd-slip.active');
  const v = btn ? parseFloat(btn.dataset.slip) : 0.5;
  return Math.min(isFinite(v) ? v : 0.5, ZAP_MAX_SLIPPAGE);
}

async function fetchZapOutQuote(lpVal) {
  const seq = ++_zapOutSeq;
  _zapOutQuote = null;
  if (!_zapOutToken || !selectedPool) { _zapHide('zapOutPreview'); _zapOutResetRows(); return; }
  _zapHide('zapOutWarn');
  const lpUnits = _zapToUnits(lpVal, 18);
  const chainId = typeof getChainId === 'function' ? getChainId() : 1;
  const sender = (typeof walletAddress !== 'undefined' && walletAddress)
    ? walletAddress : '0x1111111111111111111111111111111111111111';
  _zapOutResetRows('…');
  _zapShow('zapOutPreview');
  const url = `${_zapBase()}/route?chainId=${chainId}&tokenIn=${_zapOutSourceAddress()}` +
    `&tokenOut=${_zapOutToken.address}&amountIn=${lpUnits}` +
    `&slippage=${Math.round(_zapOutSlippagePct() * 100)}&fromAddress=${sender}&receiver=${sender}`;
  try {
    const r = await fetch(url);
    const j = await r.json();
    if (seq !== _zapOutSeq) return;
    if (j.error) {
      _zapWarn(j.error, true, 'zapOutWarn');
      _zapHide('zapOutPreview');
      _zapOutResetRows();
      updateWithdrawButton();
      return;
    ...
    _zapOutQuote = j;
    const dec = Number(_zapOutToken.decimals) || 18;
    const out = _zapFromUnits(j.amountOut || '0', dec);
    const minOut = _zapFromUnits(j.minAmountOut || '0', dec);
    const sym = _zapOutToken.symbol || '';
    _setText('zapOutAmount', _fmtNum(out));
    _setText('zapOutMin', `${_fmtNum(minOut)} ${sym}`);
    _setText('zapOutRoute', (j.route || []).join(' → ') || '--');
    const gasEl = document.getElementById('withdrawGas');
    if (gasEl) gasEl.textContent = j.gas ? `${(Number(j.gas) / 1000).toFixed(0)}k gas` : '--';
    _zapRenderLoss('zapOutLoss', parseFloat(lpVal), _zapLpAddress(), out, _zapOutToken.address, _zapBandPct(j));
  } catch (e) {
    if (seq === _zapOutSeq) {
      _zapWarn('Quote failed — try again', true, 'zapOutWarn');
      _zapHide('zapOutPreview');
    }
  }
  updateWithdrawButton();
...
async function handleZapOutSubmit() {
  if (!_zapOutQuote || !_zapOutQuote.tx) { _zapWarn('No quote yet', true, 'zapOutWarn'); return; }
  const btn = document.getElementById('withdrawSubmit');
  const lpVal = document.getElementById('withdrawLPAmount').value;
  if (BigInt(_zapToUnits(lpVal, 18)) > _zapOutSourceBalance()) {
    _zapWarn(_zapOutFromGauge() ? 'More than you have staked' : 'More LP than you hold', true, 'zapOutWarn');
    return;
  }
  try {
    if (btn) { btn.disabled = true; btn.textContent = 'Confirm in wallet…'; }
    const hash = await _zapSendRoute(_zapOutQuote, _zapOutSourceAddress(), _zapToUnits(lpVal, 18), 'zapOutWarn');
    _zapWarn(`Sent: ${String(hash).slice(0, 12)}…`, false, 'zapOutWarn');
  } catch (e) {
    _zapWarn((e && e.message ? e.message : 'Transaction rejected').slice(0, 120), true, 'zapOutWarn');
  } finally {
...
// Slippage buttons inside the Zap tab behave like the other tabs.
document.addEventListener('click', (e) => {
  const b = e.target && e.target.closest ? e.target.closest('#tab-zap .zap-slip') : null;
  if (!b) return;
  document.querySelectorAll('#tab-zap .zap-slip').forEach(x => x.classList.remove('active'));
