...
// GET /ohlc/:pool_address?chain=&main=SYM&ref=SYM&bucket=SEC&start=&end=
// Candles built from our own trade index. Lite chains are not served by
// prices.curve.finance at all, so without this the pool chart has zero candles
// and the volume histogram (which is keyed BY candle timestamps) stays empty.
// Price convention matches Curve's price API: value = how many MAIN per one REF.
app.get('/ohlc/:pool_address', (req, res) => {
  const pool = req.params.pool_address.toLowerCase();
  const chain = (req.query.chain || 'ethereum').toLowerCase();
  const main = String(req.query.main || '');
  const ref = String(req.query.ref || '');
  const bucket = Math.max(60, parseInt(req.query.bucket) || 3600);
  const end = parseInt(req.query.end) || Math.floor(Date.now() / 1000);
  const start = parseInt(req.query.start) || (end - 30 * 86400);
  if (!main || !ref || main === ref) return res.status(400).json({ error: 'main and ref required' });
  let db;
  try { db = openDB(); } catch (err) { return res.status(500).json({ error: 'Database not found.' }); }
  try {
    const rows = db.prepare(`
      SELECT timestamp, token_in, token_out, amount_in, amount_out, usd_value
      FROM trades
      WHERE chain = ? AND pool = ? AND timestamp >= ? AND timestamp <= ?
        AND ((token_in = ? AND token_out = ?) OR (token_in = ? AND token_out = ?))
      ORDER BY timestamp ASC
    `).all(chain, pool, start, end, ref, main, main, ref);
    db.close();
    const byBucket = new Map();
    for (const r of rows) {
      const ain = Number(r.amount_in), aout = Number(r.amount_out);
      if (!(ain > 0) || !(aout > 0)) continue;
      // MAIN per one REF: selling ref buys main -> out/in; the mirrored leg -> in/out.
      const price = (r.token_in === ref) ? (aout / ain) : (ain / aout);
      if (!isFinite(price) || price <= 0) continue;
      const t = Math.floor(r.timestamp / bucket) * bucket;
      let b = byBucket.get(t);
      if (!b) { b = { time: t, open: price, high: price, low: price, close: price, volume: 0, count: 0 }; byBucket.set(t, b); }
      if (price > b.high) b.high = price;
      if (price < b.low) b.low = price;
      b.close = price;
      b.volume += Number(r.usd_value) || 0;
      b.count += 1;
    }
    const data = Array.from(byBucket.values()).sort((a, b) => a.time - b.time);
    res.json({ chain, pool, main, ref, bucket, count: data.length, data });
  } catch (err) {
...
// GET /trades/:pool_address/list?chain=&page=&per_page=
// Raw trades, newest first — the pool activity panel needs rows, not buckets.
// Lite chains have no upstream trades API, this is their only source.
app.get('/trades/:pool_address/list', (req, res) => {
  const pool = req.params.pool_address.toLowerCase();
  const chain = (req.query.chain || 'ethereum').toLowerCase();
  ...
  try {
    const rows = db.prepare(`
      SELECT tx_hash, timestamp, token_in, token_out, amount_in, amount_out, usd_value
      FROM trades WHERE chain = ? AND pool = ?
      ORDER BY timestamp DESC LIMIT ? OFFSET ?
    `).all(chain, pool, perPage, (page - 1) * perPage);
    db.close();
    res.json({ chain, pool, page, per_page: perPage, data: rows });
  } catch (err) {
    try { db.close(); } catch (e) {}
    res.status(500).json({ error: String(err) });
  }
