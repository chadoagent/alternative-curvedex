...
// A uint256 getter on an OLD Vyper pool answers with the value followed by
// trailing zero words (0x…3d0900 then ~30 empty words on the MIM metapool's
// fee()). BigInt() over the whole blob is astronomically large, Number() of it
// is Infinity, the isFinite guard drops it — and the Fee row shows '--' while
// the call actually succeeded. The value is the FIRST word in both encodings.
function _firstUintWord(hex) {
  if (typeof hex !== 'string' || hex.length < 3 || hex.slice(0, 2) !== '0x') return null;
...
// ============================================================
// ↔️ Pools-tab price-direction toggle (candleSeries + шапка страницы пула)
// Mirrors the Trade-tab inversion architecture but for the separate /pools surface.
// Reuses the candle-inversion math (_tpInvCandle); разворачивается ТОЛЬКО ячейка
// показанной пары, остальные курсы в строке стоят в каноническом порядке.
// ============================================================
...
// ПАРА, КОТОРУЮ ПОКАЗЫВАЕТ ГРАФИК ПУЛА. У пула из трёх монет пар три, и кнопка ↔️
// обходит их по кругу: A/B → B/A → A/C → C/A → B/C → C/B. До этого она умела только
// перевернуть первую пару, и в трёхмонетном пуле выглядела нерабочей (Alexandr
// crvecodev/2370). Выбор запоминается в браузере на КОНКРЕТНЫЙ пул.
let poolPricePairIdx = null;   // null = идём за выбором токенов в форме свопа
// Знаменатель, выбранный человеком НА ВЕСЬ ПУЛ (индекс монеты). Разворот одной
// пары переворачивает и остальные пары с той же монетой: у SPY/USDG и NVDA/USDG
// знаменатель общий, и держать его по паре значит спорить с собой (Alexandr 23.09).
let poolQuoteIdx = null;

// Все неупорядоченные пары пула: [[0,1],[0,2],[1,2]] для трёх монет.
function _poolPairList(pool) {
  const n = ((pool && pool.coinsAddresses) || []).length;
  const out = [];
  for (let i = 0; i < n; i++) for (let j = i + 1; j < n; j++) out.push([i, j]);
  return out;
}

function _poolPairPrefKey(pool) {
  return 'cdx:poolPair:' + getChainKey() + ':' + String((pool && pool.address) || '').toLowerCase();
}

function _savePoolPairPref(pool) {
  if (!pool) return;
  try {
    localStorage.setItem(_poolPairPrefKey(pool),
      JSON.stringify({ pairIdx: poolPricePairIdx, quoteIdx: poolQuoteIdx }));
  } catch (e) { /* приватный режим — просто не запоминаем */ }
}

// Вызывается при открытии пула: восстановить его пару и направление.
function _restorePoolPairPref(pool) {
  poolPricePairIdx = null;
  poolQuoteIdx = null;
  poolPriceInverted = false;
  if (!pool) return;
  try {
    const o = JSON.parse(localStorage.getItem(_poolPairPrefKey(pool)) || 'null');
    if (o) {
      const pairs = _poolPairList(pool);
      if (typeof o.pairIdx === 'number' && pairs[o.pairIdx]) poolPricePairIdx = o.pairIdx;
      const nCoins = ((pool.coinsAddresses) || []).length;
      if (typeof o.quoteIdx === 'number' && o.quoteIdx >= 0 && o.quoteIdx < nCoins) poolQuoteIdx = o.quoteIdx;
    }
  } catch (e) { /* мусор в хранилище — идём по умолчанию */ }
  // Сторона показа ВЫЧИСЛЯЕТСЯ из знаменателя и привязанности — отдельного
  // хранимого флага нет, иначе два источника расходятся.
  poolPriceInverted = _pairPrefersInverted(pool, _poolCanonPair(pool));
}

function _poolCanonPairIdx(pool) {
  const pairs = _poolPairList(pool);
  if (!pairs.length) return -1;
  if (poolPricePairIdx != null && pairs[poolPricePairIdx]) return poolPricePairIdx;
  const f = (typeof selectedFromToken !== 'undefined' && selectedFromToken) ? selectedFromToken.index : 0;
  const t = (typeof selectedToToken !== 'undefined' && selectedToToken) ? selectedToToken.index : 1;
  const lo = Math.min(f, t), hi = Math.max(f, t);
  const k = pairs.findIndex(p => p[0] === lo && p[1] === hi);
  return k >= 0 ? k : 0;
}

// Пара в КАНОНИЧЕСКОМ порядке [lo, hi] — ровно та, в которой лежат свечи в кэше.
function _poolCanonPair(pool) {
  const pairs = _poolPairList(pool);
  const idx = _poolCanonPairIdx(pool);
  return idx >= 0 ? pairs[idx] : null;
}
...
function _syncPoolPriceInvertBtn() {
  ...
  const cp = p ? _poolCanonPair(p) : null;
  if (p && p.coins && cp) {
    const base = p.coins[poolPriceInverted ? cp[1] : cp[0]];
    const quote = p.coins[poolPriceInverted ? cp[0] : cp[1]];
    const more = _poolPairList(p).length > 1 ? ' Click to invert, then to move to the next pair.' : ' Click to invert.';
    btn.title = `Showing 1 ${base} = … ${quote}.` + more;
  } else {
...
// Сторона пары, в которой курс читается человеком: если ОДНА монета привязанная
// (каталог рейтингов, тот же индекс, что у фильтра «Stablecoins»), а вторая нет,
// цену показываем В ПРИВЯЗАННОЙ: «NVDA/USDG 229», не «USDG/NVDA 0.004369»
// (Alexandr 23.09). Списка символов здесь нет — решает каталог; запасной путь —
// общий _isStableSym, когда индекс ещё не загрузился.
function _isPeggedCoin(pool, k) {
  const addr = ((pool.coinsAddresses || [])[k] || '').toLowerCase();
  const idx = window._ratingsTokens;
  if (idx && idx.size > 0 && addr) return idx.has(addr);
  const sym = (pool.coins || [])[k];
  return !!(window._isStableSym && window._isStableSym(sym));
}
// Сторона по привязанности монет — правило по умолчанию.
function _pegPrefersInverted(pool, pr) {
  if (!pool || !pr) return false;
  const a = _isPeggedCoin(pool, pr[0]), b = _isPeggedCoin(pool, pr[1]);
  return a && !b;
}
// true — пару надо показать развёрнутой относительно канонического coins[i]/coins[j].
// Выбранный человеком знаменатель пула сильнее правила по привязанности.
function _pairPrefersInverted(pool, pr) {
  if (!pool || !pr) return false;
  if (poolQuoteIdx != null) {
    if (pr[1] === poolQuoteIdx) return false;
    if (pr[0] === poolQuoteIdx) return true;
  }
  return _pegPrefersInverted(pool, pr);
}
// Разворот ПОКАЗАННОЙ пары = смена знаменателя пула: вторая монета пары
// становится знаменателем, и все её пары переворачиваются заодно.
function _flipPoolQuote(pool) {
  const pr = _poolCanonPair(pool);
  if (!pr) return;
  const flipped = _pairPrefersInverted(pool, pr);
  poolQuoteIdx = flipped ? pr[1] : pr[0];
  poolPriceInverted = _pairPrefersInverted(pool, pr);
  _savePoolPairPref(pool);
}

// ЦЕНЫ ВСЕХ ПАР ПУЛА одной строкой (Alexandr 23.09) — вместо большой цифры.
// Источник у всех ячеек ОДИН: та же ручка свечей, что рисует график, только
// дневной агрегацией. Ячейка показанной пары берёт close ИЗ РЯДА ГРАФИКА —
// иначе на панели стояли бы два числа одной пары от разных авторов.
const _POOL_SPOT_TTL_MS = 5 * 60 * 1000;
const _poolSpotCache = {};
function _spotKey(pool, i, j) { return getChainKey() + ':' + String(pool.address).toLowerCase() + ':' + i + '-' + j; }

async function _fetchPairSpot(pool, i, j) {
  const { main, ref } = poolPriceTokens(pool, i, j);
  if (!main || !ref || main === ref) return null;
  const end = Math.floor(Date.now() / 1000);
  const start = end - 3 * 86400;
  const _cc = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains;
  const lite = !!(_cc && _cc[getChainKey()] && _cc[getChainKey()].lite);
  try {
    if (lite) {
      const addrs = pool.coinsAddresses || [];
      const idxOf = a => addrs.findIndex(x => String(x).toLowerCase() === String(a).toLowerCase());
      const rows = await _fetchLiteCandles(pool, (pool.coins || [])[idxOf(main)], (pool.coins || [])[idxOf(ref)], 1, 'day', start, end);
      const last = rows && rows.length ? rows[rows.length - 1] : null;
      return last && isFinite(last.close) ? last.close : null;
    }
    const url = `${PRICES_BASE}/ohlc/${getChainKey()}/${pool.address}?main_token=${main}&reference_token=${ref}`
    ...
    return last && isFinite(last.close) && last.close > 0 ? last.close : null;
  } catch (e) { return null; }
}

function renderPoolPairPrices(pool, shownClose) {
  const el = document.getElementById('poolPairPrices');
  if (!el) return;
  if (!pool || !pool.coins || pool.coins.length < 2) { el.innerHTML = '--'; return; }
  const pairs = _poolPairList(pool);
  const activeIdx = _poolCanonPairIdx(pool);
  const inv = (typeof poolPriceInverted !== 'undefined') && poolPriceInverted;
  const coins = pool.coins || [];
  const html = pairs.map((pr, k) => {
    const isActive = k === activeIdx;
    // Показанная пара следует кнопке ↔️, остальные — предпочтительной стороне.
    const flip = isActive ? inv : _pairPrefersInverted(pool, pr);
    const a = flip ? pr[1] : pr[0];
    const b = flip ? pr[0] : pr[1];
    let price = null;
    const hit = _poolSpotCache[_spotKey(pool, pr[0], pr[1])];
    if (hit && (Date.now() - hit.ts) < _POOL_SPOT_TTL_MS) price = hit.v;
    if (isActive) {
      if (shownClose != null && isFinite(shownClose) && shownClose > 0) price = shownClose;
      else if (price != null && inv) price = 1 / price;
    } else if (price != null && flip) {
      price = 1 / price;
    }
    const txt = (price != null && isFinite(price) && price > 0) ? fmtPrice(price) : '--';
    const valId = isActive ? ' id="chartPrice"' : '';
    return `<button type="button" class="pair-price${isActive ? ' active' : ''}" onclick="setPoolChartPair(${k})"`
      + ` title="Show ${coins[a]}/${coins[b]} on the chart">`
      + `<span class="pair-price-sym">${coins[a]}/${coins[b]}</span>`
      + `<span class="pair-price-val"${valId}>${txt}</span></button>`;
  }).join('');
  el.innerHTML = html || '--';
}
window.renderPoolPairPrices = renderPoolPairPrices;

async function loadPoolPairPrices(pool) {
  if (!pool) return;
  renderPoolPairPrices(pool);
  if (pool.deprecated) return;
  const pairs = _poolPairList(pool);
  await Promise.all(pairs.map(async (pr) => {
    const key = _spotKey(pool, pr[0], pr[1]);
    const hit = _poolSpotCache[key];
    if (hit && (Date.now() - hit.ts) < _POOL_SPOT_TTL_MS) return;
    const v = await _fetchPairSpot(pool, pr[0], pr[1]);
    if (v != null) _poolSpotCache[key] = { v, ts: Date.now() };
  }));
  if (selectedPool === pool) renderPoolPairPrices(pool);
}
window.loadPoolPairPrices = loadPoolPairPrices;

// Клик по ячейке: чужая пара — показать её на графике, своя — развернуть.
function setPoolChartPair(k) {
  const pool = (typeof selectedPool !== 'undefined') ? selectedPool : null;
  if (!pool) return;
  const pairs = _poolPairList(pool);
  if (!pairs[k]) return;
  if (k === _poolCanonPairIdx(pool)) { _flipPoolQuote(pool); }
  else { poolPricePairIdx = k; poolPriceInverted = _pairPrefersInverted(pool, pairs[k]); _savePoolPairPref(pool); }
  window._poolCanonCandles = null;
  renderPoolPairPrices(pool);
  if (typeof loadOHLC === 'function') loadOHLC();
  if (typeof _syncPoolPriceInvertBtn === 'function') _syncPoolPriceInvertBtn();
}
window.setPoolChartPair = setPoolChartPair;

function togglePoolPriceDirection() {
  const pool = (typeof selectedPool !== 'undefined') ? selectedPool : null;
  // Кнопка меняет ЗНАМЕНАТЕЛЬ пула — к другим парам ведут их собственные ячейки
  // в строке курсов, поэтому круга по парам здесь больше нет.
  if (!pool) return;
  _flipPoolQuote(pool);
  renderPoolPairPrices(pool);
...
function _renderPoolChartFromCandles(canonCandles) {
  ...
  const first = view[0];
  renderPoolPairPrices(selectedPool, last.close);
...
function renderPoolFavoritesSidebar() {
  ...
  list.innerHTML = items.map(p => {
    ...
    const baseIcon = baseAddr ? `<img src="${(typeof _tokenIconUrl === 'function') ? _tokenIconUrl(baseAddr) : ''}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">` : '';
    const quoteIcon = quoteAddr ? `<img src="${(typeof _tokenIconUrl === 'function') ? _tokenIconUrl(quoteAddr) : ''}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">` : '';
...
function _renderPoolHeader(pool, stats, candles) {
  ...
  const _cp = _poolCanonPair(pool) || [0, 1];
  const cBaseSym = (pool.coins && pool.coins[_cp[0]]) || '';
  const cQuoteSym = (pool.coins && pool.coins[_cp[1]]) || '';
  ...
  const baseAddr = (pool.coinsAddresses && pool.coinsAddresses[_inv ? _cp[1] : _cp[0]]) || '';
  const quoteAddr = (pool.coinsAddresses && pool.coinsAddresses[_inv ? _cp[0] : _cp[1]]) || '';
  // Token icons
  ...
  if (nameEl) {
    ...
    if (pool.coins && pool.coins.length >= 2) {
      ...
      // keep their position.
      // Сначала показанная пара, потом остальные монеты пула — иначе в трёхмонетном
      // пуле кнопка ↔️ стояла бы между двумя монетами, которых график не показывает.
      const _ord = [_cp[0], _cp[1]];
      for (let i = 0; i < pool.coins.length; i++) if (_ord.indexOf(i) === -1) _ord.push(i);
      if (_inv) { const t0 = _ord[0]; _ord[0] = _ord[1]; _ord[1] = t0; }
      const coinsForLabel = _ord.map(i => pool.coins[i]);
...
function updateTradePoolInfo() {
  ...
  // Lite chains have no fee FACT anywhere (no getVolumes upstream), and a figure
  // derived as volume x flat rate is exactly what we agreed not to print there
  // (Alexandr crvecodev/2360). Blank beats a made-up number.
  const _ccF = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains;
  const _liteF = !!(_ccF && _ccF[getChainKey()] && _ccF[getChainKey()].lite);
  const _feesEl = document.getElementById('infoFees');
  _feesEl.textContent = (!_liteF && pool.volumeUSD > 0) ? '~' + fmt$(estFees) : '--';
  // Прочерк ЗДЕСЬ — решение, а не недоделка (ставки комиссии на Lite нет ни у
  // кого). Помечаем элемент, чтобы проверка строк не считала его заглушкой.
  if (_liteF) _feesEl.dataset.blankByDesign = 'lite-no-fee-rate';
  else delete _feesEl.dataset.blankByDesign;
...
function initTradeChart() {
  ...
  // Кнопка копирования графика берёт график и свечи отсюда (chart_share.js).
  if (window.registerChart) registerChart('trade-chart-container', tradeChart, candleSeries);
...
// Гранулярность ручки объёма под таймфрейм. Ручка отдаёт максимум 300 записей
// и заполняет их ОТ НАЧАЛА окна: длинное окно с interval=day вернёт голову
// периода, а не хвост. Поэтому на недельном графике спрашиваем НЕДЕЛЬНЫЕ
// корзины (замер 06.09: 730 дней → 105 точек, весь период графика).
function _volGranularity(agg, unit) {
  if (unit !== 'day') return 'hour';
  return agg >= 7 ? 'week' : 'day';
}
// Сколько истории влезает в один ответ при этой гранулярности.
function _volWindowSec(gran) {
  if (gran === 'week') return 299 * 7 * 86400;
  if (gran === 'day') return 299 * 86400;
  return 299 * 3600;
}
function _volIntervalParam(gran) { return gran === 'hour' ? '' : '&interval=' + gran; }

// Окно запроса свечей — СКОЛЬКО истории тянем на каждый таймфрейм. Одна
// таблица на весь файл: до 06.09.26 её копий было шесть, и «больше свечей на
// 1W» означало шесть одинаковых правок.
// 1W = 4 года: год давал 53 толстых свечи, кадр выглядел пустым рядом с любой
// биржей (Ник 06.09, пример TradingView). Ручка цен на 1W/4 года отдаёт 209
// свечей по CVX-ETH и 163 по TriCRV — замер того же дня.
const OHLC_HOURS_BACK = { hour: { 1: 7 * 24, 4: 30 * 24 }, day: { 1: 250 * 24, 7: 730 * 24 } };
function _ohlcHoursBack(agg, unit) {
  // Переопределение окна — для рендера картинки по ссылке с заданным периодом
  // (chartshot ставит его в СВОЕЙ странице; на сайте эта переменная не задана).
  const override = Number(typeof window !== "undefined" && window.__ohlcHoursBack);
  if (override > 0) return override;
  const table = OHLC_HOURS_BACK[unit] || OHLC_HOURS_BACK.hour;
  return table[agg] || 90 * 24;
}

// Lite chains are not served by prices.curve.finance at all, so the pool chart
// had zero candles — and the volume histogram is keyed BY candle timestamps, so
// it stayed empty too (Alexandr crvecodev/2372). Build the candles from our own
// trade index instead; the same aggregation the price API does, on our rows.
// Symbols, not addresses: trades.db stores the token tickers.
async function _fetchLiteCandles(pool, mainSym, refSym, aggNum, aggUnit, start, end) {
  if (!mainSym || !refSym || mainSym === refSym) return null;
  const bucket = (aggUnit === 'day' ? 86400 : 3600) * (aggNum || 1);
  const url = `${CDX_API}/ohlc/${pool.address}?chain=${getChainKey()}`
    + `&main=${encodeURIComponent(mainSym)}&ref=${encodeURIComponent(refSym)}`
  ...
  try {
    ...
    return rows.map(d => ({ time: d.time, open: d.open, high: d.high, low: d.low, close: d.close }))
               .filter(_okCandle);
  } catch (e) { return null; }
}

// ЦЕНА ПАЯ (Virtual Price) на вкладке Pools — отдельной вкладкой, как в Yield
// (Alexandr). Свой холст, а не вторая серия на графике цены: у пары и у пая
// разный масштаб и разный формат числа, а шкалу в lightweight-charts задаёт
// ПЕРВАЯ видимая серия — подсадив их на один холст, получаешь чужой формат.
let poolChartMode = 'price';
let poolVpChart = null;
let poolVpSeries = null;

// Ряд цены пая: на полных сетях — снапшоты Curve, на Lite — наш же суточный
// снимок vp_history.json (ценовая ручка Curve эти сети не обслуживает).
async function _fetchPoolVpSeries(pool) {
  const ck = getChainKey();
  const cc = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains && CHAINS_CONFIG.chains[ck];
  if (cc && cc.lite) {
    try {
      ...
      const rows = hist && hist.chains && hist.chains[ck] && hist.chains[ck][(pool.address || '').toLowerCase()];
      if (!Array.isArray(rows)) return [];
      return rows.map(r => ({ time: Math.floor(Date.parse(r[0]) / 1000), value: Number(r[1]) / 1e18 }))
                 .filter(d => isFinite(d.time) && isFinite(d.value) && d.value > 0);
    } catch (e) { return []; }
  }
  try {
    const end = Math.floor(Date.now() / 1000);
    const start = end - 90 * 24 * 3600;
    const resp = await fetch(`${PRICES_BASE}/snapshots/${ck}/${pool.address}?start=${start}&end=${end}`);
    if (!resp.ok) return [];
    const json = await resp.json();
    const out = [];
    for (const snap of (json && json.data) || []) {
      if (snap.virtual_price == null) continue;
      ...
      if (isFinite(ts) && isFinite(vp) && vp > 0) out.push({ time: ts, value: vp });
    }
    const m = new Map();
    out.forEach(d => m.set(d.time, d));
    return [...m.values()].sort((a, b) => a.time - b.time);
  } catch (e) { return []; }
}

async function loadPoolVpChart() {
  const pool = (typeof selectedPool !== 'undefined') ? selectedPool : null;
  const container = document.getElementById('pool-vp-chart-container');
  if (!container) return;
  if (!pool) { container.innerHTML = '<div class="loading-center">Select a pool to view LP token price</div>'; return; }
  const data = await _fetchPoolVpSeries(pool);
  if (poolChartMode !== 'vp') return;              // успели переключить обратно
  if (!data.length) {
    if (poolVpChart) { try { poolVpChart.remove(); } catch (e) {} poolVpChart = null; poolVpSeries = null; }
    container.innerHTML = '<div class="loading-center">No LP token price history for this pool yet</div>';
    return;
  }
  if (!poolVpChart) {
    container.innerHTML = '';
    const isMobile = window.innerWidth <= 768;
    poolVpChart = LightweightCharts.createChart(container, {
      layout: { background: { color: '#0b0e11' }, textColor: '#848e9c', fontSize: isMobile ? 9 : 11 },
      grid: { vertLines: { color: '#1e2329' }, horzLines: { color: '#1e2329' } },
      crosshair: { mode: LightweightCharts.CrosshairMode.Normal },
      // Интерфейс CurveDEX только на английском: без явной локали библиотека
      // берёт язык браузера и подписывает ось «сент.» на английской странице.
      localization: { locale: 'en-US' },
      rightPriceScale: { visible: true, borderColor: '#2b3139', scaleMargins: { top: 0.15, bottom: 0.15 },
                         minimumWidth: isMobile ? 70 : 90, autoScale: true },
      leftPriceScale: { visible: false },
      timeScale: { borderColor: '#2b3139', timeVisible: false, secondsVisible: false },
    });
    poolVpSeries = poolVpChart.addLineSeries({
      color: '#f0b90b', lineWidth: 2, title: '',
      priceFormat: { type: 'custom', formatter: v => v.toFixed(6), minMove: 0.000001 },
    });
    if (window.registerChart) registerChart('pool-vp-chart-container', poolVpChart);
  }
  poolVpSeries.setData(data);
  poolVpChart.timeScale().fitContent();
}

// Переключатель вкладок графика пула. Таймфреймы относятся только к цене пары:
// цена пая снимается раз в сутки, и ряд у неё один.
function setPoolChartMode(mode) {
  poolChartMode = (mode === 'vp') ? 'vp' : 'price';
  document.querySelectorAll('#poolChartTabs .chart-tab').forEach(t => {
    t.classList.toggle('active', t.dataset.poolchart === poolChartMode);
  });
  const tf = document.getElementById('poolTimeSelector');
  const priceC = document.getElementById('trade-chart-container');
  const vpC = document.getElementById('pool-vp-chart-container');
  const tfSep = document.getElementById('poolToolbarSep');
  if (tf) tf.style.display = poolChartMode === 'vp' ? 'none' : '';
  if (tfSep) tfSep.style.display = poolChartMode === 'vp' ? 'none' : '';
  if (priceC) priceC.style.display = poolChartMode === 'vp' ? 'none' : '';
  if (vpC) vpC.style.display = poolChartMode === 'vp' ? '' : 'none';
  if (poolChartMode === 'vp') loadPoolVpChart();
  else if (typeof tradeChart !== 'undefined' && tradeChart) { try { tradeChart.timeScale().fitContent(); } catch (e) {} }
}
window.setPoolChartMode = setPoolChartMode;
window.loadPoolVpChart = loadPoolVpChart;
...
async function loadOHLC() {
  ...
  // Пара графика: запомненная кнопкой ↔️, иначе выбранная в форме свопа.
  const _cpO = _poolCanonPair(pool);
  const fromIdx = _cpO ? _cpO[0] : (selectedFromToken ? selectedFromToken.index : 0);
  const toIdx = _cpO ? _cpO[1] : (selectedToToken ? selectedToToken.index : 1);
  const { main: mainToken, ref: refToken } = poolPriceTokens(pool, fromIdx, toIdx);
  if (!mainToken || !refToken || mainToken === refToken) return;

  const hoursBack = _ohlcHoursBack(currentAgg, currentUnit);
  const start = Math.floor(Date.now() / 1000) - hoursBack * 3600;
  const end = Math.floor(Date.now() / 1000);
  const aggNum = currentAgg;
  const aggUnit = currentUnit;

  // Lite chain: our own index is the only candle source.
  const _ccAllO = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains;
  const _ccHereO = _ccAllO && _ccAllO[getChainKey()];
  if (_ccHereO && _ccHereO.lite) {
    const _addrs = pool.coinsAddresses || [];
    const _idxOf = a => _addrs.findIndex(x => String(x).toLowerCase() === String(a).toLowerCase());
    const _mainSym = (pool.coins || [])[_idxOf(mainToken)];
    const _refSym = (pool.coins || [])[_idxOf(refToken)];
    const liteCandles = await _fetchLiteCandles(pool, _mainSym, _refSym, aggNum, aggUnit, start, end);
    if (liteCandles && liteCandles.length > 0 && candleSeries) {
      window._poolCanonCandles = liteCandles;
      _renderPoolChartFromCandles(liteCandles);
    }
    await loadVolumeFromTrades();
    // A Lite pool has days of history, not months: with the library default the
    // handful of bars lands in a 30px strip at the right edge and the rest of the
    // canvas is empty. Fit to what we actually have.
    try { if (typeof tradeChart !== 'undefined' && tradeChart) tradeChart.timeScale().fitContent(); } catch (e) {}
    return;
  }
...
async function loadVolumeFromTrades() {
  ...
  try {
    ...
    const hoursBackV = _ohlcHoursBack(currentAgg, currentUnit);
...
// Lite chains are absent from prices.curve.finance, so their activity panel
// stayed empty while our own index already held the swaps. Same render shape as
// the upstream branch; liquidity events are not indexed, so swaps only.
async function _fetchLiteTradesPage(pool, page) {
  const url = `${CDX_API}/trades/${pool.address}/list?chain=${getChainKey()}` +
    `&per_page=${TRADES_PAGE_SIZE}&page=${page}`;
  let rows = [];
  try {
    const j = await fetchJSON(url);
    rows = (j && j.data) || [];
  } catch (e) {
    console.warn('[lite-trades] fetch failed:', e && e.message);
    return [];
  }
  const firstSym = (pool.coins && pool.coins[0]) || '';
  return rows.map(r => {
    const isBuy = String(r.token_out || '') === String(firstSym);
    const iso = new Date((r.timestamp || 0) * 1000).toISOString();
    const amtIn = Number(r.amount_in) || 0;
    const amtOut = Number(r.amount_out) || 0;
    return {
      kind: 'swap',
      type: isBuy ? 'Buy' : 'Sell',
      ...
      tx: r.tx_hash,
      who: '',
      price: amtIn > 0 ? (amtOut / amtIn) : 0,
      inSide: [{ v: amtIn, s: r.token_in }],
      outSide: [{ v: amtOut, s: r.token_out }],
...
async function _fetchTradesPage(pool, page) {
  const _ccAll = (typeof CHAINS_CONFIG !== 'undefined') && CHAINS_CONFIG && CHAINS_CONFIG.chains;
  const _ccHere = _ccAll && _ccAll[getChainKey()];
  if (_ccHere && _ccHere.lite) {
    return await _fetchLiteTradesPage(pool, page);
  }
...
// Manual refresh: the feed already polls, but a tester watching a live trade
// wants it NOW and needs to see that the click did something (Alexandr,
// crvecodev/2153). Full reload, not refreshTradesHead — the head path keeps the
// user's lazily loaded tail and stays silent when nothing is new, which reads
// as a dead button.
let _tradesRefreshing = false;
async function refreshTradesNow() {
  if (_tradesRefreshing) return;
  const btn = document.getElementById('tradesRefresh');
  ...
  try {
    ...
    console.warn('[trades] manual refresh failed:', e);
  } finally {
    // Keep the spin visible for at least a beat, otherwise a 90 ms response
    // looks like the click was swallowed.
...
window.refreshTradesNow = refreshTradesNow;
...
function swapDirection() {
  ...
  // Пользователь сам выбрал токены в форме — график снова идёт за ней.
  if (typeof poolPricePairIdx !== 'undefined') poolPricePairIdx = null;
  if (typeof selectedPool !== 'undefined' && selectedPool) {
    // Сбрасывается ПАРА, а не знаменатель пула: его человек выбирал отдельно.
    _savePoolPairPref(selectedPool);
    poolPriceInverted = _pairPrefersInverted(selectedPool, _poolCanonPair(selectedPool));
    renderPoolPairPrices(selectedPool);
    if (typeof _syncPoolPriceInvertBtn === 'function') _syncPoolPriceInvertBtn();
  }
...
function selectTokenFromModal(index) {
  ...
  // Пользователь сам выбрал токен в форме — график снова идёт за ней.
  if (typeof poolPricePairIdx !== 'undefined') poolPricePairIdx = null;
  if (typeof selectedPool !== 'undefined' && selectedPool) {
    // Сбрасывается ПАРА, а не знаменатель пула: его человек выбирал отдельно.
    _savePoolPairPref(selectedPool);
    ...
    return `<div class="pool-item${isActive ? ' active' : ''}" data-addr="${t.address}" onclick="selectTradeTokenFromSidebar('${t.address}')">
      ...
      <div class="pool-item-info">
        <div class="pool-item-name" style="display:flex;align-items:center;gap:5px">
          <img class="token-icon" src="${iconUrl}" alt="" width="20" height="20" loading="lazy" onerror="this.style.visibility='hidden'">
          ${t.symbol}
        </div>
...
// Плотность ряда — доля свечей, внутри которых была хоть одна сделка (high !== low).
// Ниже порога картинка наполовину рисуется переносом последней цены и выглядит
// ступеньками; тогда пара берётся по маршруту свопа, а не по прямому пулу.
const MIN_CHART_CANDLE_DENSITY = 0.5;
// Составной ряд обрезается по самой молодой ноге, поэтому маршрут принимается,
// только если покрывает не меньше этой доли прямого ряда.
const MIN_CHART_ROUTE_COVERAGE = 0.7;
function _candleDensity(candles) {
  if (!Array.isArray(candles) || !candles.length) return 0;
  let moved = 0;
  for (const c of candles) if (c && c.high !== c.low) moved++;
  return moved / candles.length;
}
...
// Маршрут МИНИМУМ ИЗ ДВУХ ног. BFS выше отдаёт кратчайший путь, а у пары с
// прямым пулом он всегда одноногий — то есть ровно тот пул, который мы и
// забраковали по плотности. Здесь прямое замыкание запрещено: путь обязан
// пройти через промежуточный токен, как и сам своп при маршрутизации.
function findIndirectRoute(fromAddr, toAddr) {
  fromAddr = String(fromAddr || '').toLowerCase();
  toAddr = String(toAddr || '').toLowerCase();
  if (!fromAddr || !toAddr || fromAddr === toAddr) return null;
  const adj = _getBfsAdj();
  if (!adj.has(fromAddr) || !adj.has(toAddr)) return null;
  // Ногу выбираем по ОБОРАЧИВАЕМОСТИ (объём за сутки к TVL), а не по глубине:
  // глубокий пул бывает почти не торгуемым, и именно из-за него ряд рубленый.
  // Оборачиваемость лежит в кэше пулов, лишних запросов не стоит.
  const turnover = pl => ((pl && pl.volumeUSD) || 0) / Math.max((pl && pl.tvl) || 0, 1);
  const usable = pl => pl && (pl.tvl || 0) >= MIN_CHART_POOL_TVL;
  const firstLegs = (adj.get(fromAddr) || []).filter(e => e.token !== toAddr && usable(e.pool));
  let best = null, bestScore = -1;
  for (const a of firstLegs) {
    for (const b of (adj.get(a.token) || [])) {
      if (b.token !== toAddr || !usable(b.pool) || b.pool === a.pool) continue;
      const score = Math.min(turnover(a.pool), turnover(b.pool));
      if (score > bestScore) {
        bestScore = score;
        const pools = [a.pool, b.pool];
        pools._bfsTokens = [fromAddr, a.token, toAddr];
        best = pools;
      }
    }
  }
  if (best) return best;
  // Двухногого пути нет — обычный BFS, но с запретом прямого замыкания.
  const MAX_HOPS = 3;
  const visited = new Set([fromAddr]);
  let queue = [{ token: fromAddr, path: [] }];
  for (let depth = 0; depth < MAX_HOPS && queue.length > 0; depth++) {
    const nextQueue = [];
    for (const { token, path } of queue) {
      const sorted = [...(adj.get(token) || [])].sort((x, y) => (y.pool.tvl || 0) - (x.pool.tvl || 0));
      for (const edge of sorted) {
        if (edge.token === toAddr) {
          if (path.length === 0) continue; // одноногий путь = забракованный пул
          const pools = [...path.map(x => x.pool), edge.pool];
          pools._bfsTokens = [fromAddr, ...path.map(x => x.viaToken), toAddr];
          return pools;
        }
        if (!visited.has(edge.token)) {
          visited.add(edge.token);
...
function initTradePairChart() {
  ...
  // Кнопка копирования графика берёт график и свечи отсюда (chart_share.js).
  if (window.registerChart) registerChart('trade-pair-chart-container', tradePairChart, tradePairCandleSeries);
...
async function _poolLastHourClose(pool, mainAddr, refAddr) {
  ...
  try {
    ...
    // ПУСТОЙ ОТВЕТ И ОТКАЗ ПРОБЫ — РАЗНОЕ. 200 с нулём строк это ЗАМЕР («у пула
    // свечей нет»), 429/таймаут — отказ. Склеенные в один null, они привели к
    // тому, что мёртвый пул обходил живой (03.09, CRV/WETH $0 против TriCRV).
    // Плотность (доля корзин, где цена реально двигалась) считается по уже
    // полученному ответу и лишнего запроса не стоит. Нужна ниже: у застывшего
    // пула цена ТОЧНЕЕ живой (она не шевелится), и запасной выбор по точности
    // отдавал график мёртвому пулу.
    const moved = d.filter(k => k.high !== k.low).length;
    return d.length ? { close: d[d.length - 1].close, empty: false,
                        dens: moved / d.length }
                    : { close: null, empty: true, dens: 0 };
  } catch (e) { return { close: null, empty: false }; }
}
...
async function _pickChartPoolUncached(fromTok, toTok) {
  ...
  const probes = await Promise.all(cands.slice(0, 3).map(async p => {
    ...
    const r = await _poolLastHourClose(p, a.main, a.ref);
    return { p: p, close: r.close, empty: r.empty, dens: r.dens || 0 };
  }));
  let unproven = null;
  const missed = [];
  for (const pr of probes) {
    if (pr.skip) continue;
    // Пул, у которого свечей НЕТ, дисквалифицирован: он не «непроверенный»,
    // он измеренный и пустой. Рисовать по нему нечего.
    ...
    missed.push({ p: pr.p, dev: dev, dens: pr.dens });
  }
  // НИ ОДИН не прошёл допуск — значит расхождение не про пул, а про саму
  // внешнюю цену (13.09, cvxCRV/CRV: usd-цена cvxCRV расходится с ценой в
  // пулах на ~9%, мимо допуска пролетели все три). Выбирать тут по МИНИМУМУ
  // отклонения нельзя: точность — не то свойство, которое осталось измеримым.
  // Застывший пул выигрывает такое сравнение просто потому, что не шевелится:
  // график пары уехал на пул с TVL $113, где 0 сделок за месяц, — 180 свечей
  // одной ценой. Поэтому среди непрошедших берём САМЫЙ ТОРГУЕМЫЙ (плотность
  // ряда), а при равной плотности — самый глубокий, они идут по TVL.
  const fallback = missed.length
    ? missed.slice().sort((x, y) => y.dens - x.dens)[0].p
    : null;
  const notEmpty = cands.find(c => !probes.some(pr => pr.p === c && pr.empty));
  return unproven || fallback || notEmpty || cands[0];
}
...
async function _loadProxyPoolOHLC(pool) {
  ...
  const hoursBack = _ohlcHoursBack(tradePairAgg, tradePairUnit);
  const start = Math.floor(Date.now() / 1000) - hoursBack * 3600;
  const end = Math.floor(Date.now() / 1000);
...
async function loadTradePairOHLC() {
  ...
  // Прямой пул бывает глубоким и почти не торгуемым (08.09: Yield Basis WBTC,
  // TVL $66.5M — 29 свечей со сделками из 181 на 4H). Вердикт по плотности
  // хранится на паре и на таймфрейм: 'route' — рисуем маршрут, 'direct' —
  // маршрут уже проиграл сравнение и повторно не пробуется.
  const _pathPickKey = String(tradePairAgg) + String(tradePairUnit);
  const _pathPick = selectedPair._chartPathPick;
  const _preferRoute = !!(_pathPick && _pathPick.key === _pathPickKey && _pathPick.mode === 'route');
  const _directChartPool = _preferRoute
    ? null
    : await _pickChartPool(selectedPair.baseAddr, selectedPair.quoteAddr);
  // У пары БЕЗ прямого пула путь графика приходил от роутера сделки, а он
  // ищется кратчайшим и ранжирует рёбра по TVL. Объём составной пары считается
  // МИНИМУМОМ по ногам, поэтому одна неторгуемая нога стирает бары у всей пары:
  // 08.09 CRV/WBTC шёл CRV->crvUSD->WBTC, вторая нога Yield Basis WBTC ($66.6M,
  // 12 часовых корзин с объёмом против 300 у TriCRV) — 73 бара объёма из 181.
  // Поэтому сравниваем маршрут роутера с маршрутом ПО ТОРГУЕМОСТИ и берём тот,
  // у кого УЗКОЕ МЕСТО шире: узкое место пути и есть то, что рисуется.
  // Оборачиваемость лежит в кэше пулов, лишнего запроса не стоит.
  if (selectedPair._multiRoute && !_directChartPool && !_preferRoute) {
    const _bottleneck = r => (Array.isArray(r) && r.length)
      ? Math.min(...r.map(pl => ((pl && pl.volumeUSD) || 0) / Math.max((pl && pl.tvl) || 0, 1)))
      : -1;
    const _liquid = findIndirectRoute(selectedPair.baseAddr, selectedPair.quoteAddr);
    if (_liquid && _liquid.length >= 2
        && _bottleneck(_liquid) > _bottleneck(selectedPair._multiRoute)) {
      console.log('OHLC: путь графика по торгуемости '
        + _liquid.map(pl => pl.name).join(' -> ') + ' (узкое место '
        + _bottleneck(_liquid).toFixed(4) + ') вместо маршрута сделки '
        + selectedPair._multiRoute.map(pl => pl.name).join(' -> ') + ' ('
        + _bottleneck(selectedPair._multiRoute).toFixed(4) + ')');
      selectedPair._multiRoute = _liquid;
      selectedPair._chartTokens = null;
    }
  }
  ...
  if (selectedPair._multiRoute && !_directChartPool) {
    ...
    if (!midFailed && tokens.length >= 2) {
      ...
      if (!urlBuildFailed) {
        ...
        const _synCacheKey = `syn_${route.map(p=>p.address).join('_')}_${tradePairAgg}_${tradePairUnit}_${hoursBack}`;
        // Пока висит несравнённый прямой ряд, кэш маршрута не берём: иначе
        // сравнение «плотнее ли маршрут» будет пропущено и картинка уедет на
        // маршрут без проверки.
        const _synCached = selectedPair._flatDirect ? null : _getOhlcCached(_synCacheKey);
        if (_synCached) {
        ...
        try {
          ...
          if (candles.length > 0) {
            // На маршрут мы могли уйти из-за разреженности прямого ряда. Берём
            // маршрут, только если он И плотнее, И не короче прямого: составной
            // ряд режется по самой молодой ноге (16.08: 13 недельных свечей
            // вместо 53).
            const _flatDirect = selectedPair._flatDirect;
            if (_flatDirect) {
              selectedPair._flatDirect = null;
              const _synDensity = _candleDensity(candles);
              // СВЕЖЕСТЬ — отдельное условие, не следствие плотности: составной
              // ряд обрывается на самой молодой ноге и может кончаться днями
              // раньше прямого (08.09: cbBTC/crvUSD через WBTC кончался 03.09
              // при живом прямом ряде). Отставший ряд хуже разреженного.
              const _barSec = (tradePairUnit === 'day' ? 86400 : tradePairUnit === 'minute' ? 60 : 3600)
                            * (Number(tradePairAgg) || 1);
              const _synLast = candles[candles.length - 1].time;
              const _synFresh = !(_flatDirect.last > 0) || _synLast >= _flatDirect.last - _barSec;
              if (!(_synDensity > _flatDirect.density
                    && candles.length >= _flatDirect.count * MIN_CHART_ROUTE_COVERAGE
                    && _synFresh)) {
                console.log('Chart: маршрут не лучше прямого пула ('
                  + Math.round(_synDensity * 100) + '% / ' + candles.length + ' свечей, отставание '
                  + Math.round(((_flatDirect.last || _synLast) - _synLast) / 3600) + ' ч против '
                  + Math.round(_flatDirect.density * 100) + '% / ' + _flatDirect.count
                  + ') — остаёмся на прямом');
                selectedPair._chartPathPick = { key: _flatDirect.key, mode: 'direct' };
                return loadTradePairOHLC();
              }
            }
    ...
    // Ушли на маршрут из-за разреженности прямого ряда, а маршрут не собрался —
    // возвращаемся к прямому пулу, а не к приблизительному proxy-графику.
    if (selectedPair._flatDirect) {
      selectedPair._chartPathPick = { key: selectedPair._flatDirect.key, mode: 'direct' };
      selectedPair._flatDirect = null;
      return loadTradePairOHLC();
    }
  ...
  const url = `${PRICES_BASE}/ohlc/${getChainKey()}/${pool.address}?main_token=${mainToken}&reference_token=${refToken}&agg_number=${tradePairAgg}&agg_units=${tradePairUnit}&start=${start}&end=${end}`;
  const _directCacheKey = `${pool.address}_${mainToken}_${refToken}_${tradePairAgg}_${tradePairUnit}_${hoursBack}`;
  ...
  try {
    ...
    // Прямой пул глубокий, но почти не торгуется -> ряд рисуется переносом
    // последней цены, на экране ступеньки, и свопу это не соответствует: он
    // идёт маршрутом через живые пулы. Плотность — ИЗМЕРЕНИЕ по уже полученным
    // свечам, лишнего запроса не стоит.
    const _densityKey = String(tradePairAgg) + String(tradePairUnit);
    const _directDensity = _candleDensity(candles);
    if (_directDensity < MIN_CHART_CANDLE_DENSITY
        && !(selectedPair._chartPathPick && selectedPair._chartPathPick.key === _densityKey)) {
      // Путь свопа, если он уже известен от роутера; иначе строим обходной.
      const _densRoute = (Array.isArray(selectedPair._multiRoute) && selectedPair._multiRoute.length >= 2)
        ? selectedPair._multiRoute
        : findIndirectRoute(selectedPair.baseAddr, selectedPair.quoteAddr);
      if (_densRoute && _densRoute.length >= 2) {
        selectedPair._multiRoute = _densRoute;
        selectedPair._chartPathPick = { key: _densityKey, mode: 'route' };
        selectedPair._flatDirect = { density: _directDensity, count: candles.length, key: _densityKey,
          last: candles[candles.length - 1].time };
        console.log('OHLC: прямой пул ' + (pool.name || pool.address) + ' разрежен ('
          + Math.round(_directDensity * 100) + '% свечей со сделками) — пробуем маршрут '
          + _densRoute.map(p => p.name).join(' -> '));
        return loadTradePairOHLC();
      }
    }
...
async function loadTradePairVolume() {
  ...
  // === ПРЯМАЯ ПАРА НА ЧАСОВЫХ ТФ: почасовые корзины страницами назад ===
  // Суточный снапшот ниже даёт ОДНО значение на день, размазанное на шесть
  // столбиков 4H, — ранняя часть кадра выходила ровными площадками (жалоба
  // Ника 11.09 по составной паре; у прямых так весь кадр). Ручка /volume
  // отдаёт 300 записей ОТ start, поэтому идём страницами НАЗАД, пока не
  // закроем окно свечей. Ряд вышел дырявым — молча уходим в прежние пути.
  // Условие — ПУЛ ПАРЫ СОДЕРЖИТ ОБА ТОКЕНА, а не «маршрут сделки прямой»:
  // у ETH/USDC свечи рисуются пулом TricryptoUSDC, а маршрут сделки идёт через
  // два хопа, и объём, спрошенный по маршруту, приходил пустым (замер 11.09:
  // на 1H и 4H ни одного столбика). Объём берём из того же пула, что и свечи.
  if (baseIdx >= 0 && quoteIdx >= 0
      && _volGranularity(tradePairAgg, tradePairUnit) === 'hour') {
    try {
      const cts = tradePairLastCandles || [];
      if (cts.length > 1) {
        const endHD = Math.floor(Date.now() / 1000);
        const mainTD = pool.coinsAddresses[baseIdx];
        const refTD = pool.coinsAddresses[quoteIdx];
        const _oneD = async (mm, rr, st, en) => {
          try {
            const j = await fetchJSON(PRICES_BASE + '/volume/' + getChainKey() + '/' + pool.address
              + '?main_token=' + mm + '&reference_token=' + rr + '&start=' + st + '&end=' + en);
            const m = new Map();
            let sum = 0;
            for (const d of ((j && j.data) || [])) {
        ...
        const st0 = Math.max(cts[0] - 3600, endHD - 299 * 3600);
        const bothD = await Promise.all([_oneD(mainTD, refTD, st0, endHD), _oneD(refTD, mainTD, st0, endHD)]);
        const bestD = bothD.filter(Boolean).sort((x, y) => y.sum - x.sum)[0];
        if (bestD) {
          // Порядок токенов в ручке не симметричен — страницы спрашиваем тем же.
          const _firstD = (bothD[0] === bestD);
          const bMD = _firstD ? mainTD : refTD;
          const bRD = _firstD ? refTD : mainTD;
          let coveredD = Math.min(...bestD.m.keys());
          for (let g = 0; g < 6 && coveredD - 3600 > cts[0]; g++) {
            const page = await _oneD(bMD, bRD, Math.max(cts[0] - 3600, coveredD - 299 * 3600), coveredD - 1);
            if (!page || !page.m.size) break;
            for (const e of page.m) if (!bestD.m.has(e[0])) bestD.m.set(e[0], e[1]);
            const got = Math.min(...bestD.m.keys());
            if (got >= coveredD) break;
            coveredD = got;
          }
          const _bucketD = ts => {
            let b = cts[0];
            for (let i = cts.length - 1; i >= 0; i--) { if (cts[i] <= ts) { b = cts[i]; break; } }
            return b;
          };
          const aggD = new Map();
          for (const e of bestD.m) { const b = _bucketD(e[0]); aggD.set(b, (aggD.get(b) || 0) + e[1]); }
          const volDataD = [];
          for (const b of cts) { const v = aggD.get(b) || 0; if (v > 0) volDataD.push({ time: b, value: v, color: 'rgba(14,203,129,0.5)' }); }
          if (tradePairVolumeSeries && volDataD.length > cts.length / 2) {
            tradePairVolumeSeries.setData(_sqrtNormalizeVol(volDataD.slice()));
            return;
          }
        }
      }
    } catch (e) { /* прежние пути ниже */ }
  }
  ...
  if (isMultiHop) {
    ...
    // МИНИМУМ по корзине — то же бутылочное горлышко, что и в суточном пути.
    {
      try {
        ...
        const _volGran = _volGranularity(tradePairAgg, tradePairUnit);
        const _volIntervalDay = (_volGran !== 'hour');
        ...
        if (candleTs.length > 1) {
          ...
          const startH = Math.max(candleTs[0] - 86400, endH - _volWindowSec(_volGran));
          const perHop = await Promise.all(hopRoute.map(async (hop, i) => {
            ...
            const _volFetch = async (mm, rr, asDay, stOv, enOv) => {
              // asDay — добор раннего хвоста суточными корзинами на часовых ТФ.
              // stOv/enOv — СВОЁ окно: ручка отдаёт 300 записей ОТ start, поэтому
              // ранние страницы берутся отдельными запросами назад по времени.
              const gran = asDay ? 'day' : _volGran;
              const st = stOv || Math.max(candleTs[0] - 86400, endH - _volWindowSec(gran));
              const en = enOv || endH;
              try {
                const j = await fetchJSON(PRICES_BASE + '/volume/' + getChainKey() + '/' + volPool.address
                  + '?main_token=' + mm + '&reference_token=' + rr
                  + '&start=' + st + '&end=' + en
                  + _volIntervalParam(gran));
                const m = new Map();
                let sum = 0;
                for (const d of ((j && j.data) || [])) {
                  const ts = Number(d && d.timestamp) || 0;
            ...
            if (!best) return null;
            // Порядок токенов, выигравший по объёму: ранние страницы спрашиваем
            // ИМ ЖЕ, иначе хвост приедет с другой стороны и в тысячу раз ниже.
            const _bestFirst = (both[0] === best);
            const bMain = _bestFirst ? volMain : volRef;
            const bRef = _bestFirst ? volRef : volMain;
            // Часовой ответ покрывает лишь ~12.5 суток ОТ своего start. Ранний
            // хвост добираем ПОЧАСОВЫМИ страницами назад — та же ручка на своём
            // окне отдаёт часы за любой период (замер 11.09: 11-24.08 по CVX-ETH
            // приходят). Суточное размазывание ниже остаётся запасным путём: на
            // нём ранняя половина кадра выходит ровными площадками по шесть
            // одинаковых столбиков (жалоба Ника 11.09 по CVX/CRV, граница 30.08).
            let covered = candleTs.length ? Math.min(...best.m.keys()) : 0;
            if (!_volIntervalDay && candleTs.length) {
              for (let guard = 0; guard < 6 && covered - 3600 > candleTs[0]; guard++) {
                const stPage = Math.max(candleTs[0] - 3600, covered - 299 * 3600);
                const page = await _volFetch(bMain, bRef, false, stPage, covered - 1);
                if (!page || !page.m.size) break;
                for (const e of page.m) if (!best.m.has(e[0])) best.m.set(e[0], e[1]);
                const got = Math.min(...best.m.keys());
                if (got >= covered) break;
                covered = got;
              }
              if (covered - 3600 > candleTs[0]) {
                // ОБА порядка и больший из них — та же асимметрия, что у часового
                // пути двадцатью строками выше. Первый ненулевой брать нельзя:
                // на CVX-ETH стороны различаются в тысячу раз, и ранний хвост
                // рисовался в 15 раз ниже свежих столбиков (замер 07.09).
                const dayBoth = await Promise.all([
                  _volFetch(volMain, volRef, true), _volFetch(volRef, volMain, true)]);
                const dayBest = dayBoth.filter(x => x && x.sum > 0).sort((x, y) => y.sum - x.sum)[0];
                const dm = dayBest ? dayBest.m : null;
  ...
  try {
    const hoursBackTP = _ohlcHoursBack(tradePairAgg, tradePairUnit);
    ...
    if (volRespTP.status === 404 || volRespTP.status === 405) {
      ...
      try {
        ...
        if (_cts.length && _covered - 3600 > _cts[0]) {
          const _gran = _volGranularity(tradePairAgg, tradePairUnit);
          const _granFill = _gran === 'hour' ? 'day' : _gran;
          const _st = Math.max(_cts[0] - 86400, endTP - _volWindowSec(_granFill));
          const _one = async (mm, rr) => {
            const r = await fetch(`${PRICES_BASE}/volume/${getChainKey()}/${pool.address}?main_token=${mm}&reference_token=${rr}&start=${_st}&end=${endTP}${_volIntervalParam(_granFill)}`);
            if (!r.ok) return null;
            const j = await r.json();
            return j && j.data ? j.data : null;
          };
          // Больший из двух порядков, а не первый ненулевой: см. выше.
          const _dayBoth = await Promise.all([
            _one(mainToken, refToken), _one(refToken, mainToken)]);
          const _day = _dayBoth.sort((a, b) => _sum(b) - _sum(a))[0];
          const _extra = [];
          for (const d of (_day || [])) {
...
async function updateTradePairUsd() {
  const apply = async (amtId, tok, elId) => {
    ...
    if (price > 0) {
      const v = amt * price;
      // Деньги — всегда два знака: без minimumFractionDigits хвостовой ноль
      // пропадает и рядом с ценой из двух знаков это читается как обрубок.
      el.textContent = '\u2248 $' + (v >= 1
...
function renderTradeFavorites() {
  ...
  list.innerHTML = items.map(p => {
    const tvlText = p.tvl >= 1e6 ? '$' + (p.tvl / 1e6).toFixed(1) + 'M'
                  ...
                  : '$' + (p.tvl || 0).toFixed(0);
    const baseIcon = p.baseAddr ? `<img src="${_tokenIconUrl(p.baseAddr)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">` : '';
    const quoteIcon = p.quoteAddr ? `<img src="${_tokenIconUrl(p.quoteAddr)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">` : '';
    const isActive = p.name === selectedKey;
...
async function _tpVaultLegTag(from, to) {
  ...
  if ((await asset(t)) === f) return 'v' + t.slice(2, 12) + '-0';   // base -> wrapper
  if ((await asset(f)) === t) return 'v' + f.slice(2, 12) + '-1';   // wrapper -> base
  return null;
}

// _alertPoolPrefix: the shortest address prefix that still names this pool
// uniquely among the ones the page knows. The deep-link payload caps at 64
// chars and 12 hex per leg overflows it on a three-leg route (Nik 18.09,
// "cannot set an alert"); the bot resolves a prefix by startswith, so a shorter
// one costs nothing as long as it is unique. Uniqueness is MEASURED against
// allPools, not assumed - a guessed length is how two pools share one link.
function _alertPoolPrefix(addr) {
  const a = String(addr || '').toLowerCase().replace(/^0x/, '');
  const list = (typeof allPools !== 'undefined' && Array.isArray(allPools)) ? allPools : null;
  if (!list || !list.length) return a.slice(0, 12);
  for (let n = 6; n <= 12; n += 2) {
    const p = a.slice(0, n);
    let hits = 0;
    for (const q of list) {
      const b = String(q.address || '').toLowerCase().replace(/^0x/, '');
      if (b.startsWith(p)) { hits++; if (hits > 1) break; }
    }
    if (hits <= 1) return p;
  }
  return a.slice(0, 12);
...
async function openTradePairAlert() {
  ...
  const hops = [];
  let route = pair._multiRoute;
  // МАРШРУТ СДЕЛКИ — НЕ ЕДИНСТВЕННЫЙ ИСТОЧНИК ПУТИ. Роутер отваливается по
  // своему таймауту и мигает: в этот момент `_multiRoute` пуст, и колокольчик
  // отвечал «Alerts are not available for this pair yet», хотя ГРАФИК рядом
  // рисовал ту же пару составной и путь знал (замер 24.08 на CVX/CRV: убираю
  // `_multiRoute` — получаю ровно этот отказ при живых `_chartTokens`).
  // Берём путь графика: те же пулы, что дают свечи, — значит алерт следит
  // ровно за той ценой, которую человек видит на экране.
  if (!(Array.isArray(route) && route.length >= 2 && route._bfsTokens
        && route._bfsTokens.length === route.length + 1)
      && Array.isArray(pair._chartTokens) && pair._chartTokens.length >= 3) {
    const toks = pair._chartTokens.map(t => String(t || '').toLowerCase());
    const legs = [];
    for (let i = 0; i < toks.length - 1; i++) {
      const pl = await _pickChartPool(toks[i], toks[i + 1]);
      if (!pl || !pl.address) { legs.length = 0; break; }
      legs.push(pl);
    }
    if (legs.length === toks.length - 1) {
      legs._bfsTokens = toks;
      route = legs;
    }
  }
  if (Array.isArray(route) && route.length >= 2 && route._bfsTokens
    ...
    for (let i = 0; i < route.length; i++) {
      ...
      hops.push(`${_alertPoolPrefix(route[i].address)}-${main}-${ref}`);
    }
  ...
  // Telegram's start= payload caps at 64 chars. The cap is on the LINK, not on the
  // number of legs, so it is measured here: side (2) + level (up to 14) + inversion
  // (2) + repeat mode (2) + network suffix, on top of whatever the legs came out to.
  const _alertChain = (typeof activeChainKey !== 'undefined' && activeChainKey) ? activeChainKey : 'ethereum';
  const _alertNetLen = _alertChain === 'ethereum' ? 0 : 2 + _alertChain.replace(/[^a-z]/g, '').length;
  const _alertLen = 2 + hops.join('_').length + 2 + 14 + 2 + 2 + _alertNetLen;
  if (_alertLen > 64) { alert('This route is too long for an alert link.'); return; }
...
function openPoolPairAlert() {
  ...
  const _cpA = _poolCanonPair(pool);
  const mainIdx = _cpA ? _cpA[1] : ((typeof selectedToToken !== 'undefined' && selectedToToken) ? selectedToToken.index : 1);
  const refIdx = _cpA ? _cpA[0] : ((typeof selectedFromToken !== 'undefined' && selectedFromToken) ? selectedFromToken.index : 0);
...
// __autoOpenAlert: ссылка вида ...?alert=1#/trade/PAIR открывает окно подписки
// сразу. Нужна кнопке «🔔 Price alert» под графиком в Telegram (Ник 08.09):
// без неё она вела туда же, куда «Open on llama.box», и смысла не имела.
// Флаг стоит в query, а не в хэше: хэш разбирает маршрутизатор пары.
(function __autoOpenAlert() {
  var want = false;
  try { want = new URLSearchParams(location.search).get('alert') === '1'; } catch (e) { return; }
  if (!want) return;
  var tries = 0;
  var t = setInterval(function () {
    tries++;
    var ready = (typeof selectedPair !== 'undefined') && selectedPair
      && Array.isArray(window._tradeRichLastCandles) && window._tradeRichLastCandles.length > 0;
    if (ready && typeof openTradePairAlert === 'function') {
      clearInterval(t);
      openTradePairAlert();
    } else if (tries > 120) {          // минута ожидания и сдаёмся молча
      clearInterval(t);
    }
  }, 500);
})();
