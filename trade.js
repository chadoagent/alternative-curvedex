...
function toggleTradePairDirection() {
  ...
  if (canon && canon.length > 0 && typeof tradePairCandleSeries !== 'undefined' && tradePairCandleSeries) {
    ...
    try { tradePairCandleSeries.setData(canon); } catch {}
...
async function loadVolumeFromTrades() {
  ...
  try {
    let cdxDaily = [];
    const _addrLC = (pool.address || '').toLowerCase();
  ...
  try {
    ...
    if (!isETH) {
      ...
      if (allowance < dx) {
        ...
        const approveData = token.interface.encodeFunctionData('approve', [selectedPool.address, ethers.MaxUint256]);
      ...
      if (snapPools) {
        ...
        if (allHopsCovered && hopDayMaps.length >= 2) {
          ...
          try {
            ...
            let topTvl = -1;
            for (let i = 0; i < route.length; i++) {
              const t = Number(route[i] && route[i].tvl) || 0;
              if (t > topTvl) { topTvl = t; topHopIdx = i; }
            }
            const topHop = route[topHopIdx];
            let mainT, refT;
          ...
          try {
            ...
            if (topPool && topPool.address && !_curvePricesVolume404.has(_addrLCP)) {
              ...
              if (bi >= 0 && qi >= 0) {
                ...
                const startL = Math.floor(Date.now() / 1000) - hoursBack * 3600;
                const endL = Math.floor(Date.now() / 1000);
                const liveUrl = `${PRICES_BASE}/volume/${getChainKey()}/${topPool.address}?main_token=${mainT}&reference_token=${refT}&start=${startL}&end=${endL}`;
                const resp = await fetch(liveUrl);
                if (resp.status === 404 || resp.status === 405) {
