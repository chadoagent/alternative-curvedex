...
(function (global) {
  ...
  // СУММА ИЗ ПОЛЯ — СТРОКОЙ, БЕЗ float (тот же разбор, что в yield.js).
  // `parseFloat(x).toFixed(8)` ОКРУГЛЯЕТ вверх: на «весь баланс» просим больше,
  // чем есть, и транзакция падает уже в кошельке; у 18-знакового токена срезается хвост.
  function _parseAmountStrict(str, decimals, lib) {
    const E = lib || (typeof ethers !== 'undefined' ? ethers : (global && global.ethers));
    if (!E) return 0n;
    const t = String(str == null ? '' : str).trim().replace(',', '.');
    if (!t || !/^[0-9]*\.?[0-9]*$/.test(t)) return 0n;
    const dot = t.indexOf('.');
    const dec = Number(decimals) || 18;
    const whole = dot < 0 ? t : t.slice(0, dot);
    const frac = dot < 0 ? '' : t.slice(dot + 1, dot + 1 + dec);
    try { return E.parseUnits((whole || '0') + (frac ? '.' + frac : ''), dec); } catch (e) { return 0n; }
  }
  ...
  // Dust filter (display-only): a position is "dust" when it has no claimable CRV
  // AND its remaining LP is worth less than the gas you'd burn cleaning it up.
  // Threshold = (gas units for claim+withdraw) × current gas price × ETH/USD × safety.
  // Reuses globals exposed by trade.js (_michwillGasPricePreview returns wei/gas BigInt,
  // _getEthUsdPrice returns float). 60s cache because gas/eth prices drift slowly.
  // Toggle persisted in localStorage so user can opt in to "show all".
  const DUST_TOGGLE_KEY = 'curvedex_portfolio_hide_dust';
  const DUST_TTL_MS = 60_000;
  const DUST_GAS_UNITS = 150_000n; // claim_rewards + withdraw_one_coin upper bound
  const DUST_SAFETY_MULTIPLIER = 2;
  let _dustThresholdUsd = 0; // 0 = filter inactive (no data yet)
  let _dustThresholdTs = 0;

  function _isHideDust() {
    try {
      const v = localStorage.getItem(DUST_TOGGLE_KEY);
      return v === null ? true : v === '1';
    } catch (e) { return true; }
  }
  function _setHideDust(hide) {
    try { localStorage.setItem(DUST_TOGGLE_KEY, hide ? '1' : '0'); } catch (e) {}
  }
  async function _refreshDustThresholdUsd() {
    if (_dustThresholdUsd > 0 && Date.now() - _dustThresholdTs < DUST_TTL_MS) return _dustThresholdUsd;
    if (typeof window._michwillGasPricePreview !== 'function' || typeof window._getEthUsdPrice !== 'function') {
      return _dustThresholdUsd;
    }
    try {
      const [gasPrice, ethUsd] = await Promise.all([
        window._michwillGasPricePreview(),
        window._getEthUsdPrice(),
      ]);
      if (!gasPrice || gasPrice === 0n || !ethUsd || !isFinite(ethUsd) || ethUsd <= 0) return _dustThresholdUsd;
      const gasCostWei = gasPrice * DUST_GAS_UNITS;
      const gasCostEth = Number(gasCostWei) / 1e18;
      _dustThresholdUsd = gasCostEth * ethUsd * DUST_SAFETY_MULTIPLIER;
      _dustThresholdTs = Date.now();
    } catch (e) { /* keep last known value */ }
    return _dustThresholdUsd;
  }
  // Partition positions by dust filter. Returns { visible, hiddenCount }.
  // pendingCRV > 0 always wins (claimable amount is the actionable value).
  function _partitionDust(positions) {
    if (!_isHideDust() || _dustThresholdUsd <= 0) return { visible: positions, hiddenCount: 0 };
    const visible = [];
    let hidden = 0;
    for (const p of positions) {
      const claimable = (p.pendingCRV || 0) > 0 || (p.convex && (p.convex.pendingCRV || 0) > 0);
      if (claimable || (p.usdValue || 0) >= _dustThresholdUsd) {
        visible.push(p);
      } else {
        hidden++;
      }
  ...
  function _toggleHideDust() {
    _setHideDust(!_isHideDust());
    if (!_lastPositions || !_lastPositions.length) return;
    const view = document.getElementById('portfolio-content') || document.getElementById('view-portfolio');
    if (view) _renderInto(view, _lastPositions, { fromToggle: true });
  }
  ...
  function _decodeBatch(poolsBatch, results, lpIface, gaugeIface, out) {
    for (let i = 0; i < poolsBatch.length; i++) {
      ...
      if (walletBn === 0n && stakedBn === 0n && pendingBn === 0n) continue;
      ...
      // Per-LP price from a single snapshot (see lpUsdPrice in app.js). The
      // on-chain totalSupply read above is kept for display, NOT for pricing:
      // pairing it with the cached TVL is exactly what understated positions.
      let perLp = global.lpUsdPrice ? global.lpUsdPrice(pool) : 0;
      if (!(perLp > 0) && pool.tvl > 0 && totalSupplyLP > 0) perLp = pool.tvl / totalSupplyLP;
      const usdValue = totalLP * perLp;
      ...
      out.push({
        ...
        walletWei: walletBn, stakedWei: stakedBn,
        usdValue,
  ...
  async function loadPortfolioPositions(walletAddr, opts) {
    ...
    // allSettled: one worker rejecting (e.g. all 3 capped RPC retries failed for a
    // batch) should not nuke the entire portfolio render — other workers' writes
    // to allPositions stay. Partial render > blank spinner.
  ...
  async function loadCrossPlatformPositions(walletAddr) {
    ...
    if (uncachedPids.length > 0) {
      ...
      // allSettled: workers write into _convexPidToBRPCache; one rejection
      // shouldn't drop pidInfo for the rest. Missing pids fall through to
      // shutdown/empty filter below.
      await Promise.allSettled(piWorkers);
    }
    ...
    const cvxBalSum = new Array(cvxLive.length).fill(0n);
    {
      ...
      // allSettled: cvxBalSum accumulator survives partial worker failure.
    ...
    if (cvxNonZero.length > 0) {
      ...
      // allSettled: items[].pendingCRV defaults to 0n if worker rejects.
      await Promise.allSettled(ws);
    }
    ...
    if (sdEntries.length > 0) {
      ...
      // allSettled: sdBalSum accumulator survives partial worker failure.
      await Promise.allSettled(ws);
      for (let ei = 0; ei < sdEntries.length; ei++) {
  ...
  function _coinIconsHTML(coinAddrs) {
    ...
    return `<div class="pc-coin-icons">${coinAddrs.slice(0, 4).map(a =>
      `<img src="${global._tokenIconUrl(a)}" onerror="this.style.visibility='hidden'" alt="">`
    ).join('')}</div>`;
  }
  ...
  function _renderStatBar(positions, dustInfo) {
    // Stats are computed on the FULL position set (totalUsd, pending, etc.) so the
    // numbers don't change when user toggles the dust filter — the toggle only
    // affects which cards are listed below.
    ...
    const hideDust = _isHideDust();
    const hiddenCount = (dustInfo && dustInfo.hiddenCount) || 0;
    // Toggle is only rendered when filter actually has effect (threshold known +
    // there's at least one position that would be hidden or shown by flipping it).
    // Keeps UI clean for users who never have dust.
    const showToggle = _dustThresholdUsd > 0 && (hiddenCount > 0 || !hideDust);
    const toggleLabel = hideDust
      ? `Show dust${hiddenCount > 0 ? ` (${hiddenCount})` : ''}`
      : 'Hide dust';
    const toggleTitle = `Positions below ~${global.fmt$(_dustThresholdUsd)} (≈2× gas to claim+withdraw at current network price) are hidden by default.`;
    const toggleBtn = showToggle
      ? `<button class="ps-dust-toggle" onclick="Portfolio._toggleHideDust()" title="${toggleTitle}">${toggleLabel}</button>`
      : '';
  ...
  function _renderInto(container, positions, opts) {
    ...
    if (positions.length === 0) {
      container.innerHTML = banner + _renderStatBar([], null) + _emptyHTML();
      return;
    }
    // Refresh threshold opportunistically; first render after page load may run
    // without threshold (filter inactive), follow-up renders use cached value.
    // _partitionDust handles the "no threshold yet" case by returning all positions.
    const part = _partitionDust(positions);
    const cards = part.visible.length
      ? part.visible.map((p, i) => _renderPositionCard(p, i)).join('')
      : '';
    const gridBlock = part.visible.length
      ? `<div class="position-grid">${cards}</div>`
      : `<div class="portfolio-empty"><h3>All positions hidden by dust filter</h3><p>${part.hiddenCount} position(s) below threshold — click "Show dust" to reveal.</p></div>`;
    container.innerHTML = banner + _renderStatBar(positions, part) + gridBlock;
    // Kick off threshold refresh in background. If value updates and changes
    // visibility, re-render so user sees the filter take effect without manual reload.
    if (!opts.fromToggle) {
      _refreshDustThresholdUsd().then(newThreshold => {
        if (!newThreshold || newThreshold <= 0) return;
        const repart = _partitionDust(positions);
        if (repart.visible.length !== part.visible.length || repart.hiddenCount !== part.hiddenCount) {
          if (document.body.contains(container)) {
            _renderInto(container, positions, { ...opts, fromToggle: true });
          }
        }
      });
    }
  ...
  function _mergeCrossPlatform(positions, cross) {
    ...
    function _attachCvx(target, entry) {
      ...
      if (target.totalSupplyLP > 0 && target.usdValue > 0 && target.totalLP > 0) {
        ...
        // No Curve row to derive from — price the LP the one canonical way.
        const pool = _findPoolMeta((target.poolAddress || '').toLowerCase());
        perLpUsd = (pool && global.lpUsdPrice) ? global.lpUsdPrice(pool) : 0;
      }
  ...
  async function _loadAllPositions(walletAddr) {
    // allSettled: Curve and cross-platform sides are independent. If Curve API
    // burst-fails its capped RPC retries while Convex/SD succeeded (or vice
    // versa), we render the surviving half rather than blanking the dashboard.
    const [curveSettled, crossSettled] = await Promise.allSettled([
      loadPortfolioPositions(walletAddr),
      loadCrossPlatformPositions(walletAddr),
    ]);
    const curvePositions = curveSettled.status === 'fulfilled' ? curveSettled.value : [];
    if (curveSettled.status === 'rejected') {
      console.warn('[portfolio] curve load failed:', curveSettled.reason?.message);
    }
    const cross = crossSettled.status === 'fulfilled'
      ? crossSettled.value
      : { convex: new Map(), stakedao: new Map() };
    if (crossSettled.status === 'rejected') {
      console.warn('[portfolio] cross-platform load failed:', crossSettled.reason?.message);
    }
  ...
  async function claimAll() {
    ...
    // Group gauges by their minter (mainnet global Minter vs each sidechain
    // ChildGaugeFactory) — a batch must hit one minter, and only the mainnet
    // Minter has mint_many; factories expose only mint(address).
    const byMinter = new Map();
    for (const p of claimable) {
      const m = (await _minterFor(p.gaugeAddress, signer)).toLowerCase();
      if (!byMinter.has(m)) byMinter.set(m, []);
      byMinter.get(m).push(p);
    }
    const ok = confirm(`Claim CRV from ${claimable.length} gauge${claimable.length === 1 ? '' : 's'}?`);
    if (!ok) return;
    let success = 0, failed = 0;
    for (const [m, list] of byMinter) {
      const minter = new global.ethers.Contract(m, MINTER_ABI, signer);
      if (m === MINTER_ADDR.toLowerCase()) {
        for (let i = 0; i < list.length; i += 8) {
          const slice = list.slice(i, i + 8);
          const batch = slice.map(p => p.gaugeAddress);
          while (batch.length < 8) batch.push(ZERO_ADDR);
          try { const tx = await minter.mint_many(batch); await tx.wait(); success += slice.length; }
          catch (e) { console.warn('[portfolio] mint_many batch failed', e?.message); failed += slice.length; }
        }
      } else {
        for (const p of list) {
          try { const tx = await minter.mint(p.gaugeAddress); await tx.wait(); success += 1; }
          catch (e) { console.warn('[portfolio] factory.mint failed', e?.message); failed += 1; }
        }
      }
    }
  ...
  // Amount ceiling in BigInt wei (NEVER float — float round-trips overshoot the
  // exact on-chain balance and revert the tx). Display still formats via
  // formatUnits, but every amount computation stays integer wei.
  function _maxWei() {
    if (!_modalCtx) return 0n;
    const p = _lastPositions[_modalCtx.idx];
    if (!p) return 0n;
    const op = _modalCtx.activeOp || _modalCtx.mode;
    if (op === 'stake') return p.walletWei || 0n;
    if (op === 'unstake') return p.stakedWei || 0n;
    return 0n;
  }

  function _applyPreset(pct) {
    const maxWei = _maxWei();
    const amtWei = maxWei * BigInt(Math.round(pct)) / 100n;
    const input = document.getElementById('pmAmountInput');
    if (input) {
      // Exact: MAX (100%) yields the on-chain balance to the wei, no rounding.
      input.value = amtWei > 0n ? global.ethers.formatUnits(amtWei, LP_DECIMALS) : '0';
      _onAmountInput();
    }
  }
  ...
  function _onAmountInput() {
    ...
    let amtWei = null;
    try { amtWei = raw ? global.ethers.parseUnits(raw, LP_DECIMALS) : null; } catch (e) { amtWei = null; }
    const maxWei = _maxWei();
    let invalid = false;
    let statusMsg = '';
    if (amtWei == null || amtWei <= 0n) {
      invalid = true;
    } else if (amtWei > maxWei) {
      invalid = true;
  ...
  async function _updateActionLabel() {
    ...
    try {
      ...
      const walletWei = p.walletWei || 0n;
    ...
    const inputs = coins.map((sym, i) => `
      <div class="pm-coin-row">
        <label class="pm-coin-label">
          <img class="pm-coin-icon" src="${global._tokenIconUrl(addrs[i])}" alt="" onerror="this.style.visibility='hidden'">
          ${sym}
        </label>
  ...
  function _onWithdrawAmountInput() {
    ...
    const p = _lastPositions[_modalCtx.idx];
    const maxWei = p ? (p.walletWei || 0n) : 0n;
    const raw = (input.value || '').trim();
    let amtWei = null;
    try { amtWei = raw ? global.ethers.parseUnits(raw, LP_DECIMALS) : null; } catch (e) { amtWei = null; }
  ...
  // Map opaque ethers errors (estimateGas reverts surfaced as
  // "could not coalesce error" / CALL_EXCEPTION) to a human message.
  function _cleanErr(e) {
    ...
    if (e?.code === 'CALL_EXCEPTION' || /could not coalesce|missing revert|UNKNOWN_ERROR/i.test(m)) {
      return 'transaction reverted on estimate \u2014 likely the amount exceeds your balance, or this action is not available for this pool/chain.';
    }
    return m;
  }

  // Resolve the CRV minter for a gauge. Mainnet gauges mint via the global
  // Minter; sidechain (cross-chain) child-gauges mint via their own
  // ChildGaugeFactory, which gauge.factory() returns and which exposes
  // mint(address). Falls back to the global Minter when there is no factory().
  async function _minterFor(gaugeAddr, signer) {
    try {
      const g = new global.ethers.Contract(gaugeAddr, ['function factory() view returns (address)'], signer);
      const fac = await g.factory();
      if (fac && fac !== ZERO_ADDR) return fac;
    } catch (e) { /* mainnet gauge: no factory() -> global Minter */ }
    return MINTER_ADDR;
  }
  ...
  async function _buildManageSteps(op, p) {
    ...
    if (op === 'deposit' || op === 'deposit-and-stake') {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const wei = _parseAmountStrict(inp ? inp.value : '', dec, (typeof ethersLib !== 'undefined' ? ethersLib : global.ethers));
        if (wei > 0n) hasAny = true;
        try { amounts.push(wei); }
      ...
      if (global.walletAddress) {
        for (let i = 0; i < n; i++) {
          ...
          steps.push({
            ...
            data: window._buildApproveCalldata(pool.address, amounts[i]),
            fallback: window._GAS_FALLBACK.approve,
          });
        }
      }
      // add_liquidity
      const addIface = new ethersLib.Interface([window.poolLiquidityAbi(pool, n).add]);
      let ethValue = 0n;
      for (let i = 0; i < n; i++) {
      ...
      if (op === 'deposit-and-stake') {
        ...
        steps.push({
          ...
          // Оценка газа: LP ещё не существует, сумма неизвестна — см. yield.js.
          data: window._buildApproveCalldata(p.gaugeAddress, 1n),
          fallback: window._GAS_FALLBACK.approve,
        });
        const gIface = new ethersLib.Interface(['function deposit(uint256)']);
        steps.push({
    ...
    if (op === 'withdraw') {
      ...
      const rmIface = new ethersLib.Interface([window.poolLiquidityAbi(pool, n).remove]);
      return [{
        label: 'Withdraw',
        to: pool.address,
  ...
  async function executeStake(idx, amountStr) {
    ...
    try {
      ...
      const lpBalNow = await lp.balanceOf(global.walletAddress);
      if (lpBalNow === 0n) { _setStatus('No LP in wallet to stake', 'error'); return; }
      if (amountWei > lpBalNow) amountWei = lpBalNow;
      const allowance = await lp.allowance(global.walletAddress, p.gaugeAddress);
      if (allowance < amountWei) {
        _setStatus('Approving LP token…', '');
        for (const amt of window.approveAmounts(allowance, amountWei)) {
          const txA = await window.sendContractTxWithDynamicGas(lp, 'approve', [p.gaugeAddress, amt]);
          await txA.wait();
        }
      }
      _setStatus('Staking…', '');
      ...
      let tx;
      try {
        tx = await window.sendContractTxWithDynamicGas(gauge, 'deposit(uint256,address,bool)', [amountWei, walletAddr, false]);
      } catch (e1) {
        try {
          tx = await window.sendContractTxWithDynamicGas(gauge, 'deposit(uint256,address)', [amountWei, walletAddr]);
        } catch (e2) {
          tx = await window.sendContractTxWithDynamicGas(gauge, 'deposit(uint256)', [amountWei]);
        }
      }
      ...
      else _setStatus('Stake failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function executeUnstake(idx, amountStr) {
    ...
    try {
      ...
      const gauge = new global.ethers.Contract(p.gaugeAddress, global.GAUGE_ABI, signer);
      // Clamp to exact on-chain staked balance (integer wei) — never send more
      // than is staked (that reverts the gauge -> opaque "could not coalesce").
      const stakedNow = await gauge.balanceOf(global.walletAddress);
      if (stakedNow === 0n) { _setStatus('Nothing staked to unstake', 'error'); return; }
      if (amountWei > stakedNow) amountWei = stakedNow;
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Unstake failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function executeClaimRewards(idx) {
    ...
    try {
      ...
      // CRV is emitted via the mainnet Minter; on sidechains (e.g. Fantom) that
      // address has no code — claiming via it is invalid (no-op / revert). Tell
      // the user instead of failing with an opaque coalesce error.
      const minterAddr = await _minterFor(p.gaugeAddress, signer);
      const minter = new global.ethers.Contract(minterAddr, MINTER_ABI, signer);
      const tx = await minter.mint(p.gaugeAddress);
      console.log('[portfolio] mint sent', tx.hash, ETHERSCAN_TX + tx.hash);
      await tx.wait();
      ...
      console.error('[portfolio] claim failed', e);
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Claim failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function claimCRV(idx) {
    ...
    try {
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Claim CRV failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function claimExtras(idx) {
    ...
    try {
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Claim extras failed: ' + _cleanErr(e), 'error');
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (const amt of window.approveAmounts(allowance, amounts[i])) {
          await (await window.sendContractTxWithDynamicGas(token, 'approve', [pool.address, amt])).wait();
        }
      }
      // Step 2: add_liquidity
      _setStatus('Step 2/3: adding liquidity…', '');
      const liqAbi = window.poolLiquidityAbi(pool, n);
      let minMint = 0n;
      try {
        const calcIface = new global.ethers.Interface([liqAbi.calc]);
      ...
      const addIface = new global.ethers.Interface([liqAbi.add]);
      ...
      try {
        await (await window.sendContractTxWithDynamicGas(gaugeContract, 'deposit(uint256,address,bool)', [lpBal, global.walletAddress, false])).wait();
      } catch (_) {
        try {
          await (await window.sendContractTxWithDynamicGas(gaugeContract, 'deposit(uint256,address)', [lpBal, global.walletAddress])).wait();
        } catch (__) {
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function executeDeposit(idx) {
    ...
    try {
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Deposit failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function executeWithdraw(idx, amountStr) {
    ...
    try {
      ...
      const lpTok = new global.ethers.Contract(p.lpTokenAddress || pool.address, global.ERC20_ABI, signer);
      const lpBalNow = await lpTok.balanceOf(global.walletAddress);
      if (lpBalNow === 0n) { _setStatus('No LP in wallet to withdraw', 'error'); return; }
      if (amountWei > lpBalNow) amountWei = lpBalNow;
      const n = pool.coins.length;
      const minAmounts = new Array(n).fill(0n);
      const iface = new global.ethers.Interface([window.poolLiquidityAbi(pool, n).remove]);
      const contract = new global.ethers.Contract(pool.address, iface, signer);
      const tx = await window.sendContractTxWithDynamicGas(contract, 'remove_liquidity', [amountWei, minAmounts]);
      console.log('[portfolio] withdraw sent', tx.hash, ETHERSCAN_TX + tx.hash);
      await tx.wait();
      ...
      if (_isUserRejection(e)) _setStatus('Transaction rejected', 'error');
      else _setStatus('Withdraw failed: ' + _cleanErr(e), 'error');
    }
  }
  ...
  async function _refreshOnePosition(idx) {
    ...
    try {
      ...
      p.walletWei = walletBn;
      p.stakedWei = stakedBn;
      ...
      } else if (pool && global.lpUsdPrice) {
        p.usdValue = p.totalLP * global.lpUsdPrice(pool);
      }
  ...
  function menuDisconnect() {
    ...
    const _rp = (typeof window.routePath === 'function') ? window.routePath() : window.location.hash;
    if (_rp === '#/portfolio') global.navigate('#/yield');
  }
