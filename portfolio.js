  ...
  async function executeStake(idx, amountStr) {
    ...
    try {
      ...
      if (allowance < amountWei) {
        ...
        const txA = await window.sendContractTxWithDynamicGas(lp, 'approve', [p.gaugeAddress, global.ethers.MaxUint256]);
        await txA.wait();
      }
      _setStatus('Staking…', '');
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (allowance < amounts[i]) {
          await (await window.sendContractTxWithDynamicGas(token, 'approve', [pool.address, global.ethers.MaxUint256])).wait();
        }
      }
      ...
      if (allowance < lpBal) {
        await (await window.sendContractTxWithDynamicGas(lpToken, 'approve', [gauge, global.ethers.MaxUint256])).wait();
      }
