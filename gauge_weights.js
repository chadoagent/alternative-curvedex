...
(function() {
  ...
  const collectorBase = () =>
    ((typeof window !== 'undefined' && window.__DYNAMIC_BASE) || '') + '/curvedex/collector/';

  // Какие недельные снапшоты вообще существуют. Без этого страница на КАЖДОЙ
  // загрузке просила gauge_weights_<текущая неделя>.json и получала 404:
  // сборщик снапшотов не запускался с 02.05.26 (последняя неделя — 2939).
  let _snapWeeks = null;
  async function snapshotWeekAvailable(weekIdx) {
    if (_snapWeeks === null) {
      _snapWeeks = fetch(collectorBase() + 'gauge_weights_index.json', { cache: 'force-cache' })
        .then(r => (r.ok ? r.json() : null))
        .then(j => new Set((j && Array.isArray(j.weeks)) ? j.weeks : []))
        .catch(() => new Set());
    }
    return (await _snapWeeks).has(weekIdx);
  }
  ...
  async function fetchPrevWeekWeights(gaugeAddrs, prevTs) {
    ...
    if (todoBefore.length > 0 && await snapshotWeekAvailable(prevWeekIdx)) {
      try {
        const snapUrl = collectorBase() + `gauge_weights_${prevWeekIdx}.json`;
