...
// Keyed by build: a release that fixes the CATALOG (2026-09-22, the published
// catalog had shrunk to 293 tokens) must not wait out the hour-long TTL in
// every tester's browser — a new __APP_VERSION__ invalidates the copy.
...
// A cache.json with empty `tokens` is a broken upstream build (2026-07-12: the
// aggregator's Pharos source 401'd and emitted tokens:{}; users who visited
// during the outage had the empty blob stuck in localStorage for the whole TTL,
// surviving hard-refresh). Never serve or store such a payload — fall through
// to the network instead, so recovered data heals stuck clients automatically.
// Canonical asset name without the chain qualifier a bridged deployment adds:
// "USD Coin (Arb1)" -> "usd coin", so a wrapper can be matched to its origin.
function _ratingsNormName(s) {
  return String(s || '').toLowerCase().replace(/\s*\([^)]*\)/g, ' ')
    .replace(/\s+/g, ' ').trim();
}

function _ratingsCacheUsable(data) {
  return !!(data && data.tokens && Object.keys(data.tokens).length);
}
...
async function fetchRatingsCache() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.ts && (Date.now() - parsed.ts) < RATINGS_LS_TTL_MS
          && _ratingsCacheUsable(parsed.data)) {
        return parsed.data;
      }
    }
  } catch (_) { /* fall through to network */ }

  // Drop copies left by previous builds — the blob is ~500 KB and localStorage
  // is a few MB, so keeping one per release would eventually hit the quota.
  try {
    for (const k of Object.keys(localStorage)) {
      if (k.startsWith('curvedex.ratingsCache') && k !== RATINGS_LS_KEY) localStorage.removeItem(k);
    }
  ...
  _ratingsCachePromise = (async () => {
    try {
      ...
      try {
        if (_ratingsCacheUsable(data)) {
          localStorage.setItem(RATINGS_LS_KEY, JSON.stringify({ ts: Date.now(), data }));
        } else {
          localStorage.removeItem(RATINGS_LS_KEY); // purge a previously-stuck empty blob
        }
...
// Yearn Curation risk score (1=safest … 5=riskiest) -> letter grade.
// Monotone натяжка confirmed by Alexandr 2026-07-01: integers 1=A,2=B,3=C,4=D,5=F;
// thirds give +/- (1.5 -> A-). Rendered via _gradeColor on our palette.
function _yearnGrade(s) {
  if (typeof s !== 'number' || isNaN(s)) return null;
  const bands = [[1.333, 'A'], [1.667, 'A-'], [2.0, 'B+'], [2.333, 'B'], [2.667, 'B-'], [3.0, 'C+'], [3.333, 'C'], [3.667, 'C-'], [4.0, 'D+'], [4.333, 'D'], [4.667, 'D-']];
  for (const [hi, g] of bands) if (s < hi) return g;
  return 'F';
}
...
function _renderCard(symbol, address, entry, deployAges) {
  ...
  const y = entry.yearn && typeof entry.yearn === 'object' ? entry.yearn : null;
  const yGrade = y && typeof y.score === 'number' ? _yearnGrade(y.score) : null;
  const yearnRow = y ? `<div class="info-src-row"><span class="info-src-label">Yearn</span><span class="info-src-val" style="color:${_gradeColor(yGrade)}">${_esc(yGrade || '\u2014')}${typeof y.score === 'number' ? ' \u00b7 ' + y.score.toFixed(2) + '/5' : ''}</span>${y.url ? `<a href="${_esc(y.url)}" target="_blank" rel="noopener" class="info-src-link">\u2197</a>` : ''}</div>` : '';
  const research = (() => {
    const base = Array.isArray(entry.research) ? entry.research : [];
    const notes = [];
    const k = entry.kyp && typeof entry.kyp === 'object' ? entry.kyp : null;
    if (k) {
      const verdict = 'Permission audit \u2014 Gate: ' + (k.gate || '\u2014') + (k.riskLevel ? ' \u00b7 ' + k.riskLevel : '');
      notes.push({ source: 'KYP', title: verdict, url: k.url || 'https://www.kyp.one/', author: 'kyp.one', date: k.auditDate || '' });
    }
    if (y && typeof y.score === 'number') {
      const yv = 'Risk score ' + y.score.toFixed(2) + '/5 \u00b7 ' + (y.tier || '');
      notes.push({ source: 'Yearn', title: yv, url: y.url || 'https://curation.yearn.fi/', author: 'curation.yearn.fi', date: y.date || '' });
    }
    return [...notes, ...base];
  })();
  ...
  return `
    <div class="info-card">
      <div class="info-card-head">
        ...
        ${(pharos.grade != null) ? `<span class="info-grade-badge" style="background:${gradeColor};">${_esc(pharos.grade)}${pharos.score != null ? ' · ' + pharos.score : ''}</span>` : (y && yGrade ? `<span class="info-grade-badge" title="Yearn Curation \u00b7 risk score ${y.score.toFixed(2)}/5 mapped to a grade (approximation)" style="background:${_gradeColor(yGrade)};">${_esc(yGrade)}</span>` : `<span class="info-grade-badge" style="background:${gradeColor};">—</span>`)}
      </div>
      ${inheritedBadge}
      ${descr}
...
(async function _seedRatingsIndex() {
  try {
    ...
    for (const [addr, entry] of Object.entries(cache.tokens)) {
      ...
      else if (entry && entry.yearn && typeof entry.yearn.score === 'number') grades.set(key, _yearnGrade(entry.yearn.score));
      // Name index — lets a bridged wrapper inherit its canonical asset's grade
      // (see _linkBridgedTokenGrades in app.js).
      const nm = _ratingsNormName(entry && entry.name);
      const gr = grades.get(key);
      if (nm && gr && entry && entry.symbol && !byName.has(nm)) {
        byName.set(nm, { symbol: String(entry.symbol), grade: gr });
      }
    }
    window._ratingsTokens = tokens;
    window._ratingsGrades = grades;
    window._ratingsByName = byName;
    // The catalog normally lands after the pool list, so the link pass in
    // app.js has already run with an empty index — run it again now.
    if (typeof _linkBridgedTokenGrades === 'function') _linkBridgedTokenGrades();
