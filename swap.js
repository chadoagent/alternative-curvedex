...
// Non-swap consumers of this modal (the Zap tabs) register {select, current}.
function _swapModalPicker() {
  const reg = typeof window !== 'undefined' ? window._zapPickers : null;
  return (reg && reg[swapTokenModalTarget]) || null;
}

/**
 * Связность токена: в скольких пулах он вообще встречается.
 *
 * Это и есть проверка на подделку, не требующая никаких списков: чтобы попасть
 * в несколько пулов Curve, нужна настоящая ликвидность, а скам-токен под чужим
 * тикером сидит в одном своём пуле. 11.08 в выборе токена было три «USDT» —
 * настоящий (267 пулов) стоял ТРЕТЬИМ, потому что подделки раздают аирдропом,
 * и модалка поднимала наверх всё, на что есть баланс.
 */
function _tokenReach(t) {
  return (t && t.pools && typeof t.pools.size === 'number') ? t.pools.size : 0;
}

/**
 * Баланс, пригодный для сортировки. Число вида 1.4e70 — это не богатство, а
 * сломанные decimals у подделки; такое значение в ключ сортировки не пускаем.
 */
function _sortableBal(e) {
  const usd = (e && isFinite(e.usdValue)) ? e.usdValue : 0;
  const bal = (e && isFinite(e.balance)) ? e.balance : 0;
  const SANE = 1e15; // ни один настоящий баланс в человеческих единицах столько не весит
  return {
    usd: (usd > 0 && usd < SANE) ? usd : 0,
    bal: (bal > 0 && bal < SANE) ? bal : 0,
  };
}

function _reachClass(t) {
  const p = _tokenReach(t);
  return p > 0 ? Math.floor(Math.log10(p)) : -1;
}

/**
 * Лучший (самый связный) носитель каждого тикера во ВСЁМ списке токенов.
 * Клон опознаётся сравнением с ним, а не списком адресов: подделка под чужой
 * тикер всегда на порядки менее связна, чем оригинал (267 пулов против 1-2).
 */
function _bestReachBySymbol(tokens) {
  const best = new Map();
  for (const t of tokens) {
    const s = (t.symbol || '').trim().toLowerCase();
    if (!s) continue;
    const c = _reachClass(t);
    if (!best.has(s) || c > best.get(s)) best.set(s, c);
  }
  return best;
}

/**
 * Подозрительный токен — тот, что уходит ВНИЗ списка, даже если он есть на
 * кошельке. Три признака, ни один не требует чёрных списков:
 *   1) сломанные числа (баланс 1.4e70 — это не богатство, а битые decimals);
 *   2) тикер занят токеном на порядок связнее — то есть это клон;
 *   3) один пул и нулевой оборот — торговать нечем, что бы там ни лежало.
 */
function _isSuspiciousToken(t, bestBySymbol, entry) {
  const SANE = 1e15;
  if (entry) {
    const u = entry.usdValue, b = entry.balance;
    if ((u != null && (!isFinite(u) || u > SANE)) || (b != null && (!isFinite(b) || b > SANE))) return true;
  }
  const sym = (t.symbol || '').trim().toLowerCase();
  const top = bestBySymbol.has(sym) ? bestBySymbol.get(sym) : -1;
  if (_reachClass(t) < top) return true;
  if (_tokenReach(t) <= 1 && !(t.volume24h > 0)) return true;
  return false;
}

/**
 * Порядок в модалке выбора токена: СВОИ ТОКЕНЫ СВЕРХУ, по стоимости позиции.
 * Подделки не прячем, а опускаем вниз (решение владельца 11.08). Токены без
 * баланса сохраняют базовый порядок списка (оборот за 24ч) — сортировка
 * стабильна, поэтому для них компаратор возвращает 0.
 */
function _sortSwapTokensByBalance(tokens, balMap) {
  if (!balMap || balMap.size === 0) return tokens;
  const universe = (typeof tradeTokenList !== 'undefined' && tradeTokenList && tradeTokenList.length) ? tradeTokenList : tokens;
  const bestBySymbol = _bestReachBySymbol(universe);
  const info = new Map();
  for (const t of tokens) {
    const entry = balMap.get(t.address.toLowerCase());
    info.set(t, {
      susp: _isSuspiciousToken(t, bestBySymbol, entry) ? 1 : 0,
      bal: _sortableBal(entry),
    });
  }
  const cmp = (a, b) => {
    const ia = info.get(a), ib = info.get(b);
    if (ia.susp !== ib.susp) return ia.susp - ib.susp;
    const ha = (ia.bal.usd > 0 || ia.bal.bal > 0) ? 1 : 0;
    const hb = (ib.bal.usd > 0 || ib.bal.bal > 0) ? 1 : 0;
    if (ha !== hb) return hb - ha;
    if (ha === 0) return 0;
    if (ib.bal.usd !== ia.bal.usd) return ib.bal.usd - ia.bal.usd;
    if (ib.bal.bal !== ia.bal.bal) return ib.bal.bal - ia.bal.bal;
    return _tokenReach(b) - _tokenReach(a);
  };
  return [...tokens].sort(cmp);
}

function _renderSwapTokenModalRows(showList, balMap) {
  return showList.map(t => {
    const poolCount = t.pools ? t.pools.size : 0;
    // The Zap tabs (yield view) borrow this modal: same list, same balances,
    // only the click target and the "selected" marker differ. Each registers
    // itself in window._zapPickers, so this stays one code path.
    const picker = _swapModalPicker();
    const picked = picker ? picker.current() : null;
    const isSelected = picker
      ? (!!picked && String(picked.address).toLowerCase() === t.address.toLowerCase())
      : ((swapTokenModalTarget === 'from' && swapSelectedFrom && swapSelectedFrom.address === t.address) ||
    ...
    const balLine = balStr
      ...
      : '';
    return `<div class="token-modal-item${isSelected ? ' selected' : ''}" onclick="${picker ? picker.select : 'selectSwapToken'}('${t.address}')">
...
function renderSwapTokenModalList(query) {
  ...
  if (show.length === 0) {
    ...
    // Zap accepts any token, not only those held by Curve pools -> ask the router.
    if (_swapModalPicker() && typeof zapEnsoSearchInto === 'function') zapEnsoSearchInto(list, query);
    return;
  }
...
async function fetchSwapQuote() {
  ...
  try {
    ...
    // Минимум к получению — то самое число, что уходит в транзакцию (min_dy).
    // Если маршрут стоит на премии мелкого пула, запас расширен, и человек должен
    // видеть это ДО подписи, а не впервые в кошельке.
    const minOutEl = document.getElementById('swapViewMinOut');
    if (minOutEl) {
      const mo = quote.minOutput != null ? parseFloat(quote.minOutput) : null;
      const hr = quote.slippage != null ? Number(quote.slippage) : null;
      minOutEl.textContent = (mo != null && isFinite(mo))
        ? `${mo.toFixed(6)} ${swapSelectedTo.symbol}` + (hr != null ? ` (slip ${hr.toFixed(2)}%)` : '')
        : '--';
      minOutEl.style.color = (hr != null && hr > swapSlippage + 1e-9) ? 'var(--red)' : '';
    }
...
async function handleSwapViewSubmit() {
  ...
  try {
    ...
    await router.ensureApprovalForTx(txParams, swapLastQuote, walletAddress, signer);
